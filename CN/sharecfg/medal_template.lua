pg = pg or {}
pg.medal_template = rawget(pg, "medal_template") or setmetatable({
	__name = "medal_template"
}, confNEO)
pg.medal_template.all = {
	10011,
	10012,
	10013,
	10014,
	10015,
	10021,
	10022,
	10023,
	10024,
	10025,
	10031,
	10032,
	10033,
	10034,
	10035,
	10041,
	10042,
	10043,
	10044,
	10045,
	10051,
	10052,
	10053,
	10054,
	10055,
	10061,
	10062,
	10063,
	10064,
	10065,
	10071,
	10072,
	10073,
	10074,
	10075,
	10081,
	10082,
	10083,
	10084,
	10085,
	10091,
	10092,
	10093,
	10094,
	10095,
	10101,
	10102,
	10103,
	10104,
	10105,
	100611,
	100612,
	100613,
	100614,
	100615,
	100621,
	100622,
	100623,
	100624,
	100625,
	100631,
	100632,
	100633,
	100634,
	100635,
	100711,
	100712,
	100713,
	100714,
	100715,
	100721,
	100722,
	100723,
	100724,
	100725,
	100731,
	100732,
	100733,
	100734,
	100735,
	100811,
	100812,
	100813,
	100814,
	100815,
	100821,
	100822,
	100823,
	100824,
	100825,
	100831,
	100832,
	100833,
	100834,
	100835,
	100911,
	100912,
	100913,
	100914,
	100915,
	100921,
	100922,
	100923,
	100924,
	100925,
	100931,
	100932,
	100933,
	100934,
	100935,
	200001,
	200011,
	200021,
	200031,
	200041,
	200051,
	200061,
	200071,
	200081,
	200091,
	200101,
	200111,
	200121,
	200131,
	200141,
	200151,
	200161,
	200171,
	200181,
	200191,
	200201,
	200211
}
pg.base = pg.base or {}
pg.base.medal_template = {}

;(function()
	pg.base.medal_template[10011] = {
		explain1 = "授予在服役期间",
		name = "海军优秀服役勋章",
		explain2 = "表现优秀的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "该勋章授予在服役期间表现优秀的指挥官",
		next = 10012,
		icon = "2021",
		page = 1,
		target_type = 1022,
		start = 0,
		hide = 0,
		label = "l_201",
		condition = "舰队出征累积胜利2000次 ",
		rank = 1,
		id = 10011,
		target_num = 2000,
		target_id = "0"
	}
	pg.base.medal_template[10012] = {
		explain1 = "授予在服役期间",
		name = "海军优秀服役勋章",
		explain2 = "表现优秀的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "该勋章授予在服役期间表现优秀的指挥官",
		next = 10013,
		icon = "2022",
		page = 1,
		target_type = 1022,
		start = 1,
		hide = 0,
		label = "l_201",
		condition = "舰队出征累积胜利4000次 ",
		rank = 2,
		id = 10012,
		target_num = 4000,
		target_id = "0"
	}
	pg.base.medal_template[10013] = {
		explain1 = "授予在服役期间",
		name = "海军优秀服役勋章",
		explain2 = "表现优秀的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "该勋章授予在服役期间表现优秀的指挥官",
		next = 10014,
		icon = "2023",
		page = 1,
		target_type = 1022,
		start = 1,
		hide = 0,
		label = "l_201",
		condition = "舰队出征累积胜利10000次 ",
		rank = 3,
		id = 10013,
		target_num = 10000,
		target_id = "0"
	}
	pg.base.medal_template[10014] = {
		explain1 = "授予在服役期间",
		name = "海军优秀服役勋章",
		explain2 = "表现优秀的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "该勋章授予在服役期间表现优秀的指挥官",
		next = 10015,
		icon = "2024",
		page = 1,
		target_type = 1022,
		start = 1,
		hide = 0,
		label = "l_201",
		condition = "舰队出征累积胜利20000次 ",
		rank = 4,
		id = 10014,
		target_num = 20000,
		target_id = "0"
	}
	pg.base.medal_template[10015] = {
		explain1 = "授予在服役期间",
		name = "海军优秀服役勋章",
		explain2 = "表现优秀的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "该勋章授予在服役期间表现优秀的指挥官",
		next = 0,
		icon = "2025",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_201",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 10015,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10021] = {
		explain1 = "授予在服役期间",
		name = "恪尽职守勋章",
		explain2 = "恪守职责的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 10022,
		desc = "该勋章授予在服役期间出色履行职责的指挥官",
		next = 10022,
		icon = "2051",
		page = 1,
		target_type = 91,
		start = 0,
		hide = 0,
		label = "l_202",
		condition = "累积完成每日任务100次",
		rank = 1,
		id = 10021,
		target_num = 100,
		target_id = "0"
	}
	pg.base.medal_template[10022] = {
		explain1 = "授予在服役期间",
		name = "恪尽职守勋章",
		explain2 = "恪守职责的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 10023,
		desc = "该勋章授予在服役期间出色履行职责的指挥官",
		next = 10023,
		icon = "2052",
		page = 1,
		target_type = 91,
		start = 1,
		hide = 0,
		label = "l_202",
		condition = "累积完成每日任务300次",
		rank = 2,
		id = 10022,
		target_num = 300,
		target_id = "0"
	}
	pg.base.medal_template[10023] = {
		explain1 = "授予在服役期间",
		name = "恪尽职守勋章",
		explain2 = "恪守职责的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 10024,
		desc = "该勋章授予在服役期间出色履行职责的指挥官",
		next = 10024,
		icon = "2053",
		page = 1,
		target_type = 91,
		start = 1,
		hide = 0,
		label = "l_202",
		condition = "累积完成每日任务800次",
		rank = 3,
		id = 10023,
		target_num = 800,
		target_id = "0"
	}
	pg.base.medal_template[10024] = {
		explain1 = "授予在服役期间",
		name = "恪尽职守勋章",
		explain2 = "恪守职责的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 10025,
		desc = "该勋章授予在服役期间出色履行职责的指挥官",
		next = 10025,
		icon = "2054",
		page = 1,
		target_type = 91,
		start = 1,
		hide = 0,
		label = "l_202",
		condition = "累积完成每日任务1600次",
		rank = 4,
		id = 10024,
		target_num = 1600,
		target_id = "0"
	}
	pg.base.medal_template[10025] = {
		explain1 = "授予在服役期间",
		name = "恪尽职守勋章",
		explain2 = "恪守职责的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "该勋章授予在服役期间出色履行职责的指挥官",
		next = 0,
		icon = "2055",
		page = 1,
		target_type = 91,
		start = 1,
		hide = 0,
		label = "l_202",
		condition = "累积完成每日任务3200次",
		rank = 5,
		id = 10025,
		target_num = 3200,
		target_id = "0"
	}
	pg.base.medal_template[10031] = {
		explain1 = "授予在信念坚定",
		name = "海洋探索者勋章",
		explain2 = "持之以恒的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 10032,
		desc = "探索不总伴随有收获。该勋章授予拥有坚定信念并持之以恒的指挥官",
		next = 10032,
		icon = "2031",
		page = 1,
		target_type = 141,
		start = 0,
		hide = 0,
		label = "l_203",
		condition = "从关卡中累积掉落获得1名超稀有角色",
		rank = 1,
		id = 10031,
		target_num = 1,
		target_id = "5"
	}
	pg.base.medal_template[10032] = {
		explain1 = "授予在信念坚定",
		name = "海洋探索者勋章",
		explain2 = "持之以恒的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 10033,
		desc = "探索不总伴随有收获。该勋章授予拥有坚定信念并持之以恒的指挥官",
		next = 10033,
		icon = "2032",
		page = 1,
		target_type = 141,
		start = 1,
		hide = 0,
		label = "l_203",
		condition = "从关卡中累积掉落获得3名超稀有角色",
		rank = 2,
		id = 10032,
		target_num = 3,
		target_id = "5"
	}
	pg.base.medal_template[10033] = {
		explain1 = "授予在信念坚定",
		name = "海洋探索者勋章",
		explain2 = "持之以恒的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 10034,
		desc = "探索不总伴随有收获。该勋章授予拥有坚定信念并持之以恒的指挥官",
		next = 10034,
		icon = "2033",
		page = 1,
		target_type = 141,
		start = 1,
		hide = 0,
		label = "l_203",
		condition = "从关卡中累积掉落获得10名超稀有角色",
		rank = 3,
		id = 10033,
		target_num = 10,
		target_id = "5"
	}
	pg.base.medal_template[10034] = {
		explain1 = "授予在信念坚定",
		name = "海洋探索者勋章",
		explain2 = "持之以恒的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "探索不总伴随有收获。该勋章授予拥有坚定信念并持之以恒的指挥官",
		next = 10035,
		icon = "2034",
		page = 1,
		target_type = 141,
		start = 1,
		hide = 0,
		label = "l_203",
		condition = "从关卡中累积掉落获得20名超稀有角色",
		rank = 4,
		id = 10034,
		target_num = 20,
		target_id = "5"
	}
	pg.base.medal_template[10035] = {
		explain1 = "授予在信念坚定",
		name = "海洋探索者勋章",
		explain2 = "持之以恒的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "探索不总伴随有收获。该勋章授予拥有坚定信念并持之以恒的指挥官",
		next = 0,
		icon = "2035",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_203",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 10035,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10041] = {
		explain1 = "授予为港区建设",
		name = "杰出服务勋章",
		explain2 = "贡献杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "家，甜蜜的家。该勋章授予在港区建设中作出杰出贡献的指挥官",
		next = 10042,
		icon = "2041",
		page = 1,
		target_type = 133,
		start = 0,
		hide = 0,
		label = "l_204",
		condition = "累积消耗家具币达到3000枚",
		rank = 1,
		id = 10041,
		target_num = 3000,
		target_id = "0"
	}
	pg.base.medal_template[10042] = {
		explain1 = "授予为港区建设",
		name = "杰出服务勋章",
		explain2 = "贡献杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "家，甜蜜的家。该勋章授予在港区建设中作出杰出贡献的指挥官",
		next = 10043,
		icon = "2042",
		page = 1,
		target_type = 133,
		start = 1,
		hide = 0,
		label = "l_204",
		condition = "累积消耗家具币达到7000枚",
		rank = 2,
		id = 10042,
		target_num = 7000,
		target_id = "0"
	}
	pg.base.medal_template[10043] = {
		explain1 = "授予为港区建设",
		name = "杰出服务勋章",
		explain2 = "贡献杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "家，甜蜜的家。该勋章授予在港区建设中作出杰出贡献的指挥官",
		next = 10044,
		icon = "2043",
		page = 1,
		target_type = 133,
		start = 1,
		hide = 0,
		label = "l_204",
		condition = "累积消耗家具币达到15000枚",
		rank = 3,
		id = 10043,
		target_num = 15000,
		target_id = "0"
	}
	pg.base.medal_template[10044] = {
		explain1 = "授予为港区建设",
		name = "杰出服务勋章",
		explain2 = "贡献杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "家，甜蜜的家。该勋章授予在港区建设中作出杰出贡献的指挥官",
		next = 10045,
		icon = "2044",
		page = 1,
		target_type = 133,
		start = 1,
		hide = 0,
		label = "l_204",
		condition = "累积消耗家具币达到30000枚",
		rank = 4,
		id = 10044,
		target_num = 30000,
		target_id = "0"
	}
	pg.base.medal_template[10045] = {
		explain1 = "授予为港区建设",
		name = "杰出服务勋章",
		explain2 = "贡献杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "家，甜蜜的家。该勋章授予在港区建设中作出杰出贡献的指挥官",
		next = 0,
		icon = "2045",
		page = 1,
		target_type = 133,
		start = 1,
		hide = 0,
		label = "l_204",
		condition = "累积消耗家具币达到60000枚",
		rank = 5,
		id = 10045,
		target_num = 60000,
		target_id = "0"
	}
	pg.base.medal_template[10051] = {
		explain1 = "点滴中耕耘的羁绊",
		name = "玫瑰誓约纪念章",
		explain2 = "如玫瑰般绽放于心间 ",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "点滴中耕耘的羁绊，如玫瑰般绽放于心间。 ",
		next = 10052,
		icon = "2061",
		page = 1,
		target_type = 1019,
		start = 0,
		hide = 0,
		label = "l_205",
		condition = "舰队中好感度大于等于100的舰船达到10艘",
		rank = 1,
		id = 10051,
		target_num = 10,
		target_id = "10000"
	}
	pg.base.medal_template[10052] = {
		explain1 = "点滴中耕耘的羁绊",
		name = "玫瑰誓约纪念章",
		explain2 = "如玫瑰般绽放于心间 ",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "点滴中耕耘的羁绊，如玫瑰般绽放于心间。 ",
		next = 10053,
		icon = "2062",
		page = 1,
		target_type = 1019,
		start = 1,
		hide = 0,
		label = "l_205",
		condition = "舰队中好感度大于等于100的舰船达到20艘",
		rank = 2,
		id = 10052,
		target_num = 20,
		target_id = "10000"
	}
	pg.base.medal_template[10053] = {
		explain1 = "点滴中耕耘的羁绊",
		name = "玫瑰誓约纪念章",
		explain2 = "如玫瑰般绽放于心间 ",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "点滴中耕耘的羁绊，如玫瑰般绽放于心间。 ",
		next = 10054,
		icon = "2063",
		page = 1,
		target_type = 1019,
		start = 1,
		hide = 0,
		label = "l_205",
		condition = "舰队中好感度大于等于100的舰船达到50艘",
		rank = 3,
		id = 10053,
		target_num = 50,
		target_id = "10000"
	}
	pg.base.medal_template[10054] = {
		explain1 = "点滴中耕耘的羁绊",
		name = "玫瑰誓约纪念章",
		explain2 = "如玫瑰般绽放于心间 ",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "点滴中耕耘的羁绊，如玫瑰般绽放于心间。 ",
		next = 10055,
		icon = "2064",
		page = 1,
		target_type = 1019,
		start = 1,
		hide = 0,
		label = "l_205",
		condition = "舰队中好感度大于等于100的舰船达到100艘",
		rank = 4,
		id = 10054,
		target_num = 100,
		target_id = "10000"
	}
	pg.base.medal_template[10055] = {
		explain1 = "点滴中耕耘的羁绊",
		name = "玫瑰誓约纪念章",
		explain2 = "如玫瑰般绽放于心间 ",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "点滴中耕耘的羁绊，如玫瑰般绽放于心间。 ",
		next = 0,
		icon = "2065",
		page = 1,
		target_type = 1019,
		start = 1,
		hide = 0,
		label = "l_205",
		condition = "舰队中好感度大于等于100的舰船达到200艘",
		rank = 5,
		id = 10055,
		target_num = 200,
		target_id = "10000"
	}
	pg.base.medal_template[10061] = {
		explain1 = "以白鹰之名授予",
		name = "白鹰海军荣誉勋章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以白鹰之名，授予在白鹰阵营军事行动中作出最杰出贡献的海军指挥官",
		next = 10062,
		icon = "1011",
		page = 1,
		target_type = 160,
		start = 0,
		hide = 0,
		label = "l_101",
		condition = "白鹰阵营120级舰船达到12艘，白鹰阵营舰船总好感达成2000点，使用白鹰舰船在任意关卡中获得经验200万点",
		rank = 1,
		id = 10061,
		target_num = 3,
		explain2 = "最杰出的指挥官",
		target_id = {
			100611,
			100621,
			100631
		}
	}
	pg.base.medal_template[10062] = {
		explain1 = "以白鹰之名授予",
		name = "白鹰海军荣誉勋章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以白鹰之名，授予在白鹰阵营军事行动中作出最杰出贡献的海军指挥官",
		next = 10063,
		icon = "1012",
		page = 1,
		target_type = 160,
		start = 1,
		hide = 0,
		label = "l_101",
		condition = "白鹰阵营120级舰船达到24艘，白鹰阵营舰船总好感达成4000点，使用白鹰舰船在任意关卡中获得经验200万点",
		rank = 2,
		id = 10062,
		target_num = 3,
		explain2 = "最杰出的指挥官",
		target_id = {
			100612,
			100622,
			100632
		}
	}
	pg.base.medal_template[10063] = {
		explain1 = "以白鹰之名授予",
		name = "白鹰海军荣誉勋章",
		explain2 = "最杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以白鹰之名，授予在白鹰阵营军事行动中作出最杰出贡献的海军指挥官",
		next = 10064,
		icon = "1013",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_101",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 10063,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10064] = {
		explain1 = "以白鹰之名授予",
		name = "白鹰海军荣誉勋章",
		explain2 = "最杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以白鹰之名，授予在白鹰阵营军事行动中作出最杰出贡献的海军指挥官",
		next = 10065,
		icon = "1014",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_101",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 10064,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10065] = {
		explain1 = "以白鹰之名授予",
		name = "白鹰海军荣誉勋章",
		explain2 = "最杰出的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以白鹰之名，授予在白鹰阵营军事行动中作出最杰出贡献的海军指挥官",
		next = 0,
		icon = "1015",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_101",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 10065,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10071] = {
		explain1 = "以皇家之名赠予",
		name = "皇家海军十字勋章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以皇家之名，将该荣耀赠予在面对敌人时表现最英勇、最无畏的指挥官",
		next = 10072,
		icon = "1021",
		page = 1,
		target_type = 160,
		start = 0,
		hide = 0,
		label = "l_102",
		condition = "皇家阵营120级舰船达到12艘，皇家阵营舰船总好感达成2000点，使用皇家舰船在任意关卡中获得经验200万点",
		rank = 1,
		id = 10071,
		target_num = 3,
		explain2 = "最英勇的指挥官",
		target_id = {
			100711,
			100721,
			100731
		}
	}
	pg.base.medal_template[10072] = {
		explain1 = "以皇家之名赠予",
		name = "皇家海军十字勋章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以皇家之名，将该荣耀赠予在面对敌人时表现最英勇、最无畏的指挥官",
		next = 10073,
		icon = "1022",
		page = 1,
		target_type = 160,
		start = 1,
		hide = 0,
		label = "l_102",
		condition = "皇家阵营120级舰船达到24艘，皇家阵营舰船总好感达成4000点，使用皇家舰船在任意关卡中获得经验200万点",
		rank = 2,
		id = 10072,
		target_num = 3,
		explain2 = "最英勇的指挥官",
		target_id = {
			100712,
			100722,
			100732
		}
	}
	pg.base.medal_template[10073] = {
		explain1 = "以皇家之名赠予",
		name = "皇家海军十字勋章",
		explain2 = "最英勇的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以皇家之名，将该荣耀赠予在面对敌人时表现最英勇、最无畏的指挥官",
		next = 10074,
		icon = "1023",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_102",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 10073,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10074] = {
		explain1 = "以皇家之名赠予",
		name = "皇家海军十字勋章",
		explain2 = "最英勇的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以皇家之名，将该荣耀赠予在面对敌人时表现最英勇、最无畏的指挥官",
		next = 10075,
		icon = "1024",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_102",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 10074,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10075] = {
		explain1 = "以皇家之名赠予",
		name = "皇家海军十字勋章",
		explain2 = "最英勇的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以皇家之名，将该荣耀赠予在面对敌人时表现最英勇、最无畏的指挥官",
		next = 0,
		icon = "1025",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_102",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 10075,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10081] = {
		explain1 = "以重樱之名授予",
		name = "重樱勋位大绶章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以重樱之名，授予为重樱海军建设作出最卓越功绩的指挥官阁下",
		next = 10082,
		icon = "1031",
		page = 1,
		target_type = 160,
		start = 0,
		hide = 0,
		label = "l_103",
		condition = "重樱阵营120级舰船达到12艘，重樱阵营舰船总好感达成2000点，使用重樱舰船在任意关卡中获得经验200万点",
		rank = 1,
		id = 10081,
		target_num = 3,
		explain2 = "最卓越的指挥官",
		target_id = {
			100811,
			100821,
			100831
		}
	}
	pg.base.medal_template[10082] = {
		explain1 = "以重樱之名授予",
		name = "重樱勋位大绶章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以重樱之名，授予为重樱海军建设作出最卓越功绩的指挥官阁下",
		next = 10083,
		icon = "1032",
		page = 1,
		target_type = 160,
		start = 1,
		hide = 0,
		label = "l_103",
		condition = "重樱阵营120级舰船达到24艘，重樱阵营舰船总好感达成4000点，使用重樱舰船在任意关卡中获得经验200万点",
		rank = 2,
		id = 10082,
		target_num = 3,
		explain2 = "最卓越的指挥官",
		target_id = {
			100812,
			100822,
			100832
		}
	}
	pg.base.medal_template[10083] = {
		explain1 = "以重樱之名授予",
		name = "重樱勋位大绶章",
		explain2 = "最卓越的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以重樱之名，授予为重樱海军建设作出最卓越功绩的指挥官阁下",
		next = 10084,
		icon = "1033",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_103",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 10083,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10084] = {
		explain1 = "以重樱之名授予",
		name = "重樱勋位大绶章",
		explain2 = "最卓越的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以重樱之名，授予为重樱海军建设作出最卓越功绩的指挥官阁下",
		next = 10085,
		icon = "1034",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_103",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 10084,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10085] = {
		explain1 = "以重樱之名授予",
		name = "重樱勋位大绶章",
		explain2 = "最卓越的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以重樱之名，授予为重樱海军建设作出最卓越功绩的指挥官阁下",
		next = 0,
		icon = "1035",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_103",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 10085,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10091] = {
		explain1 = "以铁血之名奖励",
		name = "铁血海军功勋勋章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以铁血之名，奖励在作战中展现了杰出的指挥能力并取得了突出功绩的指挥官",
		next = 10092,
		icon = "1041",
		page = 1,
		target_type = 160,
		start = 0,
		hide = 0,
		label = "l_104",
		condition = "铁血阵营120级舰船达到12艘，铁血阵营舰船总好感达成2000点，使用铁血舰船在任意关卡中获得经验200万点",
		rank = 1,
		id = 10091,
		target_num = 3,
		explain2 = "最出色的指挥官",
		target_id = {
			100911,
			100921,
			100931
		}
	}
	pg.base.medal_template[10092] = {
		explain1 = "以铁血之名奖励",
		name = "铁血海军功勋勋章",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以铁血之名，奖励在作战中展现了杰出的指挥能力并取得了突出功绩的指挥官",
		next = 10093,
		icon = "1042",
		page = 1,
		target_type = 160,
		start = 1,
		hide = 0,
		label = "l_104",
		condition = "铁血阵营120级舰船达到24艘，铁血阵营舰船总好感达成4000点，使用铁血舰船在任意关卡中获得经验200万点",
		rank = 2,
		id = 10092,
		target_num = 3,
		explain2 = "最出色的指挥官",
		target_id = {
			100912,
			100922,
			100932
		}
	}
	pg.base.medal_template[10093] = {
		explain1 = "以铁血之名奖励",
		name = "铁血海军功勋勋章",
		explain2 = "最出色的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以铁血之名，奖励在作战中展现了杰出的指挥能力并取得了突出功绩的指挥官",
		next = 10094,
		icon = "1043",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_104",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 10093,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10094] = {
		explain1 = "以铁血之名奖励",
		name = "铁血海军功勋勋章",
		explain2 = "最出色的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以铁血之名，奖励在作战中展现了杰出的指挥能力并取得了突出功绩的指挥官",
		next = 10095,
		icon = "1044",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_104",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 10094,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10095] = {
		explain1 = "以铁血之名奖励",
		name = "铁血海军功勋勋章",
		explain2 = "最出色的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "以铁血之名，奖励在作战中展现了杰出的指挥能力并取得了突出功绩的指挥官",
		next = 0,
		icon = "1045",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_104",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 10095,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10101] = {
		explain1 = "奖励在舰队扩大中",
		name = "海军服役成就奖章 ",
		explain2 = "有所建树的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "奖励致力于舰队扩大建设并有所建树的指挥官 ",
		next = 10102,
		icon = "2011",
		page = 1,
		target_type = 1044,
		start = 0,
		hide = 0,
		label = "l_206",
		condition = "图鉴中解锁100种不同舰船",
		rank = 1,
		id = 10101,
		target_num = 100,
		target_id = "0"
	}
	pg.base.medal_template[10102] = {
		explain1 = "奖励在舰队扩大中",
		name = "海军服役成就奖章 ",
		explain2 = "有所建树的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "奖励致力于舰队扩大建设并有所建树的指挥官 ",
		next = 10103,
		icon = "2012",
		page = 1,
		target_type = 1044,
		start = 1,
		hide = 0,
		label = "l_206",
		condition = "图鉴中解锁200种不同舰船",
		rank = 2,
		id = 10102,
		target_num = 200,
		target_id = "0"
	}
	pg.base.medal_template[10103] = {
		explain1 = "奖励在舰队扩大中",
		name = "海军服役成就奖章 ",
		explain2 = "有所建树的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "奖励致力于舰队扩大建设并有所建树的指挥官 ",
		next = 10104,
		icon = "2013",
		page = 1,
		target_type = 1044,
		start = 1,
		hide = 0,
		label = "l_206",
		condition = "图鉴中解锁300种不同舰船",
		rank = 3,
		id = 10103,
		target_num = 300,
		target_id = "0"
	}
	pg.base.medal_template[10104] = {
		explain1 = "奖励在舰队扩大中",
		name = "海军服役成就奖章 ",
		explain2 = "有所建树的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "奖励致力于舰队扩大建设并有所建树的指挥官 ",
		next = 10105,
		icon = "2014",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_206",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 10104,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[10105] = {
		explain1 = "奖励在舰队扩大中",
		name = "海军服役成就奖章 ",
		explain2 = "有所建树的指挥官",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "奖励致力于舰队扩大建设并有所建树的指挥官 ",
		next = 0,
		icon = "2015",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "l_206",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 10105,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100611] = {
		explain1 = "",
		name = "白鹰阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "白鹰阵营120级的舰船达到12艘",
		rank = 1,
		id = 100611,
		target_num = 12,
		explain2 = "",
		target_id = {
			{
				1,
				120
			}
		}
	}
	pg.base.medal_template[100612] = {
		explain1 = "",
		name = "白鹰阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "白鹰阵营120级的舰船达到24艘",
		rank = 2,
		id = 100612,
		target_num = 24,
		explain2 = "",
		target_id = {
			{
				1,
				120
			}
		}
	}
	pg.base.medal_template[100613] = {
		explain1 = "",
		name = "白鹰阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100613,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100614] = {
		explain1 = "",
		name = "白鹰阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100614,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100615] = {
		explain1 = "",
		name = "白鹰阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100615,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100621] = {
		explain1 = "",
		name = "白鹰阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "白鹰阵营总好感达成2000",
		rank = 1,
		id = 100621,
		target_num = 200000,
		target_id = "1"
	}
	pg.base.medal_template[100622] = {
		explain1 = "",
		name = "白鹰阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "白鹰阵营总好感达成4000",
		rank = 2,
		id = 100622,
		target_num = 400000,
		target_id = "1"
	}
	pg.base.medal_template[100623] = {
		explain1 = "",
		name = "白鹰阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100623,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100624] = {
		explain1 = "",
		name = "白鹰阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100624,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100625] = {
		explain1 = "",
		name = "白鹰阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100625,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100631] = {
		explain1 = "",
		name = "白鹰阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "白鹰舰船出征获得经验200万点",
		rank = 1,
		id = 100631,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				1,
				0
			}
		}
	}
	pg.base.medal_template[100632] = {
		explain1 = "",
		name = "白鹰阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "白鹰舰船出征获得经验200万点",
		rank = 2,
		id = 100632,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				1,
				0
			}
		}
	}
	pg.base.medal_template[100633] = {
		explain1 = "",
		name = "白鹰阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100633,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100634] = {
		explain1 = "",
		name = "白鹰阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100634,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100635] = {
		explain1 = "",
		name = "白鹰阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100635,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100711] = {
		explain1 = "",
		name = "皇家阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "皇家阵营120级的舰船达到12艘",
		rank = 1,
		id = 100711,
		target_num = 12,
		explain2 = "",
		target_id = {
			{
				2,
				120
			}
		}
	}
	pg.base.medal_template[100712] = {
		explain1 = "",
		name = "皇家阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "皇家阵营120级的舰船达到24艘",
		rank = 2,
		id = 100712,
		target_num = 24,
		explain2 = "",
		target_id = {
			{
				2,
				120
			}
		}
	}
	pg.base.medal_template[100713] = {
		explain1 = "",
		name = "皇家阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100713,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100714] = {
		explain1 = "",
		name = "皇家阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100714,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100715] = {
		explain1 = "",
		name = "皇家阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100715,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100721] = {
		explain1 = "",
		name = "皇家阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "皇家阵营总好感达成2000",
		rank = 1,
		id = 100721,
		target_num = 200000,
		target_id = "2"
	}
	pg.base.medal_template[100722] = {
		explain1 = "",
		name = "皇家阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "皇家阵营总好感达成4000",
		rank = 2,
		id = 100722,
		target_num = 400000,
		target_id = "2"
	}
	pg.base.medal_template[100723] = {
		explain1 = "",
		name = "皇家阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100723,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100724] = {
		explain1 = "",
		name = "皇家阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100724,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100725] = {
		explain1 = "",
		name = "皇家阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100725,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100731] = {
		explain1 = "",
		name = "皇家阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "皇家舰船出征获得经验200万点",
		rank = 1,
		id = 100731,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				2,
				0
			}
		}
	}
	pg.base.medal_template[100732] = {
		explain1 = "",
		name = "皇家阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "皇家舰船出征获得经验200万点",
		rank = 2,
		id = 100732,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				2,
				0
			}
		}
	}
	pg.base.medal_template[100733] = {
		explain1 = "",
		name = "皇家阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100733,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100734] = {
		explain1 = "",
		name = "皇家阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100734,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100735] = {
		explain1 = "",
		name = "皇家阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100735,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100811] = {
		explain1 = "",
		name = "重樱阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "重樱阵营120级的舰船达到12艘",
		rank = 1,
		id = 100811,
		target_num = 12,
		explain2 = "",
		target_id = {
			{
				3,
				120
			}
		}
	}
	pg.base.medal_template[100812] = {
		explain1 = "",
		name = "重樱阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "重樱阵营120级的舰船达到24艘",
		rank = 2,
		id = 100812,
		target_num = 24,
		explain2 = "",
		target_id = {
			{
				3,
				120
			}
		}
	}
	pg.base.medal_template[100813] = {
		explain1 = "",
		name = "重樱阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100813,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100814] = {
		explain1 = "",
		name = "重樱阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100814,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100815] = {
		explain1 = "",
		name = "重樱阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100815,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100821] = {
		explain1 = "",
		name = "重樱阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "重樱阵营总好感达成2000",
		rank = 1,
		id = 100821,
		target_num = 200000,
		target_id = "3"
	}
	pg.base.medal_template[100822] = {
		explain1 = "",
		name = "重樱阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "重樱阵营总好感达成4000",
		rank = 2,
		id = 100822,
		target_num = 400000,
		target_id = "3"
	}
	pg.base.medal_template[100823] = {
		explain1 = "",
		name = "重樱阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100823,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100824] = {
		explain1 = "",
		name = "重樱阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100824,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100825] = {
		explain1 = "",
		name = "重樱阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100825,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100831] = {
		explain1 = "",
		name = "重樱阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "重樱舰船出征获得经验200万点",
		rank = 1,
		id = 100831,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				3,
				0
			}
		}
	}
	pg.base.medal_template[100832] = {
		explain1 = "",
		name = "重樱阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "重樱舰船出征获得经验200万点",
		rank = 2,
		id = 100832,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				3,
				0
			}
		}
	}
	pg.base.medal_template[100833] = {
		explain1 = "",
		name = "重樱阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100833,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100834] = {
		explain1 = "",
		name = "重樱阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100834,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100835] = {
		explain1 = "",
		name = "重樱阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100835,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100911] = {
		explain1 = "",
		name = "铁血阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "铁血阵营120级的舰船达到12艘",
		rank = 1,
		id = 100911,
		target_num = 12,
		explain2 = "",
		target_id = {
			{
				4,
				120
			}
		}
	}
	pg.base.medal_template[100912] = {
		explain1 = "",
		name = "铁血阵营子勋章1",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1042,
		start = 1,
		hide = 1,
		label = "",
		condition = "铁血阵营120级的舰船达到24艘",
		rank = 2,
		id = 100912,
		target_num = 24,
		explain2 = "",
		target_id = {
			{
				4,
				120
			}
		}
	}
	pg.base.medal_template[100913] = {
		explain1 = "",
		name = "铁血阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100913,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100914] = {
		explain1 = "",
		name = "铁血阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100914,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100915] = {
		explain1 = "",
		name = "铁血阵营子勋章1",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100915,
		target_num = 1,
		target_id = "0"
	}
end)()
;(function()
	pg.base.medal_template[100921] = {
		explain1 = "",
		name = "铁血阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "铁血阵营总好感达成2000",
		rank = 1,
		id = 100921,
		target_num = 200000,
		target_id = "4"
	}
	pg.base.medal_template[100922] = {
		explain1 = "",
		name = "铁血阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1043,
		start = 1,
		hide = 1,
		label = "",
		condition = "铁血阵营总好感达成4000",
		rank = 2,
		id = 100922,
		target_num = 400000,
		target_id = "4"
	}
	pg.base.medal_template[100923] = {
		explain1 = "",
		name = "铁血阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100923,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100924] = {
		explain1 = "",
		name = "铁血阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100924,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100925] = {
		explain1 = "",
		name = "铁血阵营子勋章2",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100925,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100931] = {
		explain1 = "",
		name = "铁血阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "铁血舰船出征获得经验200万点",
		rank = 1,
		id = 100931,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				4,
				0
			}
		}
	}
	pg.base.medal_template[100932] = {
		explain1 = "",
		name = "铁血阵营子勋章3",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 1041,
		start = 1,
		hide = 1,
		label = "",
		condition = "铁血舰船出征获得经验200万点",
		rank = 2,
		id = 100932,
		target_num = 2000000,
		explain2 = "",
		target_id = {
			{
				4,
				0
			}
		}
	}
	pg.base.medal_template[100933] = {
		explain1 = "",
		name = "铁血阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 3,
		id = 100933,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100934] = {
		explain1 = "",
		name = "铁血阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 4,
		id = 100934,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[100935] = {
		explain1 = "",
		name = "铁血阵营子勋章3",
		explain2 = "",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "",
		next = 0,
		icon = "",
		page = 1,
		target_type = 9999,
		start = 1,
		hide = 3,
		label = "",
		condition = "尚未开放，敬请期待",
		rank = 5,
		id = 100935,
		target_num = 1,
		target_id = "0"
	}
	pg.base.medal_template[200001] = {
		explain1 = "「星光下的余晖」",
		name = "抵抗之星纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 1,
		count_inherit = 0,
		desc = "铁血抵抗军颁发的纪念勋章，纪念那场对抗星之兽的伟大胜利。我们今日不会毁灭，这个世界亦然，致明日。",
		next = 0,
		icon = "9001",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9001",
		condition = "收集8个「星光下的余晖」纪念贴纸。",
		rank = 1,
		id = 200001,
		target_num = 8,
		target_id = "5711"
	}
	pg.base.medal_template[200011] = {
		explain1 = "「华裳巧展喜事长」",
		name = "华裳巧展纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 1,
		count_inherit = 0,
		desc = "虽有波折，终遂人愿。光华流转，一夜霓裳。",
		next = 0,
		icon = "9011",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9011",
		condition = "收集8个「华裳巧展喜事长」纪念贴纸。",
		rank = 1,
		id = 200011,
		target_num = 8,
		target_id = "5769"
	}
	pg.base.medal_template[200021] = {
		explain1 = "「樊笼内的神光」 ",
		name = "圣翼神光纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 1,
		count_inherit = 0,
		desc = "撒丁尼亚联盟颁发的纪念勋章，纪念这场对抗审判机战车的伟大胜利。突破樊笼后，等在前方的是希望与更伟大的辉煌。",
		next = 0,
		icon = "9021",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9021",
		condition = "收集8个「樊笼内的神光」纪念贴纸。",
		rank = 1,
		id = 200021,
		target_num = 8,
		target_id = "5812"
	}
	pg.base.medal_template[200031] = {
		explain1 = "「深谷来信」 ",
		name = "深谷病院调查纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 1,
		count_inherit = 0,
		desc = "恭喜，你成功调查出了深谷病院的秘密，选择了理想中的结局，此行，你还满意么？",
		next = 0,
		icon = "9031",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9031",
		condition = "收集8个「深谷来信」纪念贴纸。",
		rank = 1,
		id = 200031,
		target_num = 8,
		target_id = "5875"
	}
	pg.base.medal_template[200041] = {
		explain1 = "「扬起郁金之旗」",
		name = "郁金启航纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 1,
		count_inherit = 0,
		desc = "郁金王国颁发的纪念勋章，纪念这场保卫王国的战役。新的时代已经来临，一个联合团结互助的时代。",
		next = 0,
		icon = "9041",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9041",
		condition = "收集8个「扬起郁金之旗」 纪念贴纸。",
		rank = 1,
		id = 200041,
		target_num = 8,
		target_id = "5913"
	}
	pg.base.medal_template[200051] = {
		explain1 = "「高塔上的蔷薇」",
		name = "高塔蔷薇纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "高塔走向了毁灭，蔷薇走向了新生。此行虽有波折、亦存险阻，所幸终得圆满。可喜可贺，可贺可喜。",
		next = 0,
		icon = "9051",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9051",
		condition = "收集8个「高塔上的蔷薇 纪念贴纸。",
		rank = 1,
		id = 200051,
		target_num = 8,
		target_id = "5970"
	}
	pg.base.medal_template[200061] = {
		explain1 = "「夏日重建」",
		name = "别墅假日纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "沙滩与烈日、冷饮与空调。尽情享受这场独属于度假别墅岛的独特体验，度过一个完美假期吧~",
		next = 0,
		icon = "9061",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9061",
		condition = "收集7个「夏日重建」纪念贴纸。",
		rank = 1,
		id = 200061,
		target_num = 7,
		target_id = "5971"
	}
	pg.base.medal_template[200071] = {
		explain1 = "「迷彩都市的寻踪者」",
		name = "都市寻踪纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "你是最敏锐的解谜人，每一处谜题都因你而解开。恭喜你，迷彩都市的寻踪者！",
		next = 0,
		icon = "9071",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9071",
		condition = "收集8个「迷彩都市的寻踪者」纪念贴纸。",
		rank = 1,
		id = 200071,
		target_num = 8,
		target_id = "50021"
	}
	pg.base.medal_template[200081] = {
		explain1 = "「奇渊下的秘密」",
		name = "奇渊之下纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "你发现了奇渊下的秘密，阻止了灾厄之子的阴谋，我们尊敬的指挥官，希望你在这段冒险中玩得尽兴。from：《大型港区虚拟现实体验计划2.0》制作委员会",
		next = 0,
		icon = "9081",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9081",
		condition = "收集8个「奇渊下的秘密」纪念贴纸。",
		rank = 1,
		id = 200081,
		target_num = 8,
		target_id = "50087"
	}
	pg.base.medal_template[200091] = {
		explain1 = "「起舞于天原之上」",
		name = "天域天原纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "「天域，乃外域之屏，天原，因天域而定。纵使地崩山摧，浩劫将至。天域天原，亦需永存」欢迎来到天域天原",
		next = 0,
		icon = "9091",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9091",
		condition = "收集8个「起舞于天原之上」纪念贴纸。",
		rank = 1,
		id = 200091,
		target_num = 8,
		target_id = "50136"
	}
	pg.base.medal_template[200101] = {
		explain1 = "「飓风与自由群岛」",
		name = "自由群岛纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "这里是自由群岛，属于所有向往自由之人的群岛，属于自由城邦的群岛。如果你心向自由的话，自由城邦欢迎你。",
		next = 0,
		icon = "9101",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9101",
		condition = "收集8个「飓风与自由群岛」纪念贴纸。",
		rank = 1,
		id = 200101,
		target_num = 8,
		target_id = "50209"
	}
	pg.base.medal_template[200111] = {
		explain1 = "「响彻于天穹之音」",
		name = "天穹之星纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "仿照安克雷奇国立天文台样式所制作的纪念勋章。你揭开了帷幕，你跨越了天穹，过去未来于此刻相连，新的时代到来了",
		next = 0,
		icon = "9111",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9111",
		condition = "收集8个「响彻于天穹之音」纪念贴纸。",
		rank = 1,
		id = 200111,
		target_num = 8,
		target_id = "50295"
	}
	pg.base.medal_template[200121] = {
		explain1 = "「光影风尚，拍摄进行时！」",
		name = "光影风尚纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "时尚雷达全开，进入拍摄状态！按下快门，捕捉时尚的每一面吧！你就是今日的时尚记录官！",
		next = 0,
		icon = "9121",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9121",
		condition = "收集8个「光影风尚——拍摄进行时！」纪念贴纸。",
		rank = 1,
		id = 200121,
		target_num = 8,
		target_id = "50359"
	}
	pg.base.medal_template[200131] = {
		explain1 = "「春宴怀玉香满庭」",
		name = "春宴怀玉纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "温润美玉静陈，满庭暗香浮动，一切因缘邂逅，皆待君来。步入此间的你，是否就是那位识玉的知音？",
		next = 0,
		icon = "9131",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9131",
		condition = "收集8个「春宴怀玉香满庭」纪念贴纸。",
		rank = 1,
		id = 200131,
		target_num = 8,
		target_id = "50405"
	}
	pg.base.medal_template[200141] = {
		explain1 = "「春满客栈online」",
		name = "春满客栈纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "金樽满，春风醉，招财进宝如意日。今日的客栈就交由您来打点了，尽情享受经营的热闹与乐趣吧~",
		next = 0,
		icon = "9141",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9141",
		condition = "收集8个「春满客栈online」纪念贴纸。",
		rank = 1,
		id = 200141,
		target_num = 8,
		target_id = "50441"
	}
	pg.base.medal_template[200151] = {
		explain1 = "「漫游者招募计划」",
		name = "都市漫游纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "霓虹灯牌在头顶闪烁，车流在脚下汇成光河。穿行于不眠的街巷，感受都市的每一次呼吸与脉动。欢迎来到荆棘市~",
		next = 0,
		icon = "9151",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9151",
		condition = "收集8个「漫游者招募计划」纪念贴纸。",
		rank = 1,
		id = 200151,
		target_num = 8,
		target_id = "50482"
	}
	pg.base.medal_template[200161] = {
		explain1 = "「圣印前的同盟」",
		name = "圣印同盟纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "神圣联合帝国颁发的纪念勋章，纪念这场对抗黑境的伟大胜利。神光于此见证，圣印前的同盟永存。",
		next = 0,
		icon = "9161",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9161",
		condition = "收集8个「圣印前的同盟」纪念贴纸。",
		rank = 1,
		id = 200161,
		target_num = 8,
		target_id = "50607"
	}
	pg.base.medal_template[200171] = {
		explain1 = "「奢华！享梦！浮金湾！」",
		name = "奢华盛宴纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "漫游浮金湾，纵情欢乐，这才是假日的正确打开方式~愿你的每一次出游，都满载奢华，收获惊喜！",
		next = 0,
		icon = "9171",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9171",
		condition = "收集7个「奢华！享梦！浮金湾！」纪念贴纸。",
		rank = 1,
		id = 200171,
		target_num = 7,
		target_id = "50616"
	}
	pg.base.medal_template[200181] = {
		explain1 = "「美梦巡演奇妙夜」",
		name = "美梦巡演纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "大幕拉开，灯光亮起，折纸为翼，魔术作笔。欢迎来到这场永不落幕的奇妙巡游~你的每一个梦，都如马戏团的夜晚般绚烂！",
		next = 0,
		icon = "9181",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9181",
		condition = "收集8个「美梦巡演奇妙夜」纪念贴纸。",
		rank = 1,
		id = 200181,
		target_num = 8,
		target_id = "50659"
	}
	pg.base.medal_template[200191] = {
		explain1 = "「怪谈纪实：逃离白夜山庄！」",
		name = "怪谈纪实纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "灯光未熄，铁门紧锁，废弃病院的深处藏着无人知晓的秘密。在怪谈与线索交错之间，找出通往出口的方向吧！欢迎来到白夜山庄~",
		next = 0,
		icon = "9191",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9191",
		condition = "收集8个「怪谈纪实：逃离白夜山庄！」纪念贴纸。",
		rank = 1,
		id = 200191,
		target_num = 8,
		target_id = "51078"
	}
	pg.base.medal_template[200201] = {
		explain1 = "「沉溺于星光之城」",
		name = "星光城纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "迷途的旅人，你见证了星光城的美梦，然后踏碎了它。",
		next = 0,
		icon = "9201",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9201",
		condition = "收集8个「沉溺于星光之城」纪念贴纸。",
		rank = 1,
		id = 200201,
		target_num = 8,
		target_id = "51110"
	}
	pg.base.medal_template[200211] = {
		explain1 = "「滨海极速」",
		name = "滨海极速纪念勋章",
		explain2 = "活动纪念",
		type = 0,
		is_out_of_print = 0,
		count_inherit = 0,
		desc = "引擎轰鸣，轮胎飞转，海风与热浪共同铺开这条闪耀的竞速之路。向着冠军全速前进吧！",
		next = 0,
		icon = "9211",
		page = 2,
		target_type = 131,
		start = 0,
		hide = 0,
		label = "l_9211",
		condition = "收集7个「滨海极速」纪念贴纸。",
		rank = 1,
		id = 200211,
		target_num = 7,
		target_id = "51113"
	}
end)()
