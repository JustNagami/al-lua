return {
	id = "YOUYINGMICHENG4-1",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "新手行动员",
			dir = 1,
			bgm = "story-visioncity-1",
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "这、这又是什么啊？！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashin = {
				delay = 0,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			actor = 0,
			nameColor = "#A9F548FF",
			portrait = "zhihuiguan",
			say = "小心镜面——不要盯着看！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			say = "几乎在我出声的同时，一只惨白的手猛地从路边商铺的展示屏里伸了出来，直直抓向最近的标枪！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "新手行动员",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "啊——！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			say = "标枪惊叫着后退半步，本能举起法杖挡在身前——粉白色的光芒闪耀起来，那只手像被灼伤般骤然缩了回去。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403143,
			nameColor = "#FF9B93",
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			side = 2,
			actorName = "？？？？",
			hidePaintObj = true,
			say = "呵呵……好疼呢~看来想要拖住你们的脚步，并不容易啊……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "“拖住我们的脚步”？你想掩护谁离开，刚才追逐{namecode:560:Z14}的黑影么？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "近期的少女失踪事件和你们有关？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "……哎呀？这可真意外~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "您竟然能听懂我说话？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			say = "电流的雪花中，那女人终于缓缓“爬”出屏幕。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "{namecode:531:腓特烈·卡尔}，向您问好~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "我早就听说了——这座城市里，存在着能驯服幽影，并驱使她们战斗的人呢。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403143,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "您所说的那些充满爱意的话语，我确实很感兴趣，但为了帮助那位大人……请允许我下次再听吧~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "现在，请你们离开这里。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "没有任务执行到一半，就被敌人三言两语劝走的道理吧？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "……嗨呀~谈崩了呢。没关系，我不会太粗暴的。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			say = "{namecode:531:腓特烈·卡尔}唇角弯起，与此同时，我们身边的屏幕璃纷纷破裂，无数漆黑的影子从碎裂的镜面中扑出——",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "{namecode:23:天津风}，虎！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影行动员",
			dir = 1,
			actor = 900557,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "明白！ ",
			actorPosition = {
				x = -200,
				y = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			say = "狐火蔓延开来，炽热的火光将所有扑来的影子燃烧殆尽。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "她的进攻节奏乱了……很好。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "就让我来终结这场闹剧，为敌人写下休止符吧！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			say = "虎的眸光微亮，尖尖的虎牙在唇边若隐若现，气质骤然变得危险而狂热。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "美妙的音乐……身为人类的指挥官能与我们沟通，原来是你的功劳啊。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "不过呢，同为幽影的我们，理应更能互相理解才对~你呀，不如还是和我们合作吧？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "幽影",
			dir = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "合作？商务洽谈请咨询指挥官……现在请安静！接下来，是演奏时间！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
