...
function var_0_0.Init(arg_2_0)
	local var_2_0 = pg.auction_session[arg_2_0.id]

	setText(arg_2_0.uiUnselectedLockText, i18n("auction_not_enough_assets", StringHelper.ForamtNumberK(var_2_0.threshold)))
	onButton(arg_2_0, arg_2_0.uiButton, function()
		local var_3_0 = pg.auction_session[arg_2_0.id]

		if AuctionGameTools.GetCurrencyCnt() >= var_3_0.threshold == false then
			return
		end
...
function var_0_0.SetSelected(arg_5_0, arg_5_1)
	...
	local var_5_0 = pg.auction_session[arg_5_0.id]

	if AuctionGameTools.GetCurrencyCnt() >= var_5_0.threshold then
...
function var_0_0.RefreshState(arg_6_0)
	local var_6_0 = pg.auction_session[arg_6_0.id]
	local var_6_1 = AuctionGameTools.GetCurrencyCnt() >= var_6_0.threshold

	setActive(arg_6_0.uiUnselectedLockGo, not var_6_1)
	setActive(arg_6_0.uiSelectedLockGo, not var_6_1)
	setActive(arg_6_0.uiSelectedUnLockGo, var_6_1)
	arg_6_0:RefreshTip()
end

function var_0_0.RefreshTip(arg_7_0)
	local var_7_0 = getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_AUCTION_GAME)

	setActive(arg_7_0.uiTipGo, var_7_0:GetLocationTip(arg_7_0.id))
end
