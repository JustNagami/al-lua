...
function var_0_0.init(arg_2_0)
	...
	onButton(arg_2_0, arg_2_0.uiBidBtn, function()
		...
		local var_11_2 = var_11_1:GetAuctionState()

		if pg.TimeMgr.GetInstance():GetServerTime() < var_11_1:GetTimestamp() and var_11_2 ~= AuctionGameConst.AUCTION_PHASE.BID then
			pg.TipsMgr.GetInstance():ShowTips(i18n("auction_game_wait_bid_phase"))

			return
		end

		local var_11_3 = pg.auction_session[var_11_1:GetAuctionID()].bottom_price

		if var_11_0 < var_11_3 then
			pg.TipsMgr.GetInstance():ShowTips(i18n("auction_game_min_bid", var_11_3))
...
function var_0_0.didEnter(arg_14_0)
	...
	if var_14_1 <= 1 then
		...
		local var_14_2 = getProxy(PlayerProxy):getPlayerId()

		arg_14_0.inputNum = var_14_0:GetRoundEventAndBidInfoList()[var_14_1 - 1][var_14_2].bidValue or 0
	end
...
function var_0_0.OnClickBidBtn(arg_20_0)
	...
	local var_20_1 = pg.gameset.auction_bid_time.key_value - (var_20_0:GetTimestamp() - pg.TimeMgr.GetInstance():GetServerTime())

	pg.GameTrackerMgr.GetInstance():Record(GameTrackerBuilder.BuildAuctionBid(var_20_0:GetAuctionID(), var_20_0:GetRound(), var_20_1, arg_20_0.inputNum, 0))

	local var_20_2 = arg_20_0.inputNum

	arg_20_0:emit(AuctionGameMainBidMediator.BID, var_20_2)
end
