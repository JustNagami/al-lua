return {
	id = "YOUYINGMICHENG16",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_300",
			factiontag = "妖異",
			dir = 1,
			bgm = "story-antarctica-serious",
			actor = 9600030,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「人間を手中に収める手伝い」……ふふ、いかにも安土らしいわぁ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashin = {
				delay = 0,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			location = {
				"新紀元シティNO.7・市庁舎",
				3
			}
		},
		{
			actor = 317030,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_300",
			hidePaintObj = true,
			actorName = "ある普通の後方要員",
			side = 2,
			say = "指揮官、通信リクエストが来てる……確認を……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_300",
			hidePaintObj = true,
			say = "ファイルを抱えた見慣れない後方要員が、足早に歩み寄ってきた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 317030,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_300",
			hidePaintObj = true,
			actorName = "ある普通の後方要員",
			side = 2,
			say = "それと……ジャベリンたちの会議、同じ話が10分以上ぐるぐるしている……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_300",
			hidePaintObj = true,
			portrait = "zhihuiguan",
			side = 2,
			say = "――分かった、すぐに対応する",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_300",
			hidePaintObj = true,
			say = "なんだか存在感があまりない後方要員は、軽く頷くと静かに退室していった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			say = "——10分前。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			location = {
				"市庁舎・作戦会議室",
				3
			}
		},
		{
			actor = 202030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……あら、どうしてみなさん、だんまりを決め込んでいるのですか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "どなたも口を開く気がないのなら、私が状況をはっきりとさせてもらいますわ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "私たちが今立ち向かっているのは、妖異だけではありません。「助けたい人たち」からの敵意もです。これを解決しない限り計画は前へと進みません",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "いっそNO.7を安全な軍事・貿易拠点にするのも良いでしょう。その後……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "新人エージェント",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……こ、こんな状況だからこそ、余計に引きこもってはいけません！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "新人エージェント",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "みんなに信頼してもらえないからといって、そのまま何もしなければ……それこそ、安土の勝ちになっちゃうじゃないですか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			actor = 602030,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ジャベリンの気持ちは分かるけど……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401231,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀補佐",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "失礼ですが、ジャベリンの代わりに回答させてください",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			say = "会議卓の末席でずっと記録と資料整理をしていたZ23が顔を上げた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401231,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀補佐",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "戦略的な観点から言えば、皆さんもとうにお気づきでしょうが、安土の狙いは「孤立」を作り出すことです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401231,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀補佐",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "敵の思惑通りにさせてしまえば、こちらがますます不利になりましょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……参謀補佐さんはもしや、会議を仕切りたいのですか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401231,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀補佐",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "とんでもありません。本会議の目的は決断を下すことです。議論の段階で否定的な見解ばかりを並べ、ほかの可能性を断ってしまうべきではありません",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401231,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀補佐",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "それではただ感情任せになっているのと変わりません",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ふふ、今のは一理ありますね。少し見直しましたわ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401231,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀補佐",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "とんでもありません",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "こういう時は褒め言葉を素直に受け取ればいいですよ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "新人エージェント",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そ、そうです！ニーミちゃんの言う通りです！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "新人エージェント",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "今私たちが動いても絶対に成功するとは保証できません。でも失敗を恐れて最初から諦めてしまったら何も変わらないんです！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			actor = 404070,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……それでも、事態をひっくり返すだけの現実的なきっかけが必要です",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			actor = 404070,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "さもなければ、皆さんの発言は結局、立場を述べているにすぎません",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			say = "会議が再び膠着状態へ陥りかけたその時、スクリーンに緊急ニュースが割り込んだ。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 899050,
			side = 2,
			bgName = "star_level_bg_605",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "……受信は良好ね……始めるわ",
			bgm = "battle-eagleunion",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = false,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = false,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_605",
			hidePaintObj = true,
			say = "オラージュがプレハブ演説台に立っていた。その表情は疲れきっていたが瞳には光が宿っていた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 899050,
			side = 2,
			bgName = "star_level_bg_605",
			factiontag = "企業連合会臨時代表",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "こちらは新紀元シティNO.5。わたしはNO.5企業連合会の臨時代表のオラージュです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 899050,
			side = 2,
			bgName = "star_level_bg_605",
			factiontag = "企業連合会臨時代表",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "各シティに通達するわ。連合会の前会長である明石はすでに一切の管理権限を剥奪された",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 899050,
			side = 2,
			bgName = "star_level_bg_605",
			factiontag = "企業連合会臨時代表",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "シティの対妖異戦線はわたしたちが引き継ぎ維持してるところよ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 899050,
			side = 2,
			bgName = "star_level_bg_605",
			factiontag = "企業連合会臨時代表",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そこでだけど、NO.5は正式にNO.7へ相互協力を要請するわ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 899050,
			side = 2,
			bgName = "star_level_bg_605",
			factiontag = "企業連合会臨時代表",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そちらがまだ手を差し伸べてくれるなら、わたしたちは通路の開放、そして補給と情報の共有をもって応えるわ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_605",
			hidePaintObj = true,
			say = "短い間を置いてから、オラージュは何かを思い出したように、もう一言付け加えた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_605",
			factiontag = "企業連合会臨時代表",
			dir = 1,
			actor = 899050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そうそう、明石会長が今どうしているか気になるなら……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 899050,
			side = 2,
			bgName = "star_level_bg_605",
			factiontag = "企業連合会臨時代表",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "彼女は元気よ。ただ「会議だけじゃ状況は好転しない」ということを身をもって教わっているところね",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			say = "沈黙も束の間、誰かが堪えきれずに小さく吹き出した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = false,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = false,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 102030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "NO.5にまだ肝の据わった子が残っているとはね……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "市場とは、まこと目まぐるしく移ろうものでございます……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			actor = 404070,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "NO.5がこちらとの協力に応じてくれるのであれば……長期的な計画にも大いに有益です",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			actor = 602030,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "しかも、これは今までの「協力拒否」の態度が、決して覆せないものではないことを示している",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "新人エージェント",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そうですよ！変わろうとしている人がいる限り、こっちから先に見捨てちゃいけません！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "――よく言った",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			say = "皆の視線を受けながら、最新の知らせを手に会議室に入った。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "――NO.5には正式に協力を申し入れた。ここからは手を取り合って局面を切り開いていく",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "――それと、いい知らせがもう一つ。今のNO.5の行動はほかの都市も動かした。この先の作戦でもっと多くの支援が得られるはず",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "――もし安土が恐怖と猜疑に足を取られ、立ち止まるこちらの姿を見たがっているのだとしたら……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			side = 2,
			portrait = "zhihuiguan",
			say = "――その望みは叶わない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = true,
			say = "今度の会場の沈黙はもはや否定や憂慮ではなく、同じ方に進む決意の表れになった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			actor = 602030,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "支援計画の続行に同意する",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			actor = 404070,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "関連する後方支援計画は、最新の状況に合わせて更新しました",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102030,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "外交と連絡の文書は、わたしに任せて",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "急ぎ必要な軍需物資は、相場の二割引でNO.5にお売りいたしましょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_guild_blue_n",
			factiontag = "参謀団メンバー",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……なぜ皆さま、そのような目で妾をご覧になるのです？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
