pg = pg or {}
pg.dorm3d_recall = rawget(pg, "dorm3d_recall") or setmetatable({
	__name = "dorm3d_recall"
}, confNEO)
pg.dorm3d_recall.all = {
	1,
	2,
	3,
	4,
	5,
	6,
	7,
	8,
	21,
	22,
	23,
	31,
	32,
	41,
	42,
	43,
	51,
	52,
	101,
	102,
	103,
	104,
	105,
	121,
	122,
	123,
	124,
	125,
	141,
	142,
	143,
	144,
	211,
	212,
	213,
	214
}
pg.dorm3d_recall.get_id_list_by_story_id = {
	[10010] = {
		1
	},
	[10020] = {
		2
	},
	[10030] = {
		3
	},
	[10040] = {
		4
	},
	[10050] = {
		5
	},
	[10060] = {
		6
	},
	[10070] = {
		7
	},
	[10080] = {
		8
	},
	[20500] = {
		21
	},
	[20510] = {
		22
	},
	[20520] = {
		23
	},
	[20530] = {
		32
	},
	[29991] = {
		31
	},
	[30510] = {
		51
	},
	[30520] = {
		41
	},
	[30530] = {
		42
	},
	[30540] = {
		43
	},
	[30550] = {
		52
	},
	[110510] = {
		101
	},
	[110520] = {
		102
	},
	[110530] = {
		103
	},
	[110541] = {
		104
	},
	[110551] = {
		105
	},
	[120510] = {
		121
	},
	[120520] = {
		122
	},
	[120530] = {
		123
	},
	[120541] = {
		124
	},
	[120551] = {
		125
	},
	[140510] = {
		141
	},
	[140520] = {
		142
	},
	[140530] = {
		143
	},
	[140541] = {
		144
	},
	[210510] = {
		211
	},
	[210520] = {
		212
	},
	[210530] = {
		213
	},
	[210541] = {
		214
	}
}
pg.base = pg.base or {}
pg.base.dorm3d_recall = {}

;(function()
	pg.base.dorm3d_recall[1] = {
		story_id = 10010,
		name = "ようこそ宿舎へ",
		unlock_text = "親愛度Lv1で開放",
		type = 1,
		id = 1,
		image = "recall_01",
		desc = "初めてシリアスの部屋に訪れた時……",
		unlock = {
			1,
			20220,
			1
		}
	}
	pg.base.dorm3d_recall[2] = {
		story_id = 10020,
		name = "一時の憩い",
		unlock_text = "親愛度Lv4で開放",
		type = 1,
		id = 2,
		image = "recall_02",
		desc = "湯気漂う中、シリアスが訴えかけたのは…",
		unlock = {
			1,
			20220,
			4
		}
	}
	pg.base.dorm3d_recall[3] = {
		story_id = 10030,
		name = "準備の気配り",
		unlock_text = "親愛度Lv7で開放",
		type = 1,
		id = 3,
		image = "recall_03",
		desc = "ドジを踏んでしまったシリアス……",
		unlock = {
			1,
			20220,
			7
		}
	}
	pg.base.dorm3d_recall[4] = {
		story_id = 10040,
		name = "優しいお世話",
		unlock_text = "親愛度Lv10で開放",
		type = 1,
		id = 4,
		image = "recall_04",
		desc = "検査を受けたシリアスが感じたことは……",
		unlock = {
			1,
			20220,
			10
		}
	}
	pg.base.dorm3d_recall[5] = {
		story_id = 10050,
		name = "一緒に踊りを",
		unlock_text = "親愛度Lv13で開放",
		type = 1,
		id = 5,
		image = "recall_05",
		desc = "踊りに誘われたシリアスの反応は……",
		unlock = {
			1,
			20220,
			13
		}
	}
	pg.base.dorm3d_recall[6] = {
		story_id = 10060,
		name = "掃除の秘訣",
		unlock_text = "「清掃ツールボックス」をプレゼントして開放",
		type = 1,
		id = 6,
		image = "gift_01",
		desc = "シリアス、ロイヤルメイドの掃除の極意をお見せします",
		unlock = {
			3,
			2022001
		}
	}
	pg.base.dorm3d_recall[7] = {
		story_id = 10070,
		name = "意外のトラブル",
		unlock_text = "「ロイヤルティーセット」をプレゼントすることで解放",
		type = 1,
		id = 7,
		image = "gift_02",
		desc = "最も優秀なロイヤルメイドでも失敗する時はある",
		unlock = {
			3,
			2022002
		}
	}
	pg.base.dorm3d_recall[8] = {
		story_id = 10080,
		name = "目覚めの朝",
		unlock_text = "『完璧なメイド心得』プレゼントで解放",
		type = 1,
		id = 8,
		image = "gift_03",
		desc = "朝の光の中で聞こえてくる彼女の声。……良い一日はここから始まる。",
		unlock = {
			3,
			2022003
		}
	}
	pg.base.dorm3d_recall[21] = {
		story_id = 20500,
		name = "目隠しサプライズ",
		unlock_text = "親愛度Lv4で開放",
		type = 1,
		id = 21,
		image = "recall_21",
		desc = "不意に、目が柔らかい感触で覆われた。「だーれだ？」",
		unlock = {
			1,
			30221,
			4
		}
	}
	pg.base.dorm3d_recall[22] = {
		story_id = 20510,
		name = "手のひらの暖かさ",
		unlock_text = "親愛度Lv8で開放",
		type = 1,
		id = 22,
		image = "recall_22",
		desc = "お出かけ前のスキンシップこそ、素敵な思い出の序章――",
		unlock = {
			1,
			30221,
			8
		}
	}
	pg.base.dorm3d_recall[23] = {
		story_id = 20520,
		name = "ドキドキ触れ合い",
		unlock_text = "親愛度Lv12で開放",
		type = 1,
		id = 23,
		image = "recall_23",
		desc = "目と目が合った時にしか感じられない、彼女のドキドキ",
		unlock = {
			1,
			30221,
			12
		}
	}
	pg.base.dorm3d_recall[31] = {
		story_id = 29991,
		name = "素直な手伝い",
		unlock_text = "「素直ローファー」をプレゼントして開放",
		type = 1,
		id = 31,
		image = "recall_31",
		desc = "二人っきりの時間。繊細な肌触りがもてなす感動の刹那は…",
		unlock = {
			3,
			3022101
		}
	}
	pg.base.dorm3d_recall[32] = {
		story_id = 20530,
		name = "フットマッサージャー",
		unlock_text = "「フットマッサージャー」をプレゼントして開放",
		type = 1,
		id = 32,
		image = "recall_32",
		desc = "疲れを癒やし心を癒やす、日々の体にエネルギー充填！",
		unlock = {
			3,
			3022102
		}
	}
	pg.base.dorm3d_recall[41] = {
		story_id = 30520,
		name = "一緒に遊びたい",
		unlock_text = "親愛度Lv4で開放",
		type = 1,
		id = 41,
		image = "recall_41",
		desc = "穏やかな午後。部屋でゆっくりしているアンカレッジが待っている……",
		unlock = {
			1,
			19903,
			4
		}
	}
	pg.base.dorm3d_recall[42] = {
		story_id = 30530,
		name = "かくれんぼ",
		unlock_text = "親愛度Lv8で開放",
		type = 1,
		id = 42,
		image = "recall_42",
		desc = "アンカレッジはどこにいるかな？期待でソワソワしている姿からもう丸わかりだ",
		unlock = {
			1,
			19903,
			8
		}
	}
	pg.base.dorm3d_recall[43] = {
		story_id = 30540,
		name = "目覚めの朝",
		unlock_text = "親愛度Lv12で開放",
		type = 1,
		id = 43,
		image = "recall_43",
		desc = "囁きの中で目覚める素晴らしい朝。アンカレッジと一緒に楽しむ新しい一日",
		unlock = {
			1,
			19903,
			12
		}
	}
	pg.base.dorm3d_recall[51] = {
		story_id = 30510,
		name = "幸せを描き留めて",
		unlock_text = "「キャンバス」をプレゼントして開放",
		type = 1,
		id = 51,
		image = "recall_51",
		desc = "部屋で絵を描いているアンカレッジ。絵の具と筆を使って、心に溢れる幸せを形に……",
		unlock = {
			3,
			1990301
		}
	}
	pg.base.dorm3d_recall[52] = {
		story_id = 30550,
		name = "パッチワーク",
		unlock_text = "精緻な額縁をプレゼントして解放",
		type = 1,
		id = 52,
		image = "recall_52",
		desc = "真剣にパッチワークを作っているアンカレッジ。その内容は…？",
		unlock = {
			3,
			1990302
		}
	}
	pg.base.dorm3d_recall[101] = {
		story_id = 110510,
		name = "魅力コーデ",
		unlock_text = "親愛度Lv4で開放",
		type = 1,
		id = 101,
		image = "recall_111",
		desc = "彼女からすれば、あなたがどんなものを着飾っていても魅力的に映る",
		unlock = {
			1,
			10517,
			4
		}
	}
	pg.base.dorm3d_recall[102] = {
		story_id = 110520,
		name = "甘いイタズラ",
		unlock_text = "親愛度Lv8で開放",
		type = 1,
		id = 102,
		image = "recall_112",
		desc = "はぐれる心配はない。なぜなら彼女はいつもあなたのそばにいるから",
		unlock = {
			1,
			10517,
			8
		}
	}
	pg.base.dorm3d_recall[103] = {
		story_id = 110530,
		name = "涼しい舌触り",
		unlock_text = "親愛度Lv12で開放",
		type = 1,
		id = 103,
		image = "recall_113",
		desc = "ヒンヤリとした舌触りは、彼女が口づけた甘いフレーバー",
		unlock = {
			1,
			10517,
			12
		}
	}
	pg.base.dorm3d_recall[104] = {
		story_id = 110541,
		name = "味わうクリスタル",
		unlock_text = "「水晶キャンディ」をプレゼントして開放",
		type = 1,
		id = 104,
		image = "recall_114",
		desc = "水晶のようなキャンディはどんな味だろう？彼女と一緒に味わってみよう",
		unlock = {
			3,
			1051701
		}
	}
	pg.base.dorm3d_recall[105] = {
		story_id = 110551,
		name = "ゲーム対決",
		unlock_text = "ゲーミングパッドを贈ることで開放",
		type = 1,
		id = 105,
		image = "recall_115",
		desc = "相手をこれ以上プレイさせないことも勝ち方の一つよ♪",
		unlock = {
			3,
			1051702
		}
	}
	pg.base.dorm3d_recall[121] = {
		story_id = 120510,
		name = "繋げられた愛",
		unlock_text = "親愛度Lv4で開放",
		type = 1,
		id = 121,
		image = "recall_121",
		desc = "安全安心。あなたを隣に繋ぎ止めれば",
		unlock = {
			1,
			30707,
			4
		}
	}
	pg.base.dorm3d_recall[122] = {
		story_id = 120520,
		name = "午後の寝言",
		unlock_text = "親愛度Lv8で開放",
		type = 1,
		id = 122,
		image = "recall_122",
		desc = "温かい昼下がりの夢は、どんな感じだろう……一緒に楽しみましょう♪",
		unlock = {
			1,
			30707,
			8
		}
	}
	pg.base.dorm3d_recall[123] = {
		story_id = 120530,
		name = "唇のぬくもり",
		unlock_text = "親愛度Lv12で開放",
		type = 1,
		id = 123,
		image = "recall_123",
		desc = "目と鼻の先か、唇のぬくもりを感じられる距離…どっちが先に手に入れられる？",
		unlock = {
			1,
			30707,
			12
		}
	}
	pg.base.dorm3d_recall[124] = {
		story_id = 120541,
		name = "心を鎖で繋ぎ止めて",
		unlock_text = "「繋げられた愛」をプレゼントして開放",
		type = 1,
		id = 124,
		image = "recall_124",
		desc = "鎖で互いの距離を縮めれば、好きな気持ちも溢れ出す",
		unlock = {
			3,
			3070701
		}
	}
	pg.base.dorm3d_recall[125] = {
		story_id = 120551,
		name = "シルキーラブ",
		unlock_text = "緋色の誘いプレゼントで解放",
		type = 1,
		id = 125,
		image = "recall_125",
		desc = "肌に触れるシルクの質感――着替えも甘い罠に？",
		unlock = {
			3,
			3070702
		}
	}
	pg.base.dorm3d_recall[141] = {
		story_id = 140510,
		name = "うたた寝の時間",
		unlock_text = "エーギル親密度Lv4で解放",
		type = 1,
		id = 141,
		image = "recall_141",
		desc = "のんびりとした午後は一緒にうたた寝するのにピッタリ",
		unlock = {
			1,
			49905,
			4
		}
	}
	pg.base.dorm3d_recall[142] = {
		story_id = 140520,
		name = "密着の折檻",
		unlock_text = "エーギル親密度Lv8で解放",
		type = 1,
		id = 142,
		image = "recall_142",
		desc = "手を出せば触れる渇望と、目と鼻の先にある甘いオシオキ",
		unlock = {
			1,
			49905,
			8
		}
	}
	pg.base.dorm3d_recall[143] = {
		story_id = 140530,
		name = "指先と毛先",
		unlock_text = "エーギル親密度Lv12で解放",
		type = 1,
		id = 143,
		image = "recall_143",
		desc = "繊細にして優しい感触は、指先で髪につける愛の化粧",
		unlock = {
			1,
			49905,
			12
		}
	}
	pg.base.dorm3d_recall[144] = {
		story_id = 140541,
		name = "溶け込む優しさ",
		unlock_text = "チューリップグラス購入で解放",
		type = 1,
		id = 144,
		image = "recall_144",
		desc = "秘蔵品の最高の味わいは、二人でこそ深まる",
		unlock = {
			3,
			4990501
		}
	}
	pg.base.dorm3d_recall[211] = {
		story_id = 210510,
		name = "効率的な取り方",
		unlock_text = "ナヒーモフ親密度Lv4で解放",
		type = 1,
		id = 211,
		image = "recall_211",
		desc = "激甘な秘密は、効率的な取り方の中にあり――",
		unlock = {
			1,
			79902,
			4
		}
	}
	pg.base.dorm3d_recall[212] = {
		story_id = 210520,
		name = "早朝の温もり",
		unlock_text = "ナヒーモフ親密度Lv8で解放",
		type = 1,
		id = 212,
		image = "recall_212",
		desc = "あなたの胸の中の温もりが、一番安心させてくれる",
		unlock = {
			1,
			79902,
			8
		}
	}
	pg.base.dorm3d_recall[213] = {
		story_id = 210530,
		name = "メンテ上手",
		unlock_text = "ナヒーモフ親密度Lv12で解放",
		type = 1,
		id = 213,
		image = "recall_213",
		desc = "指揮官も、なんか……修理が必要みたいにゃ～",
		unlock = {
			1,
			79902,
			12
		}
	}
	pg.base.dorm3d_recall[214] = {
		story_id = 210541,
		name = "隠れ本能",
		unlock_text = "本能トレーナー購入で開放",
		type = 1,
		id = 214,
		image = "recall_214",
		desc = "猫じゃらしの前では、データよりも本能が正直だ",
		unlock = {
			3,
			7990201
		}
	}
end)()
