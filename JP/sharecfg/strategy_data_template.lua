pg = pg or {}
pg.strategy_data_template = rawget(pg, "strategy_data_template") or setmetatable({
	__name = "strategy_data_template"
}, confNEO)
pg.strategy_data_template.__namecode__ = true
pg.strategy_data_template.all = {
	1,
	2,
	3,
	4,
	9,
	10,
	11,
	13,
	14,
	16,
	17,
	18,
	47,
	90,
	91,
	92,
	93,
	94,
	95,
	96,
	97,
	1000,
	1001,
	1002,
	1003,
	1004,
	1005,
	1006,
	1007,
	1008,
	1009,
	1010,
	1011,
	1012,
	1013,
	1014,
	1015,
	1016,
	8650,
	8732,
	8744,
	8745,
	8746,
	8750,
	8801,
	8802,
	8803,
	8806,
	8809,
	8812,
	8815,
	8832,
	8841,
	8842,
	8843,
	8846,
	8849,
	8852,
	8855,
	8858,
	8863,
	8864,
	8865,
	8867,
	8869,
	8874,
	8877,
	8880,
	8882,
	8884,
	8909,
	8910,
	8911,
	8912,
	8913,
	8914,
	8915,
	8916,
	8917,
	8918,
	8919,
	8920,
	8941,
	8942,
	8943,
	8944,
	8945,
	8946,
	8953,
	8955,
	8957,
	8959,
	8974,
	8975,
	8976,
	9211,
	9212,
	9213,
	9216,
	9219,
	9222,
	9225,
	9231,
	9232,
	9233,
	9236,
	9239,
	9242,
	9245,
	9251,
	9252,
	9253,
	9256,
	9259,
	9262,
	9265,
	9271,
	9272,
	9273,
	9276,
	9279,
	9282,
	9285,
	9391,
	9392,
	9393,
	9394,
	9395,
	9401,
	9403,
	9406,
	9409,
	9412,
	9415,
	9421,
	9423,
	9426,
	9429,
	9432,
	9435,
	9441,
	9443,
	9446,
	9449,
	9452,
	9455,
	9461,
	9463,
	9466,
	9469,
	9472,
	9475,
	9481,
	9483,
	9486,
	9489,
	9492,
	9495,
	9500,
	9502,
	9505,
	9508,
	10001,
	10002,
	10011,
	10012,
	10013,
	10014,
	10015,
	10021,
	10031,
	10032,
	10033,
	200000,
	200003,
	200006,
	200009,
	200011,
	200012,
	200013,
	200015,
	200016,
	200019,
	200023,
	200024,
	200025,
	200052,
	200054,
	200057,
	200060,
	200063,
	200067,
	200070,
	200074,
	200078,
	200079,
	200080,
	200081,
	200082,
	200083,
	200084,
	200085,
	200086,
	200087,
	200088,
	200239,
	200240,
	200241,
	200242,
	200327,
	200330,
	200333,
	200336,
	200338,
	200392,
	200394,
	200396,
	200401,
	200406,
	200411,
	200416,
	200432,
	200433,
	200434,
	200511,
	200535,
	200536,
	200538,
	200541,
	200544,
	200545,
	200546,
	200583,
	200585,
	200589,
	200592,
	200595,
	200618,
	200620,
	200622,
	200624,
	200627,
	200630,
	200633,
	200645,
	200646,
	200647,
	200648,
	200649,
	200655,
	200658,
	200661,
	200664,
	200667,
	200670,
	200673,
	200676,
	200679,
	200732,
	200735,
	200770,
	200774,
	200777,
	200780,
	200783,
	200902,
	200903,
	200905,
	200907,
	200908,
	200925,
	200926,
	200928,
	200930,
	200931,
	200932,
	200979,
	200980,
	200983,
	200984,
	200987,
	200990,
	200999,
	201021,
	201022,
	201024,
	201027,
	201029,
	201090,
	201094,
	201097,
	201100,
	201149,
	201152,
	201155,
	201157,
	201158,
	201193,
	201194,
	201197,
	201198,
	201199,
	201250,
	201270,
	201276,
	201281,
	201287,
	201301,
	201302,
	201303,
	201304,
	201305,
	201306,
	201307,
	201308,
	201309,
	201310,
	201311,
	201312,
	201313,
	201314,
	201315,
	201316,
	201317,
	201318,
	201319,
	201320,
	201321,
	201327,
	201361,
	201365,
	201377,
	201378,
	201382,
	201386,
	201410,
	201411,
	201461,
	201462,
	201463,
	201464,
	201515,
	201516,
	201517,
	201519,
	201525,
	201530,
	201531,
	201532,
	201547,
	201550,
	201551,
	201552,
	201650,
	201651,
	201654,
	201655,
	201701,
	201705,
	201706,
	201707,
	201741,
	201742,
	201743,
	201750,
	201754,
	201755,
	201759,
	201763,
	201764,
	201765,
	201820,
	201821,
	201826,
	201831,
	201832,
	201901,
	201905,
	201909,
	201910,
	201911,
	205001,
	205002,
	205003,
	205004,
	205005,
	205006,
	205007,
	300001,
	300002,
	300003,
	300004,
	300005,
	300006,
	300007,
	300008,
	300009,
	300010,
	300011,
	300012,
	300013,
	300014,
	300015,
	300016,
	300017,
	300018,
	300019,
	300020
}
pg.base = pg.base or {}
pg.base.strategy_data_template = {}

;(function()
	pg.base.strategy_data_template[1] = {
		buff_id = 100,
		name = "単縦陣",
		iconSize = "",
		type = 1,
		id = 1,
		icon = "1",
		buff_type = 0,
		desc = "次の戦闘で、全艦火力・雷装+15%、回避-10%。",
		arg = {}
	}
	pg.base.strategy_data_template[2] = {
		buff_id = 110,
		name = "複縦陣",
		iconSize = "",
		type = 1,
		id = 2,
		icon = "2",
		buff_type = 0,
		desc = "次の戦闘で、全艦回避+30%、火力・雷装-5%。",
		arg = {}
	}
	pg.base.strategy_data_template[3] = {
		buff_id = 120,
		name = "輪形陣",
		iconSize = "",
		type = 1,
		id = 3,
		icon = "3",
		buff_type = 0,
		desc = "次の戦闘で、全艦対空+20%。",
		arg = {}
	}
	pg.base.strategy_data_template[4] = {
		buff_id = 0,
		name = "緊急補修",
		iconSize = "",
		type = 2,
		id = 4,
		icon = "4",
		buff_type = 0,
		desc = "戦闘参加可能な艦のHPが10%回復する。",
		arg = {
			healthy,
			10
		}
	}
	pg.base.strategy_data_template[9] = {
		buff_id = 0,
		name = "交換",
		iconSize = "",
		type = 2,
		id = 9,
		icon = "9",
		buff_type = 0,
		desc = "隣の交戦中の味方艦隊と位置を交換する",
		arg = {
			exchange
		}
	}
	pg.base.strategy_data_template[10] = {
		buff_id = 0,
		name = "範囲外支援 ",
		iconSize = "",
		type = 4,
		id = 10,
		icon = "10",
		buff_type = 0,
		desc = "潜水艦の支援範囲外でも潜水支援を使用可能 ",
		arg = {
			map_call
		}
	}
	pg.base.strategy_data_template[11] = {
		buff_id = 0,
		name = "作戦範囲変更 ",
		iconSize = "",
		type = 3,
		id = 11,
		icon = "11",
		buff_type = 0,
		desc = "潜水艦の作戦エリアを変更する ",
		arg = {
			sub_move,
			1.1
		}
	}
	pg.base.strategy_data_template[13] = {
		buff_id = 9670,
		name = "照明弾発射用意",
		iconSize = "",
		type = 6,
		id = 13,
		icon = "yezhan_zhaomingdan",
		buff_type = 0,
		desc = "照明弾補給を入手した。入手した艦隊は今後$1回の戦闘中に照明弾を発射する事が可能",
		arg = {}
	}
	pg.base.strategy_data_template[14] = {
		buff_id = 9600,
		name = "探照灯照射用意",
		iconSize = "",
		type = 10,
		id = 14,
		icon = "yezhan_dengta",
		buff_type = 0,
		desc = "灯台を占拠した。戦闘中味方の探照灯支援が行われることがある",
		arg = {}
	}
	pg.base.strategy_data_template[16] = {
		buff_id = 0,
		name = "悪天候",
		iconSize = "",
		type = 6,
		id = 16,
		icon = "weather_101",
		buff_type = 0,
		desc = "夜の海は悪天候に見舞われている。味方前衛艦隊・敵が「隠蔽範囲」が獲得し、攻撃から狙われにくくなる",
		arg = {}
	}
	pg.base.strategy_data_template[17] = {
		buff_id = 0,
		name = "海霧",
		iconSize = "",
		type = 6,
		id = 17,
		icon = "Weather_102",
		buff_type = 0,
		desc = "艦隊はスコールに覆われている。戦闘中、味方・敵の隠蔽範囲縮小速度が遅くなる",
		arg = {}
	}
	pg.base.strategy_data_template[18] = {
		buff_id = 0,
		name = "ミサイル",
		iconSize = "",
		type = 2,
		id = 18,
		icon = "18",
		buff_type = 0,
		desc = "海域マップでミサイル攻撃を1回行える",
		arg = {
			missile
		}
	}
	pg.base.strategy_data_template[47] = {
		buff_id = 0,
		name = "作戦効率化",
		iconSize = "",
		type = 10,
		id = 47,
		icon = "tebiezuozhan",
		buff_type = 0,
		desc = "戦闘時消費する燃料が100%アップし、指揮官・艦船・オフニャ経験値、信頼度の入手が100%アップ。戦闘終了時のアイテム・報酬入手回数が2回になる",
		arg = {}
	}
	pg.base.strategy_data_template[90] = {
		buff_id = 90,
		name = "危機発生",
		iconSize = "",
		type = 90,
		id = 90,
		icon = "90",
		buff_type = 0,
		desc = "次の戦闘中、おじゃまニセキズナアイが出現し、潜水支援/航空攻撃/魚雷攻撃/主砲攻撃が機能しなくなります。おじゃまニセキズナアイは連続タップで退治することができます",
		arg = {}
	}
	pg.base.strategy_data_template[91] = {
		buff_id = 91,
		name = "危機回避～",
		iconSize = "",
		type = 90,
		id = 91,
		icon = "91",
		buff_type = 0,
		desc = "危機回避～！戦闘中おじゃまニセキズナアイが出現しなくなります",
		arg = {}
	}
	pg.base.strategy_data_template[92] = {
		buff_id = 0,
		name = "夜がやってきた！",
		iconSize = "",
		type = 10,
		id = 92,
		icon = "92",
		buff_type = 0,
		desc = "「夜状態」では、「待ち伏せマス」に敵が出現するようになります",
		arg = {}
	}
	pg.base.strategy_data_template[93] = {
		buff_id = 0,
		name = "朝を迎えた！",
		iconSize = "",
		type = 10,
		id = 93,
		icon = "93",
		buff_type = 0,
		desc = "「昼状態」では、「待ち伏せマス」に敵は出現しません",
		arg = {}
	}
	pg.base.strategy_data_template[94] = {
		buff_id = 9727,
		name = "航空支援（敵）",
		iconSize = "",
		type = 10,
		id = 94,
		icon = "94",
		buff_type = 0,
		desc = "航空支援を担う敵艦隊の存在が確認された。戦闘中、敵は航空攻撃を仕掛けてくるだろう…",
		arg = {}
	}
	pg.base.strategy_data_template[95] = {
		buff_id = 0,
		name = "航空支援（味方）",
		iconSize = "",
		type = 10,
		id = 95,
		icon = "95",
		buff_type = 0,
		desc = "航空支援が可能な味方艦隊が指示を待っている。戦闘中、我々の敵に航空攻撃を仕掛けてくれるだろう…",
		arg = {}
	}
	pg.base.strategy_data_template[96] = {
		buff_id = 262,
		name = "情報掌握",
		iconSize = "",
		type = 10,
		id = 96,
		icon = "",
		buff_type = 1,
		desc = "味方艦隊の情報が敵中枢艦隊に知られた。当該艦隊は次の敵中枢艦隊との戦闘で、与えるダメージが30％ダウンし、受けるダメージが30％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[97] = {
		buff_id = 0,
		name = "情報掌握",
		iconSize = "",
		type = 10,
		id = 97,
		icon = "96",
		buff_type = 0,
		desc = "味方艦隊の情報が敵中枢艦隊に知られた。当該艦隊は次の敵中枢艦隊との戦闘で、与えるダメージが30％ダウンし、受けるダメージが30％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[1000] = {
		buff_id = 0,
		name = "航空攻撃",
		iconSize = "",
		type = 7,
		id = 1000,
		icon = "1000",
		buff_type = 0,
		desc = "敵艦隊に航空攻撃を1回仕掛ける",
		arg = {
			support_missile
		}
	}
	pg.base.strategy_data_template[1001] = {
		buff_id = 0,
		name = "敵艦駆逐",
		iconSize = "",
		type = 7,
		id = 1001,
		icon = "1001",
		buff_type = 0,
		desc = "敵艦隊を周りの空いているマスに移動させる",
		arg = {
			expel
		}
	}
	pg.base.strategy_data_template[1002] = {
		buff_id = 9729,
		name = "機先を制す",
		iconSize = "",
		type = 10,
		id = 1002,
		icon = "1002",
		buff_type = 0,
		desc = "味方の【夜間隠蔽強度】が大幅アップし、敵の【夜間隠蔽強度】が大幅ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[1003] = {
		buff_id = 9733,
		name = "狭路の遭遇戦Ⅰ",
		iconSize = "",
		type = 10,
		id = 1003,
		icon = "1003",
		buff_type = 0,
		desc = "味方の【夜間隠蔽強度】がアップし、敵の【夜間隠蔽強度】がダウン",
		arg = {}
	}
	pg.base.strategy_data_template[1004] = {
		buff_id = 9737,
		name = "狭路の遭遇戦Ⅱ",
		iconSize = "",
		type = 10,
		id = 1004,
		icon = "1003",
		buff_type = 0,
		desc = "味方の【夜間隠蔽強度】がややアップし、敵の【夜間隠蔽強度】がややダウン",
		arg = {}
	}
	pg.base.strategy_data_template[1005] = {
		buff_id = 9741,
		name = "狭路の遭遇戦Ⅲ",
		iconSize = "",
		type = 10,
		id = 1005,
		icon = "1003",
		buff_type = 0,
		desc = "味方の【夜間隠蔽強度】がわずかにアップし、敵の【夜間隠蔽強度】がわずかにダウン",
		arg = {}
	}
	pg.base.strategy_data_template[1006] = {
		buff_id = 9745,
		name = "戦場の霧Ⅰ",
		iconSize = "",
		type = 10,
		id = 1006,
		icon = "1006",
		buff_type = 0,
		desc = "敵の回避ステータスが15%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[1007] = {
		buff_id = 9748,
		name = "戦場の霧Ⅱ",
		iconSize = "",
		type = 10,
		id = 1007,
		icon = "1006",
		buff_type = 0,
		desc = "敵の回避ステータスが10%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[1008] = {
		buff_id = 9751,
		name = "戦場の霧Ⅲ",
		iconSize = "",
		type = 10,
		id = 1008,
		icon = "1006",
		buff_type = 0,
		desc = "敵の回避ステータスが5%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[1009] = {
		buff_id = 9754,
		name = "空中支援Ⅰ（敵）",
		iconSize = "",
		type = 10,
		id = 1009,
		icon = "94",
		buff_type = 0,
		desc = "戦闘中、敵陸上航空基地からの航空支援が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[1010] = {
		buff_id = 9755,
		name = "空中支援Ⅱ（敵）",
		iconSize = "",
		type = 10,
		id = 1010,
		icon = "94",
		buff_type = 0,
		desc = "戦闘中、敵残存陸上航空基地からの航空支援が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[1011] = {
		buff_id = 9756,
		name = "空中支援Ⅲ（敵）",
		iconSize = "",
		type = 10,
		id = 1011,
		icon = "94",
		buff_type = 0,
		desc = "敵陸上航空基地が破壊され、敵の航空支援が弱体化しました",
		arg = {}
	}
	pg.base.strategy_data_template[1012] = {
		buff_id = 9757,
		name = "空中支援Ⅰ（敵）",
		iconSize = "",
		type = 10,
		id = 1012,
		icon = "94",
		buff_type = 0,
		desc = "戦闘中、敵陸上航空基地からの航空支援が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[1013] = {
		buff_id = 9758,
		name = "空中支援Ⅱ（敵）",
		iconSize = "",
		type = 10,
		id = 1013,
		icon = "94",
		buff_type = 0,
		desc = "戦闘中、敵残存陸上航空基地からの航空支援が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[1014] = {
		buff_id = 9759,
		name = "空中支援Ⅲ（敵）",
		iconSize = "",
		type = 10,
		id = 1014,
		icon = "94",
		buff_type = 0,
		desc = "戦闘中、敵残存陸上航空基地からの航空支援が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[1015] = {
		buff_id = 9760,
		name = "空中支援Ⅳ（敵）",
		iconSize = "",
		type = 10,
		id = 1015,
		icon = "94",
		buff_type = 0,
		desc = "敵陸上航空基地が破壊され、敵の航空支援が弱体化しました",
		arg = {}
	}
	pg.base.strategy_data_template[1016] = {
		buff_id = 9761,
		name = "制空权较低",
		iconSize = "",
		type = 10,
		id = 1016,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[8650] = {
		buff_id = 8650,
		name = "聚光灯",
		iconSize = "",
		type = 10,
		id = 8650,
		icon = "",
		buff_type = 0,
		desc = "聚光灯效果",
		arg = {}
	}
	pg.base.strategy_data_template[8732] = {
		buff_id = 8732,
		name = "科野の神籤",
		iconSize = "",
		type = 10,
		id = 8732,
		icon = "8732",
		buff_type = 0,
		desc = "信濃による支援航空攻撃を3回発動可能。発動すると、味方艦隊が順次「火力5%アップ」「雷装5%アップ」「航空5%アップ」を得る",
		arg = {}
	}
	pg.base.strategy_data_template[8744] = {
		buff_id = 8744,
		name = "科野の神籤",
		iconSize = "",
		type = 10,
		id = 8744,
		icon = "8732",
		buff_type = 0,
		desc = "信濃による支援航空攻撃を3回発動可能。発動すると、味方艦隊が順次「火力5%アップ」「雷装5%アップ」「航空5%アップ」を得る",
		arg = {}
	}
	pg.base.strategy_data_template[8745] = {
		buff_id = 8745,
		name = "科野の神籤",
		iconSize = "",
		type = 10,
		id = 8745,
		icon = "8732",
		buff_type = 0,
		desc = "信濃による支援航空攻撃を3回発動可能。発動すると、味方艦隊が順次「火力5%アップ」「雷装5%アップ」「航空5%アップ」を得る",
		arg = {}
	}
	pg.base.strategy_data_template[8746] = {
		buff_id = 8746,
		name = "科野の神籤",
		iconSize = "",
		type = 10,
		id = 8746,
		icon = "8732",
		buff_type = 0,
		desc = "信濃による支援航空攻撃を3回発動可能。発動すると、味方艦隊が順次「火力5%アップ」「雷装5%アップ」「航空5%アップ」を得る",
		arg = {}
	}
	pg.base.strategy_data_template[8750] = {
		buff_id = 8750,
		name = "聚光灯",
		iconSize = "",
		type = 10,
		id = 8750,
		icon = "",
		buff_type = 0,
		desc = "聚光灯效果",
		arg = {}
	}
	pg.base.strategy_data_template[8801] = {
		buff_id = 8801,
		name = "制御権喪失",
		iconSize = "",
		type = 10,
		id = 8801,
		icon = "8801",
		buff_type = 0,
		desc = "浮島要塞の制御権が奪われた！戦闘中に敵航空機が出現してしまう",
		arg = {}
	}
	pg.base.strategy_data_template[8802] = {
		buff_id = 8802,
		name = "制御権確保",
		iconSize = "",
		type = 10,
		id = 8802,
		icon = "8802",
		buff_type = 0,
		desc = "浮島要塞は我が手中にあり！戦闘中に味方航空機が出現し、敵を攻撃してくれる",
		arg = {}
	}
	pg.base.strategy_data_template[8803] = {
		buff_id = 8803,
		name = "味方要塞航空支援",
		iconSize = "",
		type = 10,
		id = 8803,
		icon = "",
		buff_type = 0,
		desc = "敵要塞航空支援",
		arg = {}
	}
	pg.base.strategy_data_template[8806] = {
		buff_id = 8806,
		name = "味方要塞航空支援",
		iconSize = "",
		type = 10,
		id = 8806,
		icon = "",
		buff_type = 0,
		desc = "敵要塞航空支援",
		arg = {}
	}
	pg.base.strategy_data_template[8809] = {
		buff_id = 8809,
		name = "味方要塞航空支援",
		iconSize = "",
		type = 10,
		id = 8809,
		icon = "",
		buff_type = 0,
		desc = "敵要塞航空支援",
		arg = {}
	}
	pg.base.strategy_data_template[8812] = {
		buff_id = 8812,
		name = "味方要塞航空支援",
		iconSize = "",
		type = 10,
		id = 8812,
		icon = "",
		buff_type = 0,
		desc = "敵要塞航空支援",
		arg = {}
	}
	pg.base.strategy_data_template[8815] = {
		buff_id = 8815,
		name = "味方要塞航空支援",
		iconSize = "",
		type = 10,
		id = 8815,
		icon = "",
		buff_type = 0,
		desc = "敵要塞航空支援",
		arg = {}
	}
	pg.base.strategy_data_template[8832] = {
		buff_id = 8832,
		name = "未来を開く砲撃",
		iconSize = "",
		type = 10,
		id = 8832,
		icon = "8832",
		buff_type = 0,
		desc = "味方からの支援砲撃を1回行い、さらに一定時間、敵に持続ダメージを与える。（META戦出現艦船変更から時間が経てば経つほどダメージUP）",
		arg = {}
	}
	pg.base.strategy_data_template[8841] = {
		buff_id = 8841,
		name = "フォトン・プリズム",
		iconSize = "",
		type = 10,
		id = 8841,
		icon = "8841",
		buff_type = 0,
		desc = "戦闘開始時、味方艦隊から支援弾幕が展開される。",
		arg = {}
	}
	pg.base.strategy_data_template[8842] = {
		buff_id = 8842,
		name = "アフタグロー・アンブラ",
		iconSize = "",
		type = 10,
		id = 8842,
		icon = "8842",
		buff_type = 0,
		desc = "味方艦隊の支援弾幕が展開せず、敵艦隊に謎の個体が出現する。",
		arg = {}
	}
	pg.base.strategy_data_template[8843] = {
		buff_id = 8843,
		name = "璀璨支援弹幕LV1",
		iconSize = "",
		type = 10,
		id = 8843,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[8846] = {
		buff_id = 8846,
		name = "璀璨支援弹幕LV2",
		iconSize = "",
		type = 10,
		id = 8846,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[8849] = {
		buff_id = 8849,
		name = "璀璨支援弹幕LV3",
		iconSize = "",
		type = 10,
		id = 8849,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[8852] = {
		buff_id = 8852,
		name = "璀璨支援弹幕LV4",
		iconSize = "",
		type = 10,
		id = 8852,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[8855] = {
		buff_id = 8855,
		name = "璀璨支援弹幕LV5",
		iconSize = "",
		type = 10,
		id = 8855,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[8858] = {
		buff_id = 8858,
		name = "アフタグロー・プリズム",
		iconSize = "",
		type = 10,
		id = 8858,
		icon = "8858",
		buff_type = 0,
		desc = "戦闘開始時、味方艦隊から支援弾幕が展開され、敵艦隊に謎の個体が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[8863] = {
		buff_id = 0,
		name = "迷路",
		iconSize = "",
		type = 10,
		id = 8863,
		icon = "8863",
		buff_type = 0,
		desc = "セイレーンが作った迷路が海を覆っている。\n艦隊が3回行動する度に、迷路の構造が変化する",
		arg = {}
	}
	pg.base.strategy_data_template[8864] = {
		buff_id = 8864,
		name = "セイレーンの歌",
		iconSize = "",
		type = 10,
		id = 8864,
		icon = "8864",
		buff_type = 0,
		desc = "強力なジャミング装置が作動している。\n戦闘中、敵のジャミング効果を与える特殊支援砲撃が味方を襲うことがある。",
		arg = {}
	}
	pg.base.strategy_data_template[8865] = {
		buff_id = 8865,
		name = "空中支援",
		iconSize = "",
		type = 10,
		id = 8865,
		icon = "8802",
		buff_type = 0,
		desc = "アクィラの支援により、\n戦闘中味方からの特殊航空攻撃が出現する場合がある。",
		arg = {}
	}
	pg.base.strategy_data_template[8867] = {
		buff_id = 8867,
		name = "空中支援",
		iconSize = "",
		type = 10,
		id = 8867,
		icon = "8802",
		buff_type = 0,
		desc = "アクィラの支援により、\n戦闘中味方からの特殊航空攻撃が出現する場合がある。",
		arg = {}
	}
	pg.base.strategy_data_template[8869] = {
		buff_id = 8869,
		name = "空中支援",
		iconSize = "",
		type = 10,
		id = 8869,
		icon = "8802",
		buff_type = 0,
		desc = "アクィラの支援により、\n戦闘中味方からの特殊航空攻撃が出現する場合がある。",
		arg = {}
	}
	pg.base.strategy_data_template[8874] = {
		buff_id = 8874,
		name = "再現環境補正システム",
		iconSize = "",
		type = 10,
		id = 8874,
		icon = "8874",
		buff_type = 0,
		desc = "戦闘中、重桜所属艦船が敵に与えるダメージが5%アップし、受けるダメージが5%増加する；ユニオン所属艦船が敵に与えるダメージが5%ダウンし、受けるダメージが5%軽減する",
		arg = {}
	}
	pg.base.strategy_data_template[8877] = {
		buff_id = 8877,
		name = "セイレーン海霧",
		iconSize = "",
		type = 10,
		id = 8877,
		icon = "8877",
		buff_type = 0,
		desc = "戦場はセイレーンが作り出した海霧に覆われている。味方艦隊の命中が5%ダウンし、海域マップでは敵艦隊の種類を確認できない",
		arg = {}
	}
	pg.base.strategy_data_template[8880] = {
		buff_id = 8880,
		name = "ミラー・ハッキング",
		iconSize = "",
		type = 10,
		id = 8880,
		icon = "8880",
		buff_type = 0,
		desc = "謎の友軍が支援してくれている。戦闘中、敵を攻撃する支援弾幕が発生する場合がある",
		arg = {}
	}
	pg.base.strategy_data_template[8882] = {
		buff_id = 8882,
		name = "ミラー・ハッキング",
		iconSize = "",
		type = 10,
		id = 8882,
		icon = "8880",
		buff_type = 0,
		desc = "謎の友軍が支援してくれている。戦闘中、敵を攻撃する支援弾幕が発生する場合がある",
		arg = {}
	}
	pg.base.strategy_data_template[8884] = {
		buff_id = 8884,
		name = "ミラー・ハッキング",
		iconSize = "",
		type = 10,
		id = 8884,
		icon = "8880",
		buff_type = 0,
		desc = "謎の友軍が支援してくれている。戦闘中、敵を攻撃する支援弾幕が発生する場合がある",
		arg = {}
	}
	pg.base.strategy_data_template[8909] = {
		buff_id = 0,
		name = "ハンティングプロトコル",
		iconSize = "",
		type = 10,
		id = 8909,
		icon = "8909",
		buff_type = 0,
		desc = "海域内のセイレーン全艦隊が味方に向けて追撃してきます",
		arg = {}
	}
	pg.base.strategy_data_template[8910] = {
		buff_id = 0,
		name = "着弾予測エリア",
		iconSize = "",
		type = 10,
		id = 8910,
		icon = "8910",
		buff_type = 0,
		desc = "海域全体がミサイルの攻撃範囲内になっています。ミサイルが着弾したマスにいる艦隊はダメージを受けます。\n着弾までの移動可能回数：3",
		arg = {}
	}
	pg.base.strategy_data_template[8911] = {
		buff_id = 0,
		name = "着弾予測エリア",
		iconSize = "",
		type = 10,
		id = 8911,
		icon = "8911",
		buff_type = 0,
		desc = "海域全体がミサイルの攻撃範囲内になっています。ミサイルが着弾したマスにいる艦隊はダメージを受けます。\n着弾までの移動可能回数：2",
		arg = {}
	}
	pg.base.strategy_data_template[8912] = {
		buff_id = 0,
		name = "着弾予測エリア",
		iconSize = "",
		type = 10,
		id = 8912,
		icon = "8912",
		buff_type = 0,
		desc = "海域全体がミサイルの攻撃範囲内になっています。ミサイルが着弾したマスにいる艦隊はダメージを受けます。\n着弾までの移動可能回数：1",
		arg = {}
	}
	pg.base.strategy_data_template[8913] = {
		buff_id = 0,
		name = "ディザスタープロトコル",
		iconSize = "",
		type = 10,
		id = 8913,
		icon = "8913",
		buff_type = 0,
		desc = "セイレーンによる気象操作装置が危険エリアを作り出しています。艦隊が移動する度に危険エリアが拡大していきます。ご注意ください",
		arg = {}
	}
	pg.base.strategy_data_template[8914] = {
		buff_id = 0,
		name = "着弾予測エリア",
		iconSize = "",
		type = 10,
		id = 8914,
		icon = "8914",
		buff_type = 0,
		desc = "海域全体がミサイルの攻撃範囲内になっています。ミサイルが着弾したマスにいる敵艦隊はダメージを受けます。\n着弾までの移動可能回数：3",
		arg = {}
	}
	pg.base.strategy_data_template[8915] = {
		buff_id = 0,
		name = "着弾予測エリア",
		iconSize = "",
		type = 10,
		id = 8915,
		icon = "8915",
		buff_type = 0,
		desc = "海域全体がミサイルの攻撃範囲内になっています。ミサイルが着弾したマスにいる敵艦隊はダメージを受けます。\n着弾までの移動可能回数：2",
		arg = {}
	}
	pg.base.strategy_data_template[8916] = {
		buff_id = 0,
		name = "着弾予測エリア",
		iconSize = "",
		type = 10,
		id = 8916,
		icon = "8916",
		buff_type = 0,
		desc = "海域全体がミサイルの攻撃範囲内になっています。ミサイルが着弾したマスにいる敵艦隊はダメージを受けます。\n着弾までの移動可能回数：1",
		arg = {}
	}
	pg.base.strategy_data_template[8917] = {
		buff_id = 0,
		name = "絶海氷層",
		iconSize = "",
		type = 10,
		id = 8917,
		icon = "8917",
		buff_type = 0,
		desc = "艦隊の移動を阻む氷の壁が出現している。対応する守備艦隊を撃破すると除去可能",
		arg = {}
	}
	pg.base.strategy_data_template[8918] = {
		buff_id = 8918,
		name = "演算式シンクロナイズⅠ",
		iconSize = "",
		type = 10,
		id = 8918,
		icon = "8918",
		buff_type = 0,
		desc = "隕石接続によって演算された幻像。\n戦闘中、無差別で敵味方を攻撃する弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[8919] = {
		buff_id = 8918,
		name = "演算式シンクロナイズⅡ",
		iconSize = "",
		type = 10,
		id = 8919,
		icon = "8919",
		buff_type = 0,
		desc = "隕石接続によって演算された幻像。\n戦闘中、無差別で敵味方を攻撃する弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[8920] = {
		buff_id = 8918,
		name = "演算式シンクロナイズⅢ",
		iconSize = "",
		type = 10,
		id = 8920,
		icon = "8920",
		buff_type = 0,
		desc = "隕石接続によって演算された幻像。\n戦闘中、無差別で敵味方を攻撃する弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[8941] = {
		buff_id = 0,
		name = "鉄血ジャミング",
		iconSize = "",
		type = 10,
		id = 8941,
		icon = "8941",
		buff_type = 0,
		desc = "鉄血によるジャミングで、敵艦隊の規模と種類が海域マップ上で判別不可能になっている。注意！",
		arg = {}
	}
	pg.base.strategy_data_template[8942] = {
		buff_id = 8942,
		name = "特異点の波動",
		iconSize = "",
		type = 10,
		id = 8942,
		icon = "8942",
		buff_type = 0,
		desc = "特異点の特殊環境により、戦闘中味方の主砲が与えるダメージが20%アップ、味方の航空攻撃が与えるダメージが20%ダウン、さらに味方戦艦主砲の散布が5アップ",
		arg = {}
	}
	pg.base.strategy_data_template[8943] = {
		buff_id = 0,
		name = "セイレーンジャミング",
		iconSize = "",
		type = 10,
		id = 8943,
		icon = "8943",
		buff_type = 0,
		desc = "セイレーンの特殊施設によるジャミングが戦場全域に影響を及ぼしている。注意！",
		arg = {}
	}
	pg.base.strategy_data_template[8944] = {
		buff_id = 8944,
		name = "セイレーンジャミング・誘爆",
		iconSize = "",
		type = 10,
		id = 8944,
		icon = "8944",
		buff_type = 0,
		desc = "戦闘中の艦隊は炎上ダメージを受け続ける",
		arg = {}
	}
	pg.base.strategy_data_template[8945] = {
		buff_id = 8945,
		name = "セイレーンジャミング・遅滞",
		iconSize = "",
		type = 10,
		id = 8945,
		icon = "8945",
		buff_type = 0,
		desc = "戦闘中の艦隊の速力が大幅ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[8946] = {
		buff_id = 8946,
		name = "セイレーンジャミング・撹乱",
		iconSize = "",
		type = 10,
		id = 8946,
		icon = "8946",
		buff_type = 0,
		desc = "戦闘中の艦隊は一定の間隔で電磁ジャミングの影響を受ける",
		arg = {}
	}
	pg.base.strategy_data_template[8953] = {
		buff_id = 8953,
		name = "鉄血艦隊支援",
		iconSize = "",
		type = 10,
		id = 8953,
		icon = "8953",
		buff_type = 0,
		desc = "鉄血艦隊による支援が展開される。敵ジャミング効果が無効になり、さらに戦闘中味方による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[8955] = {
		buff_id = 8955,
		name = "鉄血艦隊支援",
		iconSize = "",
		type = 10,
		id = 8955,
		icon = "8953",
		buff_type = 0,
		desc = "鉄血艦隊による支援が展開される。敵ジャミング効果が無効になり、さらに戦闘中味方による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[8957] = {
		buff_id = 8957,
		name = "鉄血艦隊支援",
		iconSize = "",
		type = 10,
		id = 8957,
		icon = "8953",
		buff_type = 0,
		desc = "鉄血艦隊による支援が展開される。敵ジャミング効果が無効になり、さらに戦闘中味方による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[8959] = {
		buff_id = 8959,
		name = "鉄血艦隊支援",
		iconSize = "",
		type = 10,
		id = 8959,
		icon = "8953",
		buff_type = 0,
		desc = "鉄血艦隊による支援が展開される。敵ジャミング効果が無効になり、さらに戦闘中味方による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[8974] = {
		buff_id = 8942,
		name = "特異点の波動TYPE-A",
		iconSize = "",
		type = 10,
		id = 8974,
		icon = "8942",
		buff_type = 0,
		desc = "特異点の特殊環境により、戦闘中味方の砲撃が与えるダメージが20%アップ、味方の航空攻撃が与えるダメージが20%ダウン、さらに味方戦艦主砲の散布が5アップ",
		arg = {}
	}
	pg.base.strategy_data_template[8975] = {
		buff_id = 8975,
		name = "特異点の波動TYPE-B",
		iconSize = "",
		type = 10,
		id = 8975,
		icon = "8975",
		buff_type = 0,
		desc = "特異点の特殊環境により、味方の航空攻撃が与えるダメージが20%アップし、味方の砲撃が与えるダメージが20%ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[8976] = {
		buff_id = 8918,
		name = "演算式シンクロナイズⅠ",
		iconSize = "",
		type = 10,
		id = 8976,
		icon = "8918",
		buff_type = 0,
		desc = "「敵」の精神干渉によって演算された幻像。\n戦闘中、無差別に敵味方を攻撃する弾幕が出現",
		arg = {}
	}
	pg.base.strategy_data_template[9211] = {
		buff_id = 9211,
		name = "竜宮仕掛-白浪",
		iconSize = "",
		type = 10,
		id = 9211,
		icon = "9211",
		buff_type = 0,
		desc = "竜宮仕掛により、戦闘中敵の支援弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[9212] = {
		buff_id = 9212,
		name = "竜宮仕掛-白浪",
		iconSize = "",
		type = 10,
		id = 9212,
		icon = "9212",
		buff_type = 0,
		desc = "確保した竜宮仕掛により、戦闘中味方の支援弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[9213] = {
		buff_id = 9213,
		name = "白浪弾幕支援Lv1",
		iconSize = "",
		type = 10,
		id = 9213,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9216] = {
		buff_id = 9216,
		name = "白浪弾幕支援Lv2",
		iconSize = "",
		type = 10,
		id = 9216,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9219] = {
		buff_id = 9219,
		name = "白浪弾幕支援Lv3",
		iconSize = "",
		type = 10,
		id = 9219,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
end)()
;(function()
	pg.base.strategy_data_template[9222] = {
		buff_id = 9222,
		name = "白浪弾幕支援Lv4",
		iconSize = "",
		type = 10,
		id = 9222,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9225] = {
		buff_id = 9225,
		name = "白浪弾幕支援Lv5",
		iconSize = "",
		type = 10,
		id = 9225,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9231] = {
		buff_id = 9231,
		name = "竜宮仕掛-朱焔",
		iconSize = "",
		type = 10,
		id = 9231,
		icon = "9231",
		buff_type = 0,
		desc = "竜宮仕掛により、戦闘中敵の支援弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[9232] = {
		buff_id = 9232,
		name = "竜宮仕掛-朱焔",
		iconSize = "",
		type = 10,
		id = 9232,
		icon = "9232",
		buff_type = 0,
		desc = "確保した竜宮仕掛により、戦闘中味方の支援弾幕が出現します ",
		arg = {}
	}
	pg.base.strategy_data_template[9233] = {
		buff_id = 9233,
		name = "朱焔弾幕支援Lv1",
		iconSize = "",
		type = 10,
		id = 9233,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9236] = {
		buff_id = 9236,
		name = "朱焔弾幕支援Lv2",
		iconSize = "",
		type = 10,
		id = 9236,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9239] = {
		buff_id = 9239,
		name = "朱焔弾幕支援Lv3",
		iconSize = "",
		type = 10,
		id = 9239,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9242] = {
		buff_id = 9242,
		name = "朱焔弾幕支援Lv4",
		iconSize = "",
		type = 10,
		id = 9242,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9245] = {
		buff_id = 9245,
		name = "朱焔弾幕支援Lv5",
		iconSize = "",
		type = 10,
		id = 9245,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9251] = {
		buff_id = 9251,
		name = "竜宮仕掛-青渦",
		iconSize = "",
		type = 10,
		id = 9251,
		icon = "9251",
		buff_type = 0,
		desc = "竜宮仕掛により、戦闘中敵の支援弾幕が出現し、更に敵はシールドを獲得します",
		arg = {}
	}
	pg.base.strategy_data_template[9252] = {
		buff_id = 9252,
		name = "竜宮仕掛-青渦",
		iconSize = "",
		type = 10,
		id = 9252,
		icon = "9252",
		buff_type = 0,
		desc = "確保した竜宮仕掛により、戦闘中味方の支援弾幕が出現し、更に味方はシールドを獲得します ",
		arg = {}
	}
	pg.base.strategy_data_template[9253] = {
		buff_id = 9253,
		name = "青渦弾幕支援Lv1",
		iconSize = "",
		type = 10,
		id = 9253,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9256] = {
		buff_id = 9256,
		name = "青渦弾幕支援Lv2",
		iconSize = "",
		type = 10,
		id = 9256,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9259] = {
		buff_id = 9259,
		name = "青渦弾幕支援Lv3",
		iconSize = "",
		type = 10,
		id = 9259,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9262] = {
		buff_id = 9262,
		name = "青渦弾幕支援Lv4",
		iconSize = "",
		type = 10,
		id = 9262,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9265] = {
		buff_id = 9265,
		name = "青渦弾幕支援Lv5",
		iconSize = "",
		type = 10,
		id = 9265,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9271] = {
		buff_id = 9271,
		name = "竜宮仕掛-緑風",
		iconSize = "",
		type = 10,
		id = 9271,
		icon = "9271",
		buff_type = 0,
		desc = "竜宮仕掛により、戦闘中敵の支援弾幕が出現し、更に敵の耐久が回復することがあります",
		arg = {}
	}
	pg.base.strategy_data_template[9272] = {
		buff_id = 9272,
		name = "竜宮仕掛-緑風",
		iconSize = "",
		type = 10,
		id = 9272,
		icon = "9272",
		buff_type = 0,
		desc = "確保した竜宮仕掛により、戦闘中味方の支援弾幕が出現し、さらに味方の耐久が回復することがあります",
		arg = {}
	}
	pg.base.strategy_data_template[9273] = {
		buff_id = 9273,
		name = "緑風弾幕支援Lv1",
		iconSize = "",
		type = 10,
		id = 9273,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9276] = {
		buff_id = 9276,
		name = "緑風弾幕支援Lv2",
		iconSize = "",
		type = 10,
		id = 9276,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9279] = {
		buff_id = 9279,
		name = "緑風弾幕支援Lv3",
		iconSize = "",
		type = 10,
		id = 9279,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9282] = {
		buff_id = 9282,
		name = "緑風弾幕支援Lv4",
		iconSize = "",
		type = 10,
		id = 9282,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9285] = {
		buff_id = 9285,
		name = "緑風弾幕支援Lv5",
		iconSize = "",
		type = 10,
		id = 9285,
		icon = "",
		buff_type = 0,
		desc = "特殊支援弾幕",
		arg = {}
	}
	pg.base.strategy_data_template[9391] = {
		buff_id = 0,
		name = "制御奪取 ",
		buff_type = 0,
		type = 10,
		id = 9391,
		icon = "9391",
		desc = "ピュリっち、海域制御0%を取得したぜ！戦闘を継続し、ピュリっちをもっと強化せよ！",
		arg = {},
		iconSize = {
			64,
			84
		}
	}
	pg.base.strategy_data_template[9392] = {
		buff_id = 0,
		name = "制御奪取 ",
		buff_type = 0,
		type = 10,
		id = 9392,
		icon = "9392",
		desc = "ピュリっち、海域制御25%を取得したぜ！戦闘を継続し、ピュリっちをもっと強化せよ！",
		arg = {},
		iconSize = {
			64,
			84
		}
	}
	pg.base.strategy_data_template[9393] = {
		buff_id = 0,
		name = "制御奪取 ",
		buff_type = 0,
		type = 10,
		id = 9393,
		icon = "9393",
		desc = "ピュリっち、海域制御50%を取得したぜ！戦闘を継続し、ピュリっちをもっと強化せよ！",
		arg = {},
		iconSize = {
			64,
			84
		}
	}
	pg.base.strategy_data_template[9394] = {
		buff_id = 0,
		name = "制御奪取 ",
		buff_type = 0,
		type = 10,
		id = 9394,
		icon = "9394",
		desc = "ピュリっち、海域制御75%を取得したぜ！戦闘を継続し、ピュリっちをもっと強化せよ！",
		arg = {},
		iconSize = {
			64,
			84
		}
	}
	pg.base.strategy_data_template[9395] = {
		buff_id = 0,
		name = "制御奪取 ",
		buff_type = 0,
		type = 10,
		id = 9395,
		icon = "9395",
		desc = "ピュリっち、海域制御100%を取得したぜ！戦闘を継続し、ピュリっちをもっと強化せよ！",
		arg = {},
		iconSize = {
			64,
			84
		}
	}
	pg.base.strategy_data_template[9401] = {
		buff_id = 0,
		name = "メカ支援 ",
		iconSize = "",
		type = 10,
		id = 9401,
		icon = "9401",
		buff_type = 0,
		desc = "メカ支援ゲージ充填完了！戦闘中、味方による支援弾幕が展開されます",
		arg = {}
	}
	pg.base.strategy_data_template[9403] = {
		buff_id = 9403,
		name = "宝多六花支援弹幕LV1",
		iconSize = "",
		type = 10,
		id = 9403,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9406] = {
		buff_id = 9406,
		name = "宝多六花支援弹幕LV2",
		iconSize = "",
		type = 10,
		id = 9406,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9409] = {
		buff_id = 9409,
		name = "宝多六花支援弹幕LV3",
		iconSize = "",
		type = 10,
		id = 9409,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9412] = {
		buff_id = 9412,
		name = "宝多六花支援弹幕LV4",
		iconSize = "",
		type = 10,
		id = 9412,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9415] = {
		buff_id = 9415,
		name = "宝多六花支援弹幕LV5",
		iconSize = "",
		type = 10,
		id = 9415,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9421] = {
		buff_id = 0,
		name = "メカ支援 ",
		iconSize = "",
		type = 10,
		id = 9421,
		icon = "9401",
		buff_type = 0,
		desc = "メカ支援ゲージ充填完了！戦闘中、味方による支援弾幕が展開されます",
		arg = {}
	}
	pg.base.strategy_data_template[9423] = {
		buff_id = 9423,
		name = "梦芽支援弹幕LV1",
		iconSize = "",
		type = 10,
		id = 9423,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9426] = {
		buff_id = 9426,
		name = "梦芽支援弹幕LV2",
		iconSize = "",
		type = 10,
		id = 9426,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9429] = {
		buff_id = 9429,
		name = "梦芽支援弹幕LV3",
		iconSize = "",
		type = 10,
		id = 9429,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9432] = {
		buff_id = 9432,
		name = "梦芽支援弹幕LV4",
		iconSize = "",
		type = 10,
		id = 9432,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9435] = {
		buff_id = 9435,
		name = "梦芽支援弹幕LV5",
		iconSize = "",
		type = 10,
		id = 9435,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9441] = {
		buff_id = 0,
		name = "メカ支援 ",
		iconSize = "",
		type = 10,
		id = 9441,
		icon = "9401",
		buff_type = 0,
		desc = "メカ支援ゲージ充填完了！戦闘中、味方による支援弾幕が展開されます",
		arg = {}
	}
	pg.base.strategy_data_template[9443] = {
		buff_id = 9443,
		name = "千濑支援弹幕LV1",
		iconSize = "",
		type = 10,
		id = 9443,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9446] = {
		buff_id = 9446,
		name = "千濑支援弹幕LV2",
		iconSize = "",
		type = 10,
		id = 9446,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9449] = {
		buff_id = 9449,
		name = "千濑支援弹幕LV3",
		iconSize = "",
		type = 10,
		id = 9449,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9452] = {
		buff_id = 9452,
		name = "千濑支援弹幕LV4",
		iconSize = "",
		type = 10,
		id = 9452,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9455] = {
		buff_id = 9455,
		name = "千濑支援弹幕LV5",
		iconSize = "",
		type = 10,
		id = 9455,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9461] = {
		buff_id = 0,
		name = "怪獣支援",
		iconSize = "",
		type = 10,
		id = 9461,
		icon = "9461",
		buff_type = 0,
		desc = "「怪獣」支援ゲージ充填完了！戦闘中、味方による支援弾幕が展開されます",
		arg = {}
	}
	pg.base.strategy_data_template[9463] = {
		buff_id = 9463,
		name = "新条茜支援弹幕LV1",
		iconSize = "",
		type = 10,
		id = 9463,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9466] = {
		buff_id = 9466,
		name = "新条茜支援弹幕LV2",
		iconSize = "",
		type = 10,
		id = 9466,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9469] = {
		buff_id = 9469,
		name = "新条茜支援弹幕LV3",
		iconSize = "",
		type = 10,
		id = 9469,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9472] = {
		buff_id = 9472,
		name = "新条茜支援弹幕LV4",
		iconSize = "",
		type = 10,
		id = 9472,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9475] = {
		buff_id = 9475,
		name = "新条茜支援弹幕LV5",
		iconSize = "",
		type = 10,
		id = 9475,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9481] = {
		buff_id = 0,
		name = "怪獣支援",
		iconSize = "",
		type = 10,
		id = 9481,
		icon = "9461",
		buff_type = 0,
		desc = "「怪獣」支援ゲージ充填完了！戦闘中、味方による支援弾幕が展開されます",
		arg = {}
	}
	pg.base.strategy_data_template[9483] = {
		buff_id = 9483,
		name = "貉支援弹幕LV1",
		iconSize = "",
		type = 10,
		id = 9483,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9486] = {
		buff_id = 9486,
		name = "貉支援弹幕LV2",
		iconSize = "",
		type = 10,
		id = 9486,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9489] = {
		buff_id = 9489,
		name = "貉支援弹幕LV3",
		iconSize = "",
		type = 10,
		id = 9489,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9492] = {
		buff_id = 9492,
		name = "貉支援弹幕LV4",
		iconSize = "",
		type = 10,
		id = 9492,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9495] = {
		buff_id = 9495,
		name = "貉支援弹幕LV5",
		iconSize = "",
		type = 10,
		id = 9495,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[9500] = {
		buff_id = 9500,
		name = "キューブ活性化 ",
		iconSize = "",
		type = 10,
		id = 9500,
		icon = "9500",
		buff_type = 0,
		desc = "メンタルキューブ活性化により、味方艦隊全員の与えるダメージ・受けるダメージが20%アップ ",
		arg = {}
	}
	pg.base.strategy_data_template[9502] = {
		buff_id = 9502,
		name = "海霧襲来",
		iconSize = "",
		type = 10,
		id = 9502,
		icon = "9502",
		buff_type = 0,
		desc = "不気味な海霧が戦場を覆っている…味方艦隊全員の命中が5%ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[9505] = {
		buff_id = 9505,
		name = "鉄血航空支援",
		iconSize = "",
		type = 10,
		id = 9505,
		icon = "10017",
		buff_type = 0,
		desc = "敵航空支援展開中！ステージ内に敵航空攻撃隊が出現し、更に敵の航空攻撃で受けるダメージが10%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[9508] = {
		buff_id = 9508,
		name = "極夜",
		iconSize = "",
		type = 10,
		id = 9508,
		icon = "10016",
		buff_type = 0,
		desc = "極夜状態では、敵艦隊の「艦隊編成タイプ」を視認確認できません",
		arg = {}
	}
	pg.base.strategy_data_template[10001] = {
		buff_id = 200,
		name = "完全補給",
		iconSize = "",
		type = 1000,
		id = 10001,
		icon = "10001",
		buff_type = 0,
		desc = "弾薬満タン、艦隊ダメージ+10%。",
		arg = {}
	}
	pg.base.strategy_data_template[10002] = {
		buff_id = 210,
		name = "弾薬不足",
		iconSize = "",
		type = 1000,
		id = 10002,
		icon = "10002",
		buff_type = 0,
		desc = "弾切れ寸前、艦隊ダメージ-50%。",
		arg = {}
	}
	pg.base.strategy_data_template[10011] = {
		buff_id = 220,
		name = "制空権確保",
		iconSize = "",
		type = 1001,
		id = 10011,
		icon = "10011",
		buff_type = 0,
		desc = "味方航空攻撃によるダメージが20%アップし、敵航空攻撃によるダメージが10%ダウン（浸水・炎上ダメージを除く）。味方全員の命中が10%アップし、待ち伏せ遭遇率が8%ダウン",
		arg = {
			800
		}
	}
	pg.base.strategy_data_template[10012] = {
		buff_id = 230,
		name = "制空権優勢",
		iconSize = "",
		type = 1001,
		id = 10012,
		icon = "10012",
		buff_type = 0,
		desc = "味方航空攻撃によるダメージが12%アップし、敵航空攻撃によるダメージが6%ダウン（浸水・炎上ダメージを除く）。味方全員の命中が5%アップし、待ち伏せ遭遇率が5%ダウン",
		arg = {
			500
		}
	}
	pg.base.strategy_data_template[10013] = {
		buff_id = 240,
		name = "制空拮抗中",
		iconSize = "",
		type = 1001,
		id = 10013,
		icon = "10013",
		buff_type = 0,
		desc = "味方航空攻撃によるダメージが6%ダウンし、敵航空攻撃によるダメージが3%ダウン（浸水・炎上ダメージを除く）",
		arg = {
			0
		}
	}
	pg.base.strategy_data_template[10014] = {
		buff_id = 250,
		name = "制空権劣勢",
		iconSize = "",
		type = 1001,
		id = 10014,
		icon = "10014",
		buff_type = 0,
		desc = "味方航空攻撃によるダメージが12%ダウンし、敵航空攻撃によるダメージが6%アップ（浸水・炎上ダメージを除く）。味方全員の命中・回避が3%ダウン",
		arg = {
			0
		}
	}
	pg.base.strategy_data_template[10015] = {
		buff_id = 260,
		name = "制空権喪失",
		iconSize = "",
		type = 1001,
		id = 10015,
		icon = "10015",
		buff_type = 0,
		desc = "味方航空攻撃によるダメージが20%ダウンし、敵航空攻撃によるダメージが10%アップ（浸水・炎上ダメージを除く）。味方全員の命中・回避が8%ダウン",
		arg = {
			0
		}
	}
	pg.base.strategy_data_template[10021] = {
		buff_id = 8761,
		name = "FEVERモード",
		iconSize = "",
		type = 1000,
		id = 10021,
		icon = "10021",
		buff_type = 0,
		desc = "味方艦隊戦闘開始時、自身の耐久値が最大値の2%回復する",
		arg = {}
	}
	pg.base.strategy_data_template[10031] = {
		buff_id = 8765,
		name = "特別アピール",
		iconSize = "",
		type = 1000,
		id = 10031,
		icon = "10031",
		buff_type = 0,
		desc = "戦闘開始時、味方全員の耐久が2%回復する",
		arg = {}
	}
	pg.base.strategy_data_template[10032] = {
		buff_id = 8767,
		name = "特別アピール",
		iconSize = "",
		type = 1000,
		id = 10032,
		icon = "10032",
		buff_type = 0,
		desc = "戦闘中、味方全員の敵に与えるダメージが5％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[10033] = {
		buff_id = 8769,
		name = "特別アピール",
		iconSize = "",
		type = 1000,
		id = 10033,
		icon = "10033",
		buff_type = 0,
		desc = "戦闘中、味方全員の敵から受けるダメージが5%ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[200000] = {
		buff_id = 200000,
		name = "飛行船団支援",
		iconSize = "",
		type = 10,
		id = 200000,
		icon = "200000",
		buff_type = 0,
		desc = "こちら艦長アルフレード、これより支援砲撃開始！パシャリ！――戦闘中飛行船団による砲撃支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200003] = {
		buff_id = 200003,
		name = "飛行船団支援",
		iconSize = "",
		type = 10,
		id = 200003,
		icon = "200000",
		buff_type = 0,
		desc = "こちら艦長アルフレード、これより支援砲撃開始！パシャリ！――戦闘中飛行船団による砲撃支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200006] = {
		buff_id = 200006,
		name = "飛行船団支援",
		iconSize = "",
		type = 10,
		id = 200006,
		icon = "200000",
		buff_type = 0,
		desc = "こちら艦長アルフレード、これより支援砲撃開始！パシャリ！――戦闘中飛行船団による砲撃支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200009] = {
		buff_id = 200009,
		name = "ミニマム鏡面海域",
		iconSize = "",
		type = 10,
		id = 200009,
		icon = "200009",
		buff_type = 0,
		desc = "サディア所属の艦船の戦闘中に与えるダメージが20%アップし、受けるダメージが20％ダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[200011] = {
		buff_id = 200011,
		name = "神祖の力",
		iconSize = "",
		type = 10,
		id = 200011,
		icon = "200011",
		buff_type = 0,
		desc = "嵐のような猛攻――戦闘中味方艦船の火力・航空が10%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200012] = {
		buff_id = 200012,
		name = "至尊者の力",
		iconSize = "",
		type = 10,
		id = 200012,
		icon = "200012",
		buff_type = 0,
		desc = "響かせる角笛の軍勢――戦闘中味方艦船の装填が15％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200013] = {
		buff_id = 200013,
		name = "威厳者の力",
		iconSize = "",
		type = 10,
		id = 200013,
		icon = "200013",
		buff_type = 0,
		desc = "燃ゆる太陽の光――戦闘開始時、味方艦船の耐久が上限の3%回復する",
		arg = {}
	}
	pg.base.strategy_data_template[200015] = {
		buff_id = 200015,
		name = "最善者の力",
		iconSize = "",
		type = 10,
		id = 200015,
		icon = "200015",
		buff_type = 0,
		desc = "広げる極盛の地平――戦闘中味方艦船の回避が10％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200016] = {
		buff_id = 200016,
		name = "正規者の力",
		iconSize = "",
		type = 10,
		id = 200016,
		icon = "200016",
		buff_type = 0,
		desc = "流転せし運命の波――戦闘中敵はダメージを受け続ける",
		arg = {}
	}
	pg.base.strategy_data_template[200019] = {
		buff_id = 200019,
		name = "背教者の力",
		iconSize = "",
		type = 10,
		id = 200019,
		icon = "200019",
		buff_type = 0,
		desc = "夕闇の残光の余熱――戦闘開始時、味方旗艦の主砲攻撃・航空攻撃が1回装填完了にする",
		arg = {}
	}
	pg.base.strategy_data_template[200023] = {
		buff_id = 0,
		name = "空襲",
		iconSize = "",
		type = 10,
		id = 200023,
		icon = "200023",
		buff_type = 0,
		desc = "実験作戦ルール：海域での行動中、敵空襲に遭遇する場合がある。艦隊の索敵値が上がると敵空襲に遭遇する確率が低減する",
		arg = {}
	}
	pg.base.strategy_data_template[200024] = {
		buff_id = 0,
		name = "待ち伏せ",
		iconSize = "",
		type = 10,
		id = 200024,
		icon = "200024",
		buff_type = 0,
		desc = "実験作戦ルール：海域での行動中、敵待ち伏せに遭遇する場合がある。艦隊の索敵値が上がると敵待ち伏せに遭遇する確率が低減する",
		arg = {}
	}
	pg.base.strategy_data_template[200025] = {
		buff_id = 0,
		name = "敵追撃",
		iconSize = "",
		type = 10,
		id = 200025,
		icon = "200025",
		buff_type = 0,
		desc = "実験作戦ルール：海域での行動中、一部の敵部隊はこちらの行動を追跡する場合がある",
		arg = {}
	}
	pg.base.strategy_data_template[200052] = {
		buff_id = 200052,
		name = "結界碁盤",
		iconSize = "",
		type = 10,
		id = 200052,
		icon = "200052",
		buff_type = 0,
		desc = "イベントストーリーでの「部下」艦船の全ステータスが3％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200054] = {
		buff_id = 200054,
		name = "霧隠れの真意",
		iconSize = "",
		type = 10,
		id = 200054,
		icon = "200054",
		buff_type = 0,
		desc = "戦闘中、互いの空母以外の艦船の命中が5%ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[200057] = {
		buff_id = 200057,
		name = "変革目指す先見",
		iconSize = "",
		type = 10,
		id = 200057,
		icon = "200057",
		buff_type = 0,
		desc = "戦闘中、互いの速力が5%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200060] = {
		buff_id = 200060,
		name = "孤高なりし未来像",
		iconSize = "",
		type = 10,
		id = 200060,
		icon = "200060",
		buff_type = 0,
		desc = "戦闘中、互いが受けるダメージが5%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200063] = {
		buff_id = 200063,
		name = "朗らかく正々堂々",
		iconSize = "",
		type = 10,
		id = 200063,
		icon = "200063",
		buff_type = 0,
		desc = "戦闘中、互いの戦艦・巡洋戦艦の火力・回避が5%アップ、駆逐艦の火力・回避が5％ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[200067] = {
		buff_id = 200067,
		name = "皆既無用の野望",
		iconSize = "",
		type = 10,
		id = 200067,
		icon = "200067",
		buff_type = 0,
		desc = "戦闘中、互い戦艦・巡洋戦艦の主砲散布範囲が2アップし、火力が5％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200070] = {
		buff_id = 200070,
		name = "激動の雷鳴",
		iconSize = "",
		type = 10,
		id = 200070,
		icon = "200070",
		buff_type = 0,
		desc = "戦闘中に雷撃が発生し、範囲内の味方と敵に現在耐久の一定の割合のダメージを与える",
		arg = {}
	}
	pg.base.strategy_data_template[200074] = {
		buff_id = 200074,
		name = "相争わぬ誓い",
		iconSize = "",
		type = 10,
		id = 200074,
		icon = "200074",
		buff_type = 0,
		desc = "戦闘中一定時間毎に、互いの艦船が少し回復する",
		arg = {}
	}
	pg.base.strategy_data_template[200078] = {
		buff_id = 200078,
		name = "守りたい決意",
		iconSize = "",
		type = 10,
		id = 200078,
		icon = "200078",
		buff_type = 0,
		desc = "戦闘中互いが受けるダメージを3％ダウン。一定時間ごとに、互いの艦船に弾を防ぐシールドを付与するか少し回復させる",
		arg = {}
	}
	pg.base.strategy_data_template[200079] = {
		buff_id = 0,
		name = "始まりの地",
		iconSize = "",
		type = 10,
		id = 200079,
		icon = "200079",
		buff_type = 0,
		desc = "風属性の素材が採取可能：ナナシ草、ウィングプラント、ブラッドトーン（採取道具-サイレントシックルが必要）",
		arg = {}
	}
	pg.base.strategy_data_template[200080] = {
		buff_id = 0,
		name = "異界の森",
		iconSize = "",
		type = 10,
		id = 200080,
		icon = "200080",
		buff_type = 0,
		desc = "雷属性の素材が採取可能：イナズマ鉱、導きの星の欠片、セプトリエン（採取道具-ゴールデンアックスが必要）",
		arg = {}
	}
	pg.base.strategy_data_template[200081] = {
		buff_id = 0,
		name = "紛争の城",
		iconSize = "",
		type = 10,
		id = 200081,
		icon = "200081",
		buff_type = 0,
		desc = "火属性の素材が採取可能：小さな晶石、燃える砂、マグマパウダー（採取道具-ブルーフラムロッドが必要）",
		arg = {}
	}
	pg.base.strategy_data_template[200082] = {
		buff_id = 0,
		name = "滅失の都",
		iconSize = "",
		type = 10,
		id = 200082,
		icon = "200082",
		buff_type = 0,
		desc = "氷属性の素材が採取可能：プルムル、サルディン、湖底のぬし（採取道具-神竿バルムヘルが必要）",
		arg = {}
	}
	pg.base.strategy_data_template[200083] = {
		buff_id = 0,
		name = "中枢の拠",
		iconSize = "",
		type = 10,
		id = 200083,
		icon = "200083",
		buff_type = 0,
		desc = "セイレーン素材が採取可能：謎の結晶鉱、謎の装甲材、謎のエナジーコア（採取道具-レーザーアームが必要）",
		arg = {}
	}
end)()
;(function()
	pg.base.strategy_data_template[200084] = {
		buff_id = 0,
		name = "サイレントシックル",
		iconSize = "",
		type = 10,
		id = 200084,
		icon = "200084",
		buff_type = 0,
		desc = "採取しやすいように改良されたこの鎌は、まるで風のように静かに軽やかに草や花を刈り取る",
		arg = {}
	}
	pg.base.strategy_data_template[200085] = {
		buff_id = 0,
		name = "ゴールデンアックス",
		iconSize = "",
		type = 10,
		id = 200085,
		icon = "200085",
		buff_type = 0,
		desc = "切れ味や重さを細かく調整し改良を施されたため、普通の斧よりもラクに木材も薪も作れる",
		arg = {}
	}
	pg.base.strategy_data_template[200086] = {
		buff_id = 0,
		name = "ブルーフラムロッド",
		iconSize = "",
		type = 10,
		id = 200086,
		icon = "200086",
		buff_type = 0,
		desc = "フラムロッドの強化版。先端の宝玉を青い物に変えたため、以前よりも効率的に岩や水晶を破壊することが可能となった",
		arg = {}
	}
	pg.base.strategy_data_template[200087] = {
		buff_id = 0,
		name = "神竿バルムヘル",
		iconSize = "",
		type = 10,
		id = 200087,
		icon = "200087",
		buff_type = 0,
		desc = "釣り竿の強化版。強化したことにより、魚が警戒せずに寄ってくるように改良されている",
		arg = {}
	}
	pg.base.strategy_data_template[200088] = {
		buff_id = 0,
		name = "レーザーアーム",
		iconSize = "",
		type = 10,
		id = 200088,
		icon = "200088",
		buff_type = 0,
		desc = "強大な握力と推力、そして灼熱の採掘レーザーを発射できるセイレーンのメカニカルアーム――錬金術の素材を採取するのに使えるかも？",
		arg = {}
	}
	pg.base.strategy_data_template[200239] = {
		buff_id = 200239,
		name = "自律型支援ドローン",
		iconSize = "",
		type = 10,
		id = 200239,
		icon = "200239",
		buff_type = 0,
		desc = "アンチエックスボディの戦闘サポートにより、戦闘中味方火力・雷装・航空が10%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200240] = {
		buff_id = 200240,
		name = "アビータの脅威・予兆",
		iconSize = "",
		type = 10,
		id = 200240,
		icon = "200240",
		buff_type = 0,
		desc = "「何かがおかしい…」戦闘中無差別攻撃する弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200241] = {
		buff_id = 200240,
		name = "アビータの脅威・肉迫",
		iconSize = "",
		type = 10,
		id = 200241,
		icon = "200241",
		buff_type = 0,
		desc = "「黒い靄に覆われるものは…」戦闘中無差別攻撃する弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200242] = {
		buff_id = 200242,
		name = "空间净化系统",
		iconSize = "",
		type = 10,
		id = 200242,
		icon = "200242",
		buff_type = 0,
		desc = "——「空间净化系统正在启动中」——\n每隔一段时间，场上敌我双方所有单位均会根据百分比损失一定耐久值。",
		arg = {}
	}
	pg.base.strategy_data_template[200327] = {
		buff_id = 200327,
		name = "聖堂施設制御装置-制御不能",
		iconSize = "",
		type = 10,
		id = 200327,
		icon = "200327",
		buff_type = 0,
		desc = "聖堂施設の制御装置が制御不能状態にある。\n戦闘中一定時間毎に特殊弾幕が出現し、戦場にいるすべてを対象に耐久最大値に比例するダメージを与える。",
		arg = {}
	}
	pg.base.strategy_data_template[200330] = {
		buff_id = 200330,
		name = "聖堂施設制御装置-制御回復",
		iconSize = "",
		type = 10,
		id = 200330,
		icon = "200330",
		buff_type = 0,
		desc = "聖堂施設の制御装置が回復しつつある。\n戦闘中一定時間毎に特殊弾幕が出現し、戦場にいるすべての敵に耐久最大値に比例するダメージを与える。",
		arg = {}
	}
	pg.base.strategy_data_template[200333] = {
		buff_id = 200333,
		name = "聖堂施設制御装置-制御安定",
		iconSize = "",
		type = 10,
		id = 200333,
		icon = "200333",
		buff_type = 0,
		desc = "聖堂施設の制御装置が安定状態にある。\n戦闘中一定時間毎に特殊弾幕が出現し、戦場にいるすべての敵に耐久最大値に比例するダメージを与え、さらにすべての味方艦船の耐久を回復させる",
		arg = {}
	}
	pg.base.strategy_data_template[200336] = {
		buff_id = 200336,
		name = "信仰心象の加護-アイリス",
		iconSize = "",
		type = 10,
		id = 200336,
		icon = "200336",
		buff_type = 0,
		desc = "アイリスへの祈りが届き、篤い心象が現実に影響する——\n戦闘中、味方アイリス艦船の火力・雷装・航空・対空・装填・命中・回避が3％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200338] = {
		buff_id = 200338,
		name = "信仰心象の加護-ロイヤル",
		iconSize = "",
		type = 10,
		id = 200338,
		icon = "200338",
		buff_type = 0,
		desc = "？？？への祈りが届き、篤い心象が現実に影響する——\n戦闘中、味方ロイヤル艦船の火力・雷装・航空・対空・装填・命中・回避が3％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200392] = {
		buff_id = 0,
		name = "追撃戦術",
		iconSize = "",
		type = 10,
		id = 200392,
		icon = "8909",
		buff_type = 0,
		desc = "敵艦隊がこちらの艦隊に追撃を仕掛ける",
		arg = {}
	}
	pg.base.strategy_data_template[200394] = {
		buff_id = 200394,
		name = "侵食空間",
		iconSize = "",
		type = 10,
		id = 200394,
		icon = "200394",
		buff_type = 0,
		desc = "現在の侵食レベル：0\n味方艦隊が戦闘を行うと、戦闘終了時侵食レベルが1アップ（最大5まで）。侵食レベルに応じて、味方艦隊は戦闘中での与えるダメージ・受けるダメージがアップする。\n侵食レベル5の場合、戦闘中味方艦船の速力・命中がダウンする。\nMETA艦船など一部の艦船は侵食レベルによる被ダメージアップ・命中速力ダウン効果を受けない",
		arg = {}
	}
	pg.base.strategy_data_template[200396] = {
		buff_id = 200396,
		name = "侵食空間",
		iconSize = "",
		type = 10,
		id = 200396,
		icon = "200394",
		buff_type = 0,
		desc = "現在の侵食レベル：1\n味方艦隊が戦闘を行うと、戦闘終了時侵食レベルが1アップ（最大5まで）。侵食レベルに応じて、味方艦隊は戦闘中での与えるダメージ・受けるダメージがアップする。\n侵食レベル5の場合、戦闘中味方艦船の速力・命中がダウンする。\nMETA艦船など一部の艦船は侵食レベルによる被ダメージアップ・命中速力ダウン効果を受けない",
		arg = {}
	}
	pg.base.strategy_data_template[200401] = {
		buff_id = 200401,
		name = "侵食空間",
		iconSize = "",
		type = 10,
		id = 200401,
		icon = "200394",
		buff_type = 0,
		desc = "現在の侵食レベル：2\n味方艦隊が戦闘を行うと、戦闘終了時侵食レベルが1アップ（最大5まで）。侵食レベルに応じて、味方艦隊は戦闘中での与えるダメージ・受けるダメージがアップする。\n侵食レベル5の場合、戦闘中味方艦船の速力・命中がダウンする。\nMETA艦船など一部の艦船は侵食レベルによる被ダメージアップ・命中速力ダウン効果を受けない",
		arg = {}
	}
	pg.base.strategy_data_template[200406] = {
		buff_id = 200406,
		name = "侵食空間",
		iconSize = "",
		type = 10,
		id = 200406,
		icon = "200394",
		buff_type = 0,
		desc = "現在の侵食レベル：3\n味方艦隊が戦闘を行うと、戦闘終了時侵食レベルが1アップ（最大5まで）。侵食レベルに応じて、味方艦隊は戦闘中での与えるダメージ・受けるダメージがアップする。\n侵食レベル5の場合、戦闘中味方艦船の速力・命中がダウンする。\nMETA艦船など一部の艦船は侵食レベルによる被ダメージアップ・命中速力ダウン効果を受けない",
		arg = {}
	}
	pg.base.strategy_data_template[200411] = {
		buff_id = 200411,
		name = "侵食空間",
		iconSize = "",
		type = 10,
		id = 200411,
		icon = "200394",
		buff_type = 0,
		desc = "現在の侵食レベル：4\n味方艦隊が戦闘を行うと、戦闘終了時侵食レベルが1アップ（最大5まで）。侵食レベルに応じて、味方艦隊は戦闘中での与えるダメージ・受けるダメージがアップする。\n侵食レベル5の場合、戦闘中味方艦船の速力・命中がダウンする。\nMETA艦船など一部の艦船は侵食レベルによる被ダメージアップ・命中速力ダウン効果を受けない",
		arg = {}
	}
	pg.base.strategy_data_template[200416] = {
		buff_id = 200416,
		name = "侵食空間",
		iconSize = "",
		type = 10,
		id = 200416,
		icon = "200394",
		buff_type = 0,
		desc = "現在の侵食レベル：5\n味方艦隊が戦闘を行うと、戦闘終了時侵食レベルが1アップ（最大5まで）。侵食レベルに応じて、味方艦隊は戦闘中での与えるダメージ・受けるダメージがアップする。\n侵食レベル5の場合、戦闘中味方艦船の速力・命中がダウンする。\nMETA艦船など一部の艦船は侵食レベルによる被ダメージアップ・命中速力ダウン効果を受けない",
		arg = {}
	}
	pg.base.strategy_data_template[200432] = {
		buff_id = 200432,
		name = "構造の理・援護",
		iconSize = "",
		type = 10,
		id = 200432,
		icon = "200432",
		buff_type = 0,
		desc = "艦船のサポートで侵食レベル上限が下がり、戦闘中一定時間毎に味方の耐久が回復し、更にステータスが上昇する。\nステージの侵食レベル上限：1",
		arg = {}
	}
	pg.base.strategy_data_template[200433] = {
		buff_id = 200432,
		name = "構造の理・援護",
		iconSize = "",
		type = 10,
		id = 200433,
		icon = "200432",
		buff_type = 0,
		desc = "艦船のサポートで侵食レベル上限が下がり、戦闘中一定時間毎に味方の耐久が回復し、更にステータスが上昇する。\nステージの侵食レベル上限：2",
		arg = {}
	}
	pg.base.strategy_data_template[200434] = {
		buff_id = 200432,
		name = "構造の理・援護",
		iconSize = "",
		type = 10,
		id = 200434,
		icon = "200432",
		buff_type = 0,
		desc = "艦船のサポートで侵食レベル上限が下がり、戦闘中一定時間毎に味方の耐久が回復し、更にステータスが上昇する。\nステージの侵食レベル上限：3",
		arg = {}
	}
	pg.base.strategy_data_template[200511] = {
		buff_id = 200511,
		name = "古立特联动复刻弹幕支援",
		iconSize = "",
		type = 10,
		id = 200511,
		icon = "",
		buff_type = 0,
		desc = "不显示图标及说明",
		arg = {}
	}
	pg.base.strategy_data_template[200535] = {
		buff_id = 200535,
		name = "連合艦隊",
		iconSize = "",
		type = 10,
		id = 200535,
		icon = "200535",
		buff_type = 0,
		desc = "戦闘中、味方艦船の所属陣営が合計2つ以上の場合、所属陣営1つにつき味方艦船全員の全ステータスが1%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200536] = {
		buff_id = 0,
		name = "侵食性錯体",
		iconSize = "",
		type = 10,
		id = 200536,
		icon = "200536",
		buff_type = 0,
		desc = "「世界を覆う謎の物質…これは一体？」\n一部の敵が撃破された時に謎の物質を放出し、周囲の敵の耐久を回復し、同時に味方艦船にダメージを与える",
		arg = {}
	}
	pg.base.strategy_data_template[200538] = {
		buff_id = 200538,
		name = "審判廷支援",
		iconSize = "",
		type = 10,
		id = 200538,
		icon = "200538",
		buff_type = 0,
		desc = "「指揮官、私の次の一手はこれよ」\n戦闘中、味方の審判型量産艦と審判型武装機構による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200541] = {
		buff_id = 200541,
		name = "審判廷支援",
		iconSize = "",
		type = 10,
		id = 200541,
		icon = "200538",
		buff_type = 0,
		desc = "「指揮官、私の次の一手はこれよ」\n戦闘中、味方の審判型量産艦と審判型武装機構による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200544] = {
		buff_id = 200544,
		name = "偽神の領域Ⅰ",
		iconSize = "",
		type = 10,
		id = 200544,
		icon = "200544",
		buff_type = 0,
		desc = "——「かの者が降臨せん」——\n偽神の領域内の艦船はすべてのステータスがダウン\nまた、戦闘中に敵の支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200545] = {
		buff_id = 200545,
		name = "偽神の領域Ⅱ",
		iconSize = "",
		type = 10,
		id = 200545,
		icon = "200545",
		buff_type = 0,
		desc = "——「かの▅▇は降▇▅せん」——\n偽神の領域内の艦船はすべてのステータスがさらにダウン\nまた、戦闘中に敵の強力な支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200546] = {
		buff_id = 200546,
		name = "偽神の領域Ⅲ",
		iconSize = "",
		type = 10,
		id = 200546,
		icon = "200546",
		buff_type = 0,
		desc = "——「▇▆▇▊▇▇▆▅▇▆」——\n偽神の領域内の艦船はすべてのステータスが大幅にダウン\nまた、戦闘中に敵のより強力な支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200583] = {
		buff_id = 200583,
		name = "闇靄の侵食",
		iconSize = "",
		type = 10,
		id = 200583,
		icon = "200583",
		buff_type = 0,
		desc = "災いの闇靄の侵食が、最恐の敵を凝華させる。\n戦闘中一定時間ごとに、味方を襲ってくる弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200585] = {
		buff_id = 200585,
		name = "神木の加護",
		iconSize = "",
		type = 10,
		id = 200585,
		icon = "200585",
		buff_type = 0,
		desc = "ワタツミの封印の加護が闇靄を弱らせ始めている。\n戦闘中一定時間ごとに、味方艦船に耐久上限の一定の割合のダメージを無効にできるシールドを生成する",
		arg = {}
	}
	pg.base.strategy_data_template[200589] = {
		buff_id = 200589,
		name = "心象の霊河",
		iconSize = "",
		type = 10,
		id = 200589,
		icon = "200589",
		buff_type = 0,
		desc = "願いの力で難局を乗り越えていけ。\n戦闘中一定時間ごとに、味方艦船を援護する特殊弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200592] = {
		buff_id = 200592,
		name = "心象の霊河",
		iconSize = "",
		type = 10,
		id = 200592,
		icon = "200589",
		buff_type = 0,
		desc = "願いの力で難局を乗り越えていけ。\n戦闘中一定時間ごとに、味方艦船を援護する特殊弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200595] = {
		buff_id = 200595,
		name = "心象の霊河",
		iconSize = "",
		type = 10,
		id = 200595,
		icon = "200589",
		buff_type = 0,
		desc = "願いの力で難局を乗り越えていけ。\n戦闘中一定時間ごとに、味方艦船を援護する特殊弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200618] = {
		buff_id = 200618,
		name = "レジェンド旗艦",
		iconSize = "",
		type = 10,
		id = 200618,
		icon = "200618",
		buff_type = 0,
		desc = "「偉大なるロイヤル・フォーチュン号」出撃！\n戦闘中、味方艦隊を援護してくれる、偉大なる旗艦の砲撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200620] = {
		buff_id = 200620,
		name = "レジェンド旗艦",
		iconSize = "",
		type = 10,
		id = 200620,
		icon = "200618",
		buff_type = 0,
		desc = "「偉大なるロイヤル・フォーチュン号」出撃！\n戦闘中、味方艦隊を援護してくれる、偉大なる旗艦の砲撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200622] = {
		buff_id = 200622,
		name = "レジェンド旗艦",
		iconSize = "",
		type = 10,
		id = 200622,
		icon = "200618",
		buff_type = 0,
		desc = "「偉大なるロイヤル・フォーチュン号」出撃！\n戦闘中、味方艦隊を援護してくれる、偉大なる旗艦の砲撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200624] = {
		buff_id = 200624,
		name = "異常気象:嵐",
		iconSize = "",
		type = 10,
		id = 200624,
		icon = "200624",
		buff_type = 0,
		desc = "猛り狂う嵐の中で戦い続ける……\nテンペスタ所属艦船が受けるダメージが3％ダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[200627] = {
		buff_id = 200627,
		name = "異常気象:台風の目",
		iconSize = "",
		type = 10,
		id = 200627,
		icon = "200627",
		buff_type = 0,
		desc = "凪の台風の目で戦い続ける……\nテンペスタ所属艦船が与えるダメージが3％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[200630] = {
		buff_id = 200630,
		name = "メテオミサイル",
		iconSize = "",
		type = 10,
		id = 200630,
		icon = "200630",
		buff_type = 0,
		desc = "戦闘中、作戦支援端末「たーみなる」が制御するミサイル支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200633] = {
		buff_id = 200633,
		name = "メテオミサイル",
		iconSize = "",
		type = 10,
		id = 200633,
		icon = "200630",
		buff_type = 0,
		desc = "戦闘中、作戦支援端末「たーみなる」が制御するミサイル支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200645] = {
		buff_id = 200645,
		name = "地脈仕掛-陰",
		iconSize = "",
		type = 10,
		id = 200645,
		icon = "200645",
		buff_type = 0,
		desc = "地脈仕掛が敵に制御されている。戦闘中、敵の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200646] = {
		buff_id = 200646,
		name = "地脈仕掛-陽",
		iconSize = "",
		type = 10,
		id = 200646,
		icon = "200646",
		buff_type = 0,
		desc = "地脈仕掛が敵に制御されている。戦闘中、敵の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200647] = {
		buff_id = 200647,
		name = "地脈仕掛-閃",
		iconSize = "",
		type = 10,
		id = 200647,
		icon = "200647",
		buff_type = 0,
		desc = "地脈仕掛が敵に制御されている。戦闘中、敵の支援弾幕・敵はダメージを軽減できるシールドをもつ",
		arg = {}
	}
	pg.base.strategy_data_template[200648] = {
		buff_id = 200648,
		name = "地脈仕掛-繚",
		iconSize = "",
		type = 10,
		id = 200648,
		icon = "200648",
		buff_type = 0,
		desc = "地脈仕掛が敵に制御されている。戦闘中、敵の支援弾幕が出現する他、敵に耐久回復効果が付与される",
		arg = {}
	}
	pg.base.strategy_data_template[200649] = {
		buff_id = 200649,
		name = "地脈仕掛-絢",
		iconSize = "",
		type = 10,
		id = 200649,
		icon = "200649",
		buff_type = 0,
		desc = "地脈仕掛が敵に制御されている。戦闘中、敵の支援弾幕が出現し、命中した艦船は中毒状態になり、持続ダメージを受ける",
		arg = {}
	}
	pg.base.strategy_data_template[200655] = {
		buff_id = 200655,
		name = "地脈仕掛-陰",
		iconSize = "",
		type = 10,
		id = 200655,
		icon = "200650",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200658] = {
		buff_id = 200658,
		name = "地脈仕掛-陽",
		iconSize = "",
		type = 10,
		id = 200658,
		icon = "200651",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200661] = {
		buff_id = 200661,
		name = "地脈仕掛-閃",
		iconSize = "",
		type = 10,
		id = 200661,
		icon = "200652",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕・味方はダメージを軽減できるシールドをもつ",
		arg = {}
	}
	pg.base.strategy_data_template[200664] = {
		buff_id = 200664,
		name = "地脈仕掛-繚",
		iconSize = "",
		type = 10,
		id = 200664,
		icon = "200653",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕が出現する他、味方に耐久回復効果が付与される",
		arg = {}
	}
	pg.base.strategy_data_template[200667] = {
		buff_id = 200667,
		name = "地脈仕掛-陰",
		iconSize = "",
		type = 10,
		id = 200667,
		icon = "200650",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200670] = {
		buff_id = 200670,
		name = "地脈仕掛-陽",
		iconSize = "",
		type = 10,
		id = 200670,
		icon = "200651",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、敵味方支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200673] = {
		buff_id = 200673,
		name = "地脈仕掛-閃",
		iconSize = "",
		type = 10,
		id = 200673,
		icon = "200652",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕・味方はダメージを軽減できるシールドをもつ",
		arg = {}
	}
	pg.base.strategy_data_template[200676] = {
		buff_id = 200676,
		name = "地脈仕掛-繚",
		iconSize = "",
		type = 10,
		id = 200676,
		icon = "200653",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕が出現する他、味方に耐久回復効果が付与される",
		arg = {}
	}
	pg.base.strategy_data_template[200679] = {
		buff_id = 200679,
		name = "地脈仕掛-絢",
		iconSize = "",
		type = 10,
		id = 200679,
		icon = "200654",
		buff_type = 0,
		desc = "地脈仕掛を味方が制御している。戦闘中、味方の支援弾幕が出現し、命中した敵を中毒状態にして持続ダメージを与える",
		arg = {}
	}
	pg.base.strategy_data_template[200732] = {
		buff_id = 200732,
		name = "浮遊艦隊制圧",
		iconSize = "",
		type = 10,
		id = 200732,
		icon = "200732",
		buff_type = 0,
		desc = "謎の浮遊艦隊によるジャミングを受けている。戦闘中味方の命中・回避が5％ダウンし、さらに受けるダメージが5%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200735] = {
		buff_id = 200735,
		name = "理事会艦隊支援",
		iconSize = "",
		type = 10,
		id = 200735,
		icon = "200735",
		buff_type = 0,
		desc = "理事会艦隊による電子戦闘支援を受けている。戦闘中敵の命中・回避が5%ダウンし、さらに受けるダメージが5%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200770] = {
		buff_id = 200770,
		name = "極寒の氷原",
		iconSize = "",
		type = 10,
		id = 200770,
		icon = "200770",
		buff_type = 0,
		desc = "南極の極寒環境で味方の速力が3ダウンし、回避が10%ダウンする。戦闘中戦場に無差別で被害をもたらす雪嵐が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200774] = {
		buff_id = 200774,
		name = "指揮サポート・「ヘレナ」",
		iconSize = "",
		type = 10,
		id = 200774,
		icon = "200774",
		buff_type = 0,
		desc = "「ヘレナ」の戦術指揮ネットワークへのサポートにより、各艦隊を思うように指揮し連携させることが可能。\n海域の特定回数の戦闘では、連合艦隊による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200777] = {
		buff_id = 200777,
		name = "指揮サポート・「ヘレナ」",
		iconSize = "",
		type = 10,
		id = 200777,
		icon = "200774",
		buff_type = 0,
		desc = "「ヘレナ」の戦術指揮ネットワークへのサポートにより、各艦隊を思うように指揮し連携させることが可能。\n海域の特定回数の戦闘では、連合艦隊による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200780] = {
		buff_id = 200780,
		name = "実験型爆撃機支援",
		iconSize = "",
		type = 10,
		id = 200780,
		icon = "200780",
		buff_type = 0,
		desc = "サラトガの秘密航空兵器の支援により、戦闘中味方の特殊航空弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200783] = {
		buff_id = 200783,
		name = "実験型爆撃機支援",
		iconSize = "",
		type = 10,
		id = 200783,
		icon = "200780",
		buff_type = 0,
		desc = "サラトガの秘密航空兵器の支援により、戦闘中味方の特殊航空弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200902] = {
		buff_id = 200902,
		name = "ソング・オブ・アリザリン",
		iconSize = "",
		type = 10,
		id = 200902,
		icon = "200902",
		buff_type = 0,
		desc = "ユニット「Alizarin」のステージ！戦闘開始から15秒間、歌声に励まされて味方全員の火力・雷装・航空・装填が5%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[200903] = {
		buff_id = 200903,
		name = "ソング・オブ・シアニジン",
		iconSize = "",
		type = 10,
		id = 200903,
		icon = "200903",
		buff_type = 0,
		desc = "ユニット「Cyanidin」のステージ！戦闘開始から15秒間、歌声に励まされて味方全員にそれぞれの耐久上限の2％分のダメージを無効にできるシールドを生成し、さらに15秒間その艦船が受けるダメージを5％軽減する",
		arg = {}
	}
	pg.base.strategy_data_template[200905] = {
		buff_id = 200905,
		name = "スポットライト-アリザリン",
		iconSize = "",
		type = 10,
		id = 200905,
		icon = "",
		buff_type = 0,
		desc = "注目効果",
		arg = {}
	}
	pg.base.strategy_data_template[200907] = {
		buff_id = 200907,
		name = "スポットライト-シアニジン",
		iconSize = "",
		type = 10,
		id = 200907,
		icon = "",
		buff_type = 0,
		desc = "注目効果",
		arg = {}
	}
	pg.base.strategy_data_template[200908] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 200908,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘開始時味方前衛艦隊に艦船が1隻のみ存在する場合、戦闘はチャレンジモードに入り難易度が上昇するが、クリアするとより高いスコアが狙えるようになります。\n\nチャレンジモードの特徴：\n敵が与えるダメージが大幅に上昇し、生成される回転シールドの耐久が無限になる；\n時間経過で敵が与えるダメージがさらに上昇する\n装備・スキル【スクランブル】【ウサウサラストスタンド】が無効になる",
		arg = {}
	}
	pg.base.strategy_data_template[200925] = {
		buff_id = 200925,
		name = "デスブリッジ",
		iconSize = "",
		type = 10,
		id = 200925,
		icon = "200925",
		buff_type = 0,
		desc = "デスブリッジが有効化された。\n戦闘中、敵の「デスシャドウ」からの支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200926] = {
		buff_id = 0,
		name = "偽相体",
		iconSize = "",
		type = 10,
		id = 200926,
		icon = "200926",
		buff_type = 0,
		desc = "/*偽相体*/が撃破されると分裂し、周囲の/*偽相体*/の数を増やす。\n\nーー/*偽相体*/の存在が確認された。/*偽相体*/とは一体…？",
		arg = {}
	}
	pg.base.strategy_data_template[200928] = {
		buff_id = 200928,
		name = "「死」のマルティリウム",
		iconSize = "",
		type = 10,
		id = 200928,
		icon = "200928",
		buff_type = 0,
		desc = "「死」の概念を係留し、基盤の上でマルティリウムとしてそれを具現化する。\n戦闘中、任意の味方が敵を撃破した場合、その味方の全ステータスが1%アップ（最大5層まで）",
		arg = {}
	}
	pg.base.strategy_data_template[200930] = {
		buff_id = 200930,
		name = "「輝ける犠牲」のマルティリウム",
		iconSize = "",
		type = 10,
		id = 200930,
		icon = "200930",
		buff_type = 0,
		desc = "「死」の概念が昇華し、マルティリウムは尊き犠牲を讃頌する。\n戦闘中、任意の味方が敵を撃破した場合、その味方の全ステータスが1%アップ（最大5層まで）。任意の味方が敵を累計10体撃破した場合、特殊支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[200931] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 200931,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘開始時味方艦船が<color=#92fc63>【謎のD装置】</color>を装備している場合、戦闘はチャレンジモードに入り難易度が上昇しますが、クリアするとより高いスコアが狙えるようになります。\n\n   チャレンジモードの特徴：\n 敵が与えるダメージが<color=#ffc038>大幅に上昇し</color>、さらに一部の敵弾は航空攻撃でかき消されず、味方のシールドによって無効にならない。\n 味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。 \n\n装備・スキル【スクランブル】【ウサウサラストスタンド】【絶対絶命】【レインボー・プラン】が無効になる",
		arg = {}
	}
	pg.base.strategy_data_template[200932] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 200932,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘開始時味方艦船が<color=#92fc63>【謎のD装置】</color>を装備している場合、戦闘はチャレンジモードに入り難易度が上昇しますが、クリアするとより高いスコアが狙えるようになります。\n\n   チャレンジモードの特徴：\n 敵が与えるダメージが<color=#ffc038>大幅に上昇し</color>、さらに一部の敵弾は航空攻撃でかき消されず、味方のシールドによって無効にならない。\n 味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。 \n\n装備・スキル【スクランブル】【ウサウサラストスタンド】【絶対絶命】【レインボー・プラン】が無効になる",
		arg = {}
	}
	pg.base.strategy_data_template[200979] = {
		buff_id = 200979,
		name = "蒼紅の幻境",
		iconSize = "",
		type = 10,
		id = 200979,
		icon = "200979",
		buff_type = 0,
		desc = "——戦場は蒼紅の幻境に包まれている。\n戦闘開始15秒間、味方が受ける回復効果が10％ダウン、敵が与えるダメージが5％アップ。",
		arg = {}
	}
	pg.base.strategy_data_template[200980] = {
		buff_id = 200980,
		name = "「嵐」",
		iconSize = "",
		type = 10,
		id = 200980,
		icon = "200980",
		buff_type = 0,
		desc = "——「嵐」が吹き荒れている。\n戦闘中お互いの命中率が5％ダウンし、速力が3ダウンする。",
		arg = {}
	}
	pg.base.strategy_data_template[200983] = {
		buff_id = 0,
		name = "蒼紅の影",
		iconSize = "",
		type = 10,
		id = 200983,
		icon = "200983",
		buff_type = 0,
		desc = "——蒼紅の穢れの影がこちらを見つめている。\n戦闘中、一定の間隔で敵の特殊支援弾幕が出現する。",
		arg = {}
	}
	pg.base.strategy_data_template[200984] = {
		buff_id = 200984,
		name = "凍雨ロケット支援",
		iconSize = "",
		type = 10,
		id = 200984,
		icon = "200984",
		buff_type = 0,
		desc = "——「凍雨」アーセナルシップ、支援攻撃を開始する！\n戦闘中、一定の間隔で敵を凍らせる味方の特殊支援弾幕が出現する。",
		arg = {}
	}
	pg.base.strategy_data_template[200987] = {
		buff_id = 200987,
		name = "凍雨ロケット支援",
		iconSize = "",
		type = 10,
		id = 200987,
		icon = "200984",
		buff_type = 0,
		desc = "——「凍雨」アーセナルシップ、支援攻撃を開始する！\n戦闘中、一定の間隔で敵を凍らせる味方の特殊支援弾幕が出現する。",
		arg = {}
	}
	pg.base.strategy_data_template[200990] = {
		buff_id = 200990,
		name = "凍雨ロケット支援",
		iconSize = "",
		type = 10,
		id = 200990,
		icon = "200984",
		buff_type = 0,
		desc = "——「凍雨」アーセナルシップ、支援攻撃を開始する！\n戦闘中、一定の間隔で敵を凍らせる味方の特殊支援弾幕が出現する。",
		arg = {}
	}
	pg.base.strategy_data_template[200999] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 200999,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘開始時味方艦船が【輸送用高速ドローン】を装備している場合、戦闘はチャレンジモードに入り難易度が上昇しますが、クリアするとより高いスコアが狙えるようになります。\n\n  チャレンジモードの特徴：\n 敵が与えるダメージが大幅に上昇。敵弾がシールドを無視し、射線上の全ての味方艦船にダメージを与える；\n 敵の耐久が一定値以上の場合、味方を支援する「凍雨」アーセナルシップが画面左側に出現する。アーセナルシップは独自の耐久を持ち、一部の敵弾幕には非常に弱いが、敵に高火力の支援攻撃を行う。",
		arg = {}
	}
	pg.base.strategy_data_template[201021] = {
		buff_id = 201021,
		name = "深緋の秘境",
		iconSize = "",
		type = 10,
		id = 201021,
		icon = "201021",
		buff_type = 0,
		desc = "――「天城姉さま、もうすぐ会えますわ」\n戦闘開始15秒間、味方艦船の装填が5％ダウンし、敵の装填が10%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[201022] = {
		buff_id = 0,
		name = "深緋の秘境",
		iconSize = "",
		type = 10,
		id = 201022,
		icon = "201021",
		buff_type = 0,
		desc = "――「天城姉さま、もうすぐ会えますわ」\n戦闘開始15秒間、味方艦船の装填が5％ダウンし、敵の装填が10%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[201024] = {
		buff_id = 201024,
		name = "武蔵の雷鳴",
		iconSize = "",
		type = 10,
		id = 201024,
		icon = "200070",
		buff_type = 0,
		desc = "――「鏡湖に映されし雷雲、我が天剣と共に紫電を放たん」\n戦闘中、敵に耐久最大値に比例するダメージを与える味方の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201027] = {
		buff_id = 201027,
		name = "三笠の覚悟",
		iconSize = "",
		type = 10,
		id = 201027,
		icon = "200057",
		buff_type = 0,
		desc = "――「我は最後の砦、これ以上近づかせはせん！」\n戦闘中、味方の命中・回避が5%アップ、さらに一定の頻度で味方の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201029] = {
		buff_id = 201029,
		name = "加賀の使命",
		iconSize = "",
		type = 10,
		id = 201029,
		icon = "201029",
		buff_type = 0,
		desc = "――「例え地獄に堕ちようとも、お前のことを守る」\n戦闘中、一定の頻度で味方の支援弾幕が出現する。さらに「深緋の秘境」効果を無効にする",
		arg = {}
	}
	pg.base.strategy_data_template[201090] = {
		buff_id = 201090,
		name = "世界サンプル",
		iconSize = "",
		type = 10,
		id = 201090,
		icon = "201090",
		buff_type = 0,
		desc = "戦闘中、味方主力艦隊艦船が受ける炎上ダメージを20％軽減し、回避が5％アップ。\n味方艦隊に重桜艦船が4隻以上いる場合、戦闘開始15秒後、味方の特殊支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201094] = {
		buff_id = 201094,
		name = "ナラクの淵",
		iconSize = "",
		type = 10,
		id = 201094,
		icon = "201094",
		buff_type = 0,
		desc = "戦闘中、味方「META」艦船が与えるダメージが10％アップし、受ける回復効果が10％アップする；それ以外の艦船が受けるダメージが5％アップし、受ける回復効果が8％ダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[201097] = {
		buff_id = 201097,
		name = "蒼紅の炎",
		iconSize = "",
		type = 10,
		id = 201097,
		icon = "201097",
		buff_type = 0,
		desc = "戦闘中味方の特殊支援弾幕が出現し、命中した敵を特殊炎上状態にし、耐久上限に比例してダメージを与える",
		arg = {}
	}
	pg.base.strategy_data_template[201100] = {
		buff_id = 0,
		name = "チャレンジ説明",
		iconSize = "",
		type = 10,
		id = 201100,
		icon = "200908",
		buff_type = 0,
		desc = "ハードモード仕様：\n 敵が与えるダメージが大幅に上昇。敵弾がシールドを無視し、射線上の全ての味方艦船にダメージを与える；\n 敵の「青の影」「紅の影」が受ける徹甲弾/半徹甲弾/魚雷のダメージが大幅に減少するが、受ける炎上ダメージが上昇する。\n 敵耐久度が一定値を下回った場合、海域に味方の法陣が出現し、全ての法陣を起動すると味方の支援攻撃が出現する。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】",
		arg = {}
	}
	pg.base.strategy_data_template[201149] = {
		buff_id = 201149,
		name = "吹き荒れる嵐",
		iconSize = "",
		type = 10,
		id = 201149,
		icon = "200627",
		buff_type = 0,
		desc = "船団が嵐に突入した。テンペスタ陣営を除く味方全員の速力が3・回避が5%ダウン。さらにテンペスタ陣営の艦船が与えるダメージが10％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[201152] = {
		buff_id = 201152,
		name = "静寂の海",
		iconSize = "",
		type = 10,
		id = 201152,
		icon = "201152",
		buff_type = 0,
		desc = "船団が静寂の海に突入した。テンペスタ陣営を除く味方が受ける回復効果が10％ダウン。さらにテンペスタ陣営の艦船が受けるダメージが10％ダウン",
		arg = {}
	}
	pg.base.strategy_data_template[201155] = {
		buff_id = 201155,
		name = "女神の憐れみ",
		iconSize = "",
		type = 10,
		id = 201155,
		icon = "201155",
		buff_type = 0,
		desc = "戦闘開始25秒後味方艦隊の支援弾幕が出現し、さらに15秒間徐々に味方艦隊の耐久が回復する",
		arg = {}
	}
	pg.base.strategy_data_template[201157] = {
		buff_id = 201157,
		name = "ファンシーの力",
		iconSize = "",
		type = 10,
		id = 201157,
		icon = "201157",
		buff_type = 0,
		desc = "船団はファンシーの支援を受けている。戦闘中一定時間ごとにランダムな敵1体はファンシーの触手に襲われ、装甲破壊効果が付与される",
		arg = {}
	}
	pg.base.strategy_data_template[201158] = {
		buff_id = 0,
		name = "チャレンジ説明",
		iconSize = "",
		type = 10,
		id = 201158,
		icon = "200908",
		buff_type = 0,
		desc = "ハードモード仕様：\n 敵が与えるダメージが大幅に上昇。敵弾がシールドを無視し、射線上の全ての味方艦船にダメージを与えるが、時間経過とともに敵が受けるダメージも上昇する。\n 一部の敵攻撃はクイズ形式になっている。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】",
		arg = {}
	}
	pg.base.strategy_data_template[201193] = {
		buff_id = 201193,
		name = "星空の下",
		iconSize = "",
		type = 10,
		id = 201193,
		icon = "201193",
		buff_type = 0,
		desc = "星空の下、全てがはっきりと見える。\n味方艦隊の戦闘時に、不定期で高空から敵の支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201194] = {
		buff_id = 201194,
		name = "IB-50 プテラ",
		iconSize = "",
		type = 10,
		id = 201194,
		icon = "201194",
		buff_type = 0,
		desc = "戦闘中、レジスタンス基地から「オーニソプター・プテラ」による支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201197] = {
		buff_id = 201197,
		name = "エターナル・スター",
		iconSize = "",
		type = 10,
		id = 201197,
		icon = "201197",
		buff_type = 0,
		desc = "戦闘中、定期的に「エターナル・スター」による支援攻撃が出現する。弾幕が敵に命中した場合、ダメージを与え、さらに装甲破壊効果を与える",
		arg = {}
	}
end)()
;(function()
	pg.base.strategy_data_template[201198] = {
		buff_id = 0,
		name = "外なる獣の群れ",
		iconSize = "",
		type = 10,
		id = 201198,
		icon = "201198",
		buff_type = 0,
		desc = "外なる獣の群れが現れり。その勢いは水天の境界線をも変える。\n「外なる獣」系の敵は炎上ダメージを受けない。また周回モード以外は敵が味方艦隊を追跡するようになる",
		arg = {}
	}
	pg.base.strategy_data_template[201199] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201199,
		icon = "200908",
		buff_type = 0,
		desc = "ハードモード仕様：\n 敵が与えるダメージが大幅に上昇。敵弾がシールドを無視し、射線上の全ての味方艦船にダメージを与える；\n 敵が一部回避不能の攻撃を繰り出す時、「アドミラル・ヒッパー(META)」からの支援が出現する。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】",
		arg = {}
	}
	pg.base.strategy_data_template[201250] = {
		buff_id = 201250,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201250,
		icon = "200908",
		buff_type = 0,
		desc = "ハードモード仕様：\n敵が与えるダメージが大幅に上昇。敵の耐久が上限の40％を下回った時、攻撃パターンが変更し、3秒間ダメージを受けにくくなる；10％を下回った時、ラストモードになる。\n味方前衛艦隊の<color=#92fc63>当たり判定は前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n艦船が被弾した時、一定時間無敵になる。\n\n下記装備・スキルは無効になる：\n【ウサウサラストスタンド】、【レインボー・プラン】",
		arg = {}
	}
	pg.base.strategy_data_template[201270] = {
		buff_id = 201270,
		name = "神光の織網",
		iconSize = "",
		type = 10,
		id = 201270,
		icon = "201270",
		buff_type = 0,
		desc = "戦闘中に1度だけ、敵味方両方に影響する神光の衝撃が発生する。\nサディア所属の艦船・ユニット：耐久が少し回復する\nその他の艦船・ユニット：一定時間回避と速力がダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[201276] = {
		buff_id = 201276,
		name = "永夜の領域",
		iconSize = "",
		type = 10,
		id = 201276,
		icon = "201276",
		buff_type = 0,
		desc = "戦闘中、永夜の旗による影響が敵味方両方関係なく適用される。\nMETA艦船：与えるダメージが上昇する\nその他の艦船・ユニット：命中が少しダウン。受ける炎上ダメージが上昇する",
		arg = {}
	}
	pg.base.strategy_data_template[201281] = {
		buff_id = 201281,
		name = "戦車改造域",
		iconSize = "",
		type = 10,
		id = 201281,
		icon = "201281",
		buff_type = 0,
		desc = "メカアービトレイター・チャリオットが戦場を支配している。\nセイレーン：「過熱射撃」を得る\nその他の艦船・ユニット：受けるダメージが少しアップする",
		arg = {}
	}
	pg.base.strategy_data_template[201287] = {
		buff_id = 201287,
		name = "飛空艇団支援",
		iconSize = "",
		type = 10,
		id = 201287,
		icon = "201287",
		buff_type = 0,
		desc = "戦闘中、味方の飛空艇団の支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201301] = {
		buff_id = 201315,
		name = "定期巡回",
		iconSize = "",
		type = 10,
		id = 201301,
		icon = "201301",
		buff_type = 0,
		desc = "戦闘中追加の敵が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201302] = {
		buff_id = 201316,
		name = "怪しい影",
		iconSize = "",
		type = 10,
		id = 201302,
		icon = "201302",
		buff_type = 0,
		desc = "戦闘中敵の潜水艦が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201303] = {
		buff_id = 201317,
		name = "奇妙な音",
		iconSize = "",
		type = 10,
		id = 201303,
		icon = "201303",
		buff_type = 0,
		desc = "戦闘中一定時間経過する毎に味方の速力と命中がダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[201304] = {
		buff_id = 201322,
		name = "来訪者制限",
		iconSize = "",
		type = 10,
		id = 201304,
		icon = "201304",
		buff_type = 0,
		desc = "戦闘開始から30秒間、味方が与えるダメージが少しダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[201305] = {
		buff_id = 201323,
		name = "特別問診",
		iconSize = "",
		type = 10,
		id = 201305,
		icon = "201305",
		buff_type = 0,
		desc = "戦闘中3秒毎に、味方の内、耐久が一番高いキャラの耐久が耐久上限の割合で少しダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[201306] = {
		buff_id = 201326,
		name = "妖艶なる桜",
		iconSize = "",
		type = 10,
		id = 201306,
		icon = "201306",
		buff_type = 0,
		desc = "戦闘中敵の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201307] = {
		buff_id = 201327,
		name = "セキュリティ警戒",
		iconSize = "",
		type = 10,
		id = 201307,
		icon = "201307",
		buff_type = 0,
		desc = "戦闘中敵の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201308] = {
		buff_id = 201328,
		name = "無限ループ回廊",
		iconSize = "",
		type = 10,
		id = 201308,
		icon = "201308",
		buff_type = 0,
		desc = "旗艦以外の敵は一定時間経過する毎に、耐久上限の割合で耐久が回復する。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201309] = {
		buff_id = 201332,
		name = "地下区画探索",
		iconSize = "",
		type = 10,
		id = 201309,
		icon = "201309",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201310] = {
		buff_id = 201401,
		name = "ナースステーション探索",
		iconSize = "",
		type = 10,
		id = 201310,
		icon = "201310",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201311] = {
		buff_id = 201402,
		name = "病室探索",
		iconSize = "",
		type = 10,
		id = 201311,
		icon = "201311",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201312] = {
		buff_id = 201403,
		name = "森探索",
		iconSize = "",
		type = 10,
		id = 201312,
		icon = "201312",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201313] = {
		buff_id = 201404,
		name = "庭院探索",
		iconSize = "",
		type = 10,
		id = 201313,
		icon = "201313",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201314] = {
		buff_id = 201332,
		name = "研究所地下探索",
		iconSize = "",
		type = 10,
		id = 201314,
		icon = "201309",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201315] = {
		buff_id = 201405,
		name = "正門探索",
		iconSize = "",
		type = 10,
		id = 201315,
		icon = "201315",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201316] = {
		buff_id = 201406,
		name = "駐車場探索",
		iconSize = "",
		type = 10,
		id = 201316,
		icon = "201316",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201317] = {
		buff_id = 201407,
		name = "駐車場地下探索",
		iconSize = "",
		type = 10,
		id = 201317,
		icon = "201317",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201318] = {
		buff_id = 201408,
		name = "守衛室探索",
		iconSize = "",
		type = 10,
		id = 201318,
		icon = "201318",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201319] = {
		buff_id = 201409,
		name = "医師オフィス探索",
		iconSize = "",
		type = 10,
		id = 201319,
		icon = "201319",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201320] = {
		buff_id = 201405,
		name = "裏口探索",
		iconSize = "",
		type = 10,
		id = 201320,
		icon = "201315",
		buff_type = 0,
		desc = "探索中により多くのエリアを探索した。探索回数+1、不利効果「エリア探索」を有効化する",
		arg = {}
	}
	pg.base.strategy_data_template[201321] = {
		buff_id = 0,
		name = "エリア探索",
		iconSize = "",
		type = 10,
		id = 201321,
		icon = "201321",
		buff_type = 0,
		desc = "このステージの【探索回数】カウントに応じて下記の効果を発揮する：\n ≥1-味方艦船全員のステータスが15％ダウンし、敵旗艦が与えるダメージが20％アップ；\n ≥2-敵旗艦が「抗体」効果を得る：敵旗艦登場後、味方が主力艦隊砲撃・航空攻撃・魚雷攻撃を発動する度に、同攻撃で受けるダメージを10%/10%/5%軽減するようになる（それぞれのダメージ軽減効果は最大7回まで加算可能）；\n ≥3-味方艦船全員のステータスがさらに15％ダウンし、戦闘中5秒毎に落雷が出現し、耐久の上限を占める割合が一番高い味方艦船の耐久が上限の5％分失われる\n ≥4-味方艦船への回復効果はダメージ効果になり、戦場をゆっくり移動し、中にいる味方前衛艦隊が受けるダメージを50％増加させる渦が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201327] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201327,
		icon = "200908",
		buff_type = 0,
		desc = "ハードモード仕様：\n 敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n 味方前衛艦隊の<color=#92fc63>当たり判定は前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】",
		arg = {}
	}
	pg.base.strategy_data_template[201361] = {
		buff_id = 201361,
		name = "チュリッパの海へ",
		iconSize = "",
		type = 10,
		id = 201361,
		icon = "201361",
		buff_type = 0,
		desc = "戦闘中一定時間ごとに、味方艦船に敵弾を一定回数無効にできる回転シールドを生成する。\n チュリッパ所属艦船がこのシールドを得た場合、耐久が少し回復する",
		arg = {}
	}
	pg.base.strategy_data_template[201365] = {
		buff_id = 201365,
		name = "堰堤防御システム",
		iconSize = "",
		type = 10,
		id = 201365,
		icon = "201365",
		buff_type = 0,
		desc = "戦闘中、堰堤周辺の沿岸砲による支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201377] = {
		buff_id = 0,
		name = "第二類偽相",
		iconSize = "",
		type = 10,
		id = 201377,
		icon = "201377",
		buff_type = 0,
		desc = "「第二類偽相」が撃破された場合、「第二類偽相」が放出され、周りの「第二類偽相」に「第二類偽相」を提供する。\n\n 「第二類偽相」は気づかれた。「第二類偽相」とは一体？",
		arg = {}
	}
	pg.base.strategy_data_template[201378] = {
		buff_id = 201378,
		name = "支援・光翼の獅子",
		iconSize = "",
		type = 10,
		id = 201378,
		icon = "201378",
		buff_type = 0,
		desc = "戦闘中、光翼の獅子による支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201382] = {
		buff_id = 201382,
		name = "支援・列車",
		iconSize = "",
		type = 10,
		id = 201382,
		icon = "201382",
		buff_type = 0,
		desc = "戦闘中、「クイーンズライト号」による支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201386] = {
		buff_id = 201386,
		name = "支援・セイレーン",
		iconSize = "",
		type = 10,
		id = 201386,
		icon = "201386",
		buff_type = 0,
		desc = "戦闘中、コンパイラーが配置したⅡ型標準量産型混成艦隊による支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201410] = {
		buff_id = 201410,
		name = "支援・薔薇の塔",
		iconSize = "",
		type = 10,
		id = 201410,
		icon = "201410",
		buff_type = 0,
		desc = "薔薇の塔が防護バリアを展開した。戦闘中、味方艦隊が受けるダメージが少し軽減される",
		arg = {}
	}
	pg.base.strategy_data_template[201411] = {
		buff_id = 201411,
		name = "制圧・薔薇の塔",
		iconSize = "",
		type = 10,
		id = 201411,
		icon = "201411",
		buff_type = 0,
		desc = "薔薇の塔は制圧バリアを展開した。戦闘中、味方艦隊が与えるダメージが少し低くなる",
		arg = {}
	}
	pg.base.strategy_data_template[201461] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201461,
		icon = "200908",
		buff_type = 0,
		desc = "注意：戦闘中敵ボスが出現するまで<color=#ffc038>雑魚敵が出現する</color>。ボスの一定時間経過で出現するが、敵戦艦・敵空母を早期に撃破することで<color=#92fc63>ボスの出現時間を早めることができる</color>。敵が画面左側に到達しても、味方主力艦隊はダメージを受けない。",
		arg = {}
	}
	pg.base.strategy_data_template[201462] = {
		buff_id = 0,
		name = "EX説明",
		iconSize = "",
		type = 10,
		id = 201462,
		icon = "200908",
		buff_type = 0,
		desc = "注意：戦闘中敵ボスが出現するまで<color=#ffc038>雑魚敵が出現する</color>。ボスの一定時間経過で出現するが、敵戦艦・敵空母を早期に撃破することで<color=#92fc63>ボスの出現時間を早めることができる</color>。敵が画面左側に到達しても、味方主力艦隊はダメージを受けない。\n\n ハードモード仕様：\n 敵が与えるダメージが<color=#ffc038>大幅に上昇</color>。敵弾がシールドを無視する。\n 味方前衛艦隊の<color=#92fc63>被弾判定が大幅に縮小され、同時に先頭位置の艦船のみが被弾判定を持ち</color>、その他の艦船は無敵状態になる。\n 艦船が被弾判定を受けた際、短時間の無敵がになる。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】",
		arg = {}
	}
	pg.base.strategy_data_template[201463] = {
		buff_id = 0,
		name = "チャレンジ説明",
		iconSize = "",
		type = 10,
		id = 201463,
		icon = "200908",
		buff_type = 0,
		desc = "ボスは<color=#92fc63>黒と白の2つの形態があり</color>、それぞれに耐久と戦い方が存在する。黒形態では<color=#92fc63>受けるダメージが固定で1になる</color>。\n ボスの現在の形態がすべての攻撃を行い、もう一つの形態の耐久が0でない限り、その形態に切り替わる。また、ボスの現在の形態の耐久が0になった場合も、もう一つの形態に切り替わる；切り替わった時、切り替わった先の形態は<color=#92fc63>凶暴化</color>状態になり、与えるダメージと受けるダメージが100％アップ（黒形態が受けるダメージは固定で2になる）。",
		arg = {}
	}
	pg.base.strategy_data_template[201464] = {
		buff_id = 0,
		name = "チャレンジ説明",
		iconSize = "",
		type = 10,
		id = 201464,
		icon = "200908",
		buff_type = 0,
		desc = "ボスは<color=#92fc63>黒と白の2つの形態があり</color>、それぞれに耐久と戦い方が存在する。黒形態では<color=#92fc63>受けるダメージが固定で1になる</color>。\n ボスの現在の形態がすべての攻撃を行い、もう一つの形態の耐久が0でない限り、その形態に切り替わる。また、ボスの現在の形態の耐久が0になった場合も、もう一つの形態に切り替わる；切り替わった時、切り替わった先の形態は<color=#92fc63>凶暴化</color>状態になり、与えるダメージと受けるダメージが100％アップ（黒形態が受けるダメージは固定で2になる）。\n\n ハードモード仕様：\n 敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n 味方前衛艦隊の<color=#92fc63>当たり判定は前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】",
		arg = {}
	}
	pg.base.strategy_data_template[201515] = {
		buff_id = 201515,
		name = "精神同調Ⅰ",
		iconSize = "",
		type = 10,
		id = 201515,
		icon = "8918",
		buff_type = 0,
		desc = "——「███解除：22.8％」——\n視界に正体不明の幻像が現れ始めた。\n戦闘中、無差別に敵味方を攻撃する弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201516] = {
		buff_id = 201515,
		name = "精神同調Ⅱ",
		iconSize = "",
		type = 10,
		id = 201516,
		icon = "8919",
		buff_type = 0,
		desc = "——「███解除：37.1％」——\n幻像と現実の境界が薄れ始めた。\n戦闘中、無差別に敵味方を攻撃する弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[201517] = {
		buff_id = 201515,
		name = "精神同調Ⅲ",
		iconSize = "",
		type = 10,
		id = 201517,
		icon = "8920",
		buff_type = 0,
		desc = "——「███解除：89.9％」——\n現実は幻、幻は現実。\n戦闘中、無差別に敵味方を攻撃する弾幕が出現します",
		arg = {}
	}
	pg.base.strategy_data_template[201519] = {
		buff_id = 201519,
		name = "アマハラ加護-蝶",
		iconSize = "",
		type = 10,
		id = 201519,
		icon = "201519",
		buff_type = 0,
		desc = "アマハラ加護結界：蝶\n戦闘中、味方が投射砲撃・航空攻撃・魚雷攻撃を発動時、蝶の特殊弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201525] = {
		buff_id = 201525,
		name = "アマハラ加護-羽",
		iconSize = "",
		type = 10,
		id = 201525,
		icon = "201525",
		buff_type = 0,
		desc = "アマハラ加護結界：羽\n戦闘中20秒毎に、味方を支援する白鳳凰が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201530] = {
		buff_id = 201530,
		name = "アマハラ加護-鱗",
		iconSize = "",
		type = 10,
		id = 201530,
		icon = "201530",
		buff_type = 0,
		desc = "アマハラ加護結界：鱗\n戦闘開始後、味方を支援する竜が出現し、一定時間後に消滅する",
		arg = {}
	}
	pg.base.strategy_data_template[201531] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201531,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、ボスに随伴する赤い光の玉が出現する。光の玉はダメージを与えられないが、薫りの煙幕を散布し続ける。艦船が薫りの煙幕内にいる限り、その影響を受け、時間経過とともに速力がダウンし・受けるダメージがアップする；煙幕に累計で1秒間いると眩暈状態になり、受けるダメージが倍になり、煙幕が消えるまで攻撃・移動できなくなる。\n 煙幕から出ると、受けた煙幕の影響が半分の速さで消失する。",
		arg = {}
	}
	pg.base.strategy_data_template[201532] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201532,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、ボスに随伴する赤い光の玉が出現する。光の玉はダメージを与えられないが、薫りの煙幕を散布し続ける。当たり判定が有効な味方艦船が薫りの煙幕内にいる限り、時間経過とともに速力がダウンし・受けるダメージがアップする；煙幕に累計で1秒間いると眩暈状態になり、受けるダメージが倍になり、煙幕が消えるまで攻撃・移動できなくなる。\n 煙幕から出ると、受けた煙幕の影響が半分の速さで消失する。\n\n  ハードモード仕様：\n 敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n 味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】",
		arg = {}
	}
	pg.base.strategy_data_template[201547] = {
		buff_id = 201547,
		name = "群島遺跡支援",
		iconSize = "",
		type = 10,
		id = 201547,
		icon = "201547",
		buff_type = 0,
		desc = "戦闘中20秒毎に、味方に群島遺跡からの支援砲撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201550] = {
		buff_id = 201550,
		name = "群島遺跡支援",
		iconSize = "",
		type = 10,
		id = 201550,
		icon = "201547",
		buff_type = 0,
		desc = "戦闘中20秒毎に、味方に群島遺跡からの支援砲撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201551] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201551,
		icon = "200908",
		buff_type = 0,
		desc = "「真の決闘は公平でなければ！」\n ライムは戦いにおける「不公平」の要素を排除しようとする：\n 1.戦闘中敵味方共に<color=#92fc63>耐久を回復できない</color>。\n 2.いずれかの味方が【完全回避】または【無敵】効果を得た時、ライムも<color=#92fc63>受けたダメージを1にする</color>シールドを得る。\n 3.味方の航空攻撃で全画面の敵弾をクリアした場合、1.5秒後ライムも<color=#92fc63>全画面の弾をクリアし、その後反撃の弾幕を展開する</color>。味方が航空攻撃を発動した場合、この効果は5秒以内で2回以上発動できない",
		arg = {}
	}
	pg.base.strategy_data_template[201552] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201552,
		icon = "200908",
		buff_type = 0,
		desc = "「真の決闘は公平でなければ！」\n ライムは戦いにおける「不公平」の要素を排除しようとする：\n 1.戦闘中敵味方共に<color=#92fc63>耐久を回復できない</color>。\n 2.いずれかの味方が【完全回避】または【無敵】効果を得た時、ライムも<color=#92fc63>受けたダメージを1にする</color>シールドを得る。\n 3.味方の航空攻撃で全画面の敵弾をクリアした場合、1.5秒後ライムも<color=#92fc63>全画面の弾をクリアし、その後反撃の弾幕を展開する</color>。味方が航空攻撃を発動した場合、この効果は5秒以内で2回以上発動できない\n\n   ハードモード仕様：\n 敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n 味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】",
		arg = {}
	}
	pg.base.strategy_data_template[201650] = {
		buff_id = 0,
		name = "スター・ヴェール",
		iconSize = "",
		type = 10,
		id = 201650,
		icon = "201650",
		buff_type = 0,
		desc = "スター・ヴェールでは、一部の[第一種偽相体]が[第二種偽相体]になる。[第一種偽相体]の増援が抑制され、海域マップでの大規模敵艦隊の出現確率が低くなる",
		arg = {}
	}
	pg.base.strategy_data_template[201651] = {
		buff_id = 201651,
		name = "セーフティコントロール・エクスペリエンス",
		iconSize = "",
		type = 10,
		id = 201651,
		icon = "201651",
		buff_type = 0,
		desc = "戦闘中、アビータのエンフォーサーによる支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201654] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201654,
		icon = "200908",
		buff_type = 0,
		desc = "ボスは現在の耐久に応じて（<color=#92fc63>71%以上・70%以下・40%以下</color>）3種類の敵に擬態することがある。擬態状態のボスは受けるダメージが<color=#92fc63>50%軽減される</color>。\n擬態状態のボスの耐久上限は固定であり、<color=#92fc63>耐久が0になると擬態状態を解除し、8秒後再度擬態する</color>。ボスの装甲タイプは擬態状態でない場合は軽装甲になり、艤装状態では重装甲になる",
		arg = {}
	}
	pg.base.strategy_data_template[201655] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201655,
		icon = "200908",
		buff_type = 0,
		desc = "ボスは現在の耐久に応じて（<color=#92fc63>71%以上・70%以下・40%以下</color>）3種類の敵に擬態することがある。擬態状態のボスは受けるダメージが<color=#92fc63>50%軽減される</color>。\n擬態状態のボスの耐久上限は固定であり、<color=#92fc63>耐久が0になると擬態状態を解除し、8秒後再度擬態する</color>。ボスの装甲タイプは擬態状態でない場合は軽装甲になり、艤装状態では重装甲になる\n\nハードモード仕様：\n敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n艦船が被弾した時、一定時間無敵になる。\n\n下記装備・スキルは無効になる：\n【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】",
		arg = {}
	}
	pg.base.strategy_data_template[201701] = {
		buff_id = 201701,
		name = "金運上々",
		iconSize = "",
		type = 10,
		id = 201701,
		icon = "201701",
		buff_type = 0,
		desc = "戦闘中宝貨をモチーフとした支援攻撃が出現する ",
		arg = {}
	}
	pg.base.strategy_data_template[201705] = {
		buff_id = 201705,
		name = "光るメニュー！",
		iconSize = "",
		type = 10,
		id = 201705,
		icon = "201705",
		buff_type = 0,
		desc = "戦闘中、ランダムなメニューの効果が発動します。\n1.戦闘開始時、味方全員の耐久が1％分回復する\n2.戦闘開始30秒間、味方速力が10％アップし、与えるダメージが3%アップ\n3.味方前衛艦隊がキラキラ輝く ",
		arg = {}
	}
	pg.base.strategy_data_template[201706] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201706,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、ボスは大量の碁石を発射する。碁石は破壊されない。\n白い碁石は軽装甲タイプの艦船に大ダメージを与え、黒い碁石は中装甲・重装甲タイプの艦船に大ダメージを与える ",
		arg = {}
	}
	pg.base.strategy_data_template[201707] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201707,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、ボスは大量の碁石を発射する。\n味方艦船が碁石に触れた時、自身が<color=#92fc63>「色標記」を持っていない</color>場合、その碁石の色の「色標記」が付与される；<color=#92fc63>持っている「色標記」がその碁石の色と異なる場合</color>、自身が持っている「色標記」がその碁石の色に変わる；自身が<color=#92fc63>碁石と同じ色の「色標記」を持っている場合</color>、極めて高い<color=#ffc038>固定ダメージ</color>を受ける（この固定ダメージは0.5秒に1度しか発生できない。ボスが発射した碁石は破壊されない。味方が航空攻撃を行った場合、全ての「色標記」を削除する。\n\nハードモード仕様：\n敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n艦船が被弾した時、一定時間無敵になる。\n\n下記装備・スキルは無効になる：\n【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】 ",
		arg = {}
	}
	pg.base.strategy_data_template[201741] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201741,
		icon = "200908",
		buff_type = 0,
		desc = "ボスは自身から距離が20以上離れた相手から受けるダメージを軽減する<color=#92fc63>（距離が20より1離れる度に受けるダメージを1％軽減する。最大50％まで）</color>；距離が20以上離れた相手に与えるダメージが増加する<color=#92fc63>（距離が20より1離れる度に受けるダメージが2％増加する。最大100％まで）</color>。\nボスが<color=#92fc63>前衛艦隊から受ける砲撃ダメージが100％アップし、雷撃ダメージが50％アップする</color>。\nボスは接近してくる爆弾を投下できる艦載機を極めて速く撃墜できる。\n<color=#ddf700>ヒント：ボスの初期位置から味方前衛艦隊の移動できる範囲の最左端までの距離は約70</color>",
		arg = {}
	}
	pg.base.strategy_data_template[201742] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201742,
		icon = "200908",
		buff_type = 0,
		desc = "ボスは自身から距離が20以上離れた相手から受けるダメージを軽減する<color=#92fc63>（距離が20より1長くなる度に受けるダメージを1％軽減する。最大50％まで）</color>；距離が20以上離れる相手に与えるダメージが増加する<color=#92fc63>（距離が20より1長くなる度に受けるダメージを2％増加する。最大100％まで）</color>。\nボスが<color=#92fc63>前衛艦隊から受ける砲撃ダメージが100％アップし、雷撃ダメージが50％アップする</color>。\nボスは接近してくる爆弾を投下できる艦載機を極めて速く撃墜できる。\n<color=#ddf700>ヒント：ボスの初期位置から味方前衛艦隊の移動できる範囲の最左端までの距離は約70</color>\n\nハードモード仕様：\n敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n艦船が被弾した時、一定時間無敵になる。\n\n下記装備・スキルは無効になる：\n【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】、【決して屈さぬぞ！】",
		arg = {}
	}
	pg.base.strategy_data_template[201743] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201743,
		icon = "200908",
		buff_type = 0,
		desc = "任意のボスが撃破されると、もう一人のボスが【FEVER】モードになり、<color=#ffc038>攻撃方法が変更し、与えるダメージが大幅にアップ</color>。 ハードモード仕様：\n 敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消されない。\n 味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】、【決して屈さぬぞ！】",
		arg = {}
	}
	pg.base.strategy_data_template[201750] = {
		buff_id = 0,
		name = "黒き領域の侵食",
		iconSize = "",
		type = 10,
		id = 201750,
		icon = "201750",
		buff_type = 0,
		desc = "悪意具現体が撃破されると、悪意具現体を出現させ、周りの悪意具現体の数を増やす。\n——黒き領域の侵食により、悪意が増幅する",
		arg = {}
	}
	pg.base.strategy_data_template[201754] = {
		buff_id = 201754,
		name = "審判号の支援",
		iconSize = "",
		type = 10,
		id = 201754,
		icon = "201754",
		buff_type = 0,
		desc = "「審判号」による偵察支援で、戦闘中味方の命中・与えるクリティカルダメージが5％アップ",
		arg = {}
	}
	pg.base.strategy_data_template[201755] = {
		buff_id = 201755,
		name = "黒き太陽",
		iconSize = "",
		type = 10,
		id = 201755,
		icon = "201755",
		buff_type = 0,
		desc = "海域での戦闘中、審判型武装機構による支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201759] = {
		buff_id = 201759,
		name = "神光の織網",
		iconSize = "",
		type = 10,
		id = 201759,
		icon = "201270",
		buff_type = 0,
		desc = "海域での戦闘中、神光の織網による支援が出現し、命中した敵の回避と速力をダウンさせる",
		arg = {}
	}
	pg.base.strategy_data_template[201763] = {
		buff_id = 201763,
		name = "オービタルストライク",
		iconSize = "",
		type = 10,
		id = 201763,
		icon = "201763",
		buff_type = 0,
		desc = "海域での戦闘中、オービタル・キャノンによる支援攻撃が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201764] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201764,
		icon = "200908",
		buff_type = 0,
		desc = "ボスは<color=#92fc63>耐久が20%を下回ると強化状態になり</color>、3秒間持続するダメージ軽減効果を受け、更に耐久が上限の30％回復する。その後攻撃形態が変わり、攻撃が激しくなるが<color=#92fc63>1秒毎に耐久が上限の0.5%失う</color>。",
		arg = {}
	}
	pg.base.strategy_data_template[201765] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201765,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中味方が受ける回復効果はボスに奪われ、一度につきボスの耐久が<color=#92fc63>上限の1％分回復される</color>。ボスの黒い弾幕は航空攻撃によって消すことができない。\n ボスは<color=#92fc63>耐久が20%を下回ると強化状態になり</color>、3秒間持続するダメージ軽減効果を受け、更に耐久が上限の30％回復する。その後攻撃形態が変わり、攻撃が激しくなるが<color=#92fc63>1秒毎に耐久が上限の0.5%失う</color>。\n\n  ハードモード仕様：\n 敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消すことができない。\n 味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n 艦船が被弾した時、一定時間無敵になる。\n\n 下記装備・スキルは無効になる：\n 【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】、【決して屈さぬぞ！】",
		arg = {}
	}
	pg.base.strategy_data_template[201820] = {
		buff_id = 0,
		name = "夢光のアストリウム",
		iconSize = "",
		type = 10,
		id = 201820,
		icon = "201820",
		buff_type = 0,
		desc = "メトロスターライトで、感情の片影が散り散りになっている。\n戦闘中「感情の片影」が出現し、撃破されるとさらに多くの「感情の片影」を生み出す場合がある",
		arg = {}
	}
	pg.base.strategy_data_template[201821] = {
		buff_id = 201821,
		name = "雨中の花海",
		iconSize = "",
		type = 10,
		id = 201821,
		icon = "201821",
		buff_type = 0,
		desc = "戦闘中にヨークタウン(META)の支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201826] = {
		buff_id = 201826,
		name = "侵食性錯体",
		iconSize = "",
		type = 10,
		id = 201826,
		icon = "201826",
		buff_type = 0,
		desc = "戦闘中にハイエロファントの支援が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201831] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201831,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、ボノム・リシャールの周りに赤黒い竜巻が出現し、接近する味方を吸い込み、竜巻に近づき過ぎた味方は<color=#92fc63>持続でダメージを受けるようになる</color>。\n特定の状況下で、戦場に風向きの告知が出現し、味方がボノム・リシャールの左（西）か右（東）の特定の範囲内にいると【侵食】状態になり、<color=#92fc63>1秒毎に耐久上限の0.5%分のダメージを受け、さらに敵弾から受けるダメージが20％アップ</color>。",
		arg = {}
	}
	pg.base.strategy_data_template[201832] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201832,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、ボノム・リシャールの周りに赤黒い竜巻が出現し、接近する味方を吸い込み、竜巻に近づき過ぎた味方は<color=#ee0000>竜巻の影響で戦闘不能</color>になる。\n特定の状況下で、戦場に風向きの告知が出現し、味方前衛艦隊の先頭艦船が<color=#92fc63>ボノム・リシャールの左（西）か右（東）の特定の範囲内にいる</color>と、<color=#92fc63>一定時間毎に味方前衛艦隊全員に【侵食】</color>を1個付与する。【侵食】は最大10個まで蓄積され、前衛艦隊の先頭艦船が範囲から離れると一定時間毎に1個削除される。\n【侵食】がある味方艦船は下記の効果が適用される：\nMETAではない艦船：艦船の【侵食】が10個になると、その艦船は竜巻の影響で戦闘不能になる；\nMETA艦船：【侵食】1個につき、自身が<color=#92fc63>受けるダメージと与えるダメージが10％アップ</color>するものの、【侵食】が10個になると<color=#92fc63>1秒毎に耐久上限の2％分の耐久を失う</color>\n\nハードモード仕様：\n敵が与えるダメージが<color=#ffc038>極大アップ</color>。大きなサイズの敵弾は航空攻撃・シールドによって消すことができない。\n味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n艦船が被弾した時、一定時間無敵になる。\n\n下記装備・スキルは無効になる：\n【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】、【決して屈さぬぞ！】",
		arg = {}
	}
	pg.base.strategy_data_template[201901] = {
		buff_id = 201901,
		name = "怪異空間",
		iconSize = "",
		type = 10,
		id = 201901,
		icon = "201901",
		buff_type = 0,
		desc = "不気味な霧が立ち込めている。戦闘中、味方の回避と命中が5％ダウンし、敵は与ダメージが5％アップし、被ダメージが5％ダウンする",
		arg = {}
	}
	pg.base.strategy_data_template[201905] = {
		buff_id = 201905,
		name = "共鳴の声",
		iconSize = "",
		type = 10,
		id = 201905,
		icon = "201905",
		buff_type = 0,
		desc = "戦闘中、タイガーによる支援弾幕が出現する",
		arg = {}
	}
	pg.base.strategy_data_template[201909] = {
		buff_id = 201909,
		name = "一致団結",
		iconSize = "",
		type = 10,
		id = 201909,
		icon = "201909",
		buff_type = 0,
		desc = "戦闘中、味方の数が多いほど強化効果を得る。味方1隻につき、味方全体の与ダメージが1％アップし、受けるダメージを1％軽減する（最大10隻分まで）",
		arg = {}
	}
	pg.base.strategy_data_template[201910] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201910,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、安土の耐久が初めて20％を下回った時に1度だけ発動、<color=#92fc63>暴走状態になり、さらに15秒間あらゆるダメージを無効化する</color>",
		arg = {}
	}
	pg.base.strategy_data_template[201911] = {
		buff_id = 0,
		name = "チャレンジモード",
		iconSize = "",
		type = 10,
		id = 201911,
		icon = "200908",
		buff_type = 0,
		desc = "戦闘中、安土の耐久が20％を下回った時に発動、<color=#92fc63>暴走状態になり、さらに15秒間あらゆるダメージを無効化する</color>；暴走状態終了後、45秒間はこの効果が発動できなくなる。\n暴走状態の安土が発射する弾はダメージを与えなくなるが、弾が味方に命中する度に安土の<color=#92fc63>耐久が上限の3％回復する</color>。\n\nハードモード仕様：\n味方前衛艦隊の<color=#92fc63>当たり判定が前衛艦隊先頭艦だけになり、さらに当たり判定の大きさが大幅に小さくなる</color>。\n艦船が被弾した時、一定時間無敵になる。\n\n敵が与えるダメージが<color=#ffc038>極大アップ</color>し、さらに敵弾は航空攻撃など一部の方法によって消すことができなくなるが、味方が航空攻撃を発動した時、前衛艦隊先頭の艦船に3秒間持続・すべてのダメージを無効にできる強化を付与する。\n下記装備・スキルは無効になる：\n【ウサウサラストスタンド】、【レインボー・プラン】、【絶対絶命】、【決して屈さぬぞ！】",
		arg = {}
	}
	pg.base.strategy_data_template[205001] = {
		buff_id = 205001,
		name = "活动关卡标志A",
		iconSize = "",
		type = 10,
		id = 205001,
		icon = "",
		buff_type = 0,
		desc = "仅用作标记",
		arg = {}
	}
	pg.base.strategy_data_template[205002] = {
		buff_id = 205002,
		name = "活动关卡标志B",
		iconSize = "",
		type = 10,
		id = 205002,
		icon = "",
		buff_type = 0,
		desc = "仅用作标记",
		arg = {}
	}
	pg.base.strategy_data_template[205003] = {
		buff_id = 205003,
		name = "活动关卡标志C",
		iconSize = "",
		type = 10,
		id = 205003,
		icon = "",
		buff_type = 0,
		desc = "仅用作标记",
		arg = {}
	}
	pg.base.strategy_data_template[205004] = {
		buff_id = 205004,
		name = "活动关卡标志D",
		iconSize = "",
		type = 10,
		id = 205004,
		icon = "",
		buff_type = 0,
		desc = "仅用作标记",
		arg = {}
	}
	pg.base.strategy_data_template[205005] = {
		buff_id = 205005,
		name = "活动关卡标志SP",
		iconSize = "",
		type = 10,
		id = 205005,
		icon = "",
		buff_type = 0,
		desc = "仅用作标记",
		arg = {}
	}
	pg.base.strategy_data_template[205006] = {
		buff_id = 205006,
		name = "活动关卡标志B3",
		iconSize = "",
		type = 10,
		id = 205006,
		icon = "",
		buff_type = 0,
		desc = "仅用作标记",
		arg = {}
	}
	pg.base.strategy_data_template[205007] = {
		buff_id = 205007,
		name = "活动关卡标志D3",
		iconSize = "",
		type = 10,
		id = 205007,
		icon = "",
		buff_type = 0,
		desc = "仅用作标记",
		arg = {}
	}
	pg.base.strategy_data_template[300001] = {
		buff_id = 0,
		name = "コアレベルLv1",
		iconSize = "",
		type = 10,
		id = 300001,
		icon = "buff_yumia_1",
		buff_type = 0,
		desc = "戦闘中、味方の火力・航空・雷装が3%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[300002] = {
		buff_id = 0,
		name = "コアレベルLv2",
		iconSize = "",
		type = 10,
		id = 300002,
		icon = "buff_yumia_2",
		buff_type = 0,
		desc = "戦闘中15秒毎に、味方の耐久が上限を占める割合が一番低いキャラの耐久が上限の1％分回復する",
		arg = {}
	}
	pg.base.strategy_data_template[300003] = {
		buff_id = 0,
		name = "コアレベルLv3",
		iconSize = "",
		type = 10,
		id = 300003,
		icon = "buff_yumia_3",
		buff_type = 0,
		desc = "戦闘開始後15秒及びその後20秒毎に支援弾幕を展開し、この弾幕が命中した敵は8秒間受けるダメージが3％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[300004] = {
		buff_id = 0,
		name = "コアレベルLv4",
		iconSize = "",
		type = 10,
		id = 300004,
		icon = "buff_yumia_4",
		buff_type = 0,
		desc = "戦闘で味方キャラが入手する好感度がアップする",
		arg = {}
	}
	pg.base.strategy_data_template[300005] = {
		buff_id = 0,
		name = "コアレベルLv1",
		iconSize = "",
		type = 10,
		id = 300005,
		icon = "buff_yumia_1",
		buff_type = 0,
		desc = "戦闘中、味方の火力・航空・雷装が3%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[300006] = {
		buff_id = 0,
		name = "コアレベルLv2",
		iconSize = "",
		type = 10,
		id = 300006,
		icon = "buff_yumia_2",
		buff_type = 0,
		desc = "戦闘中15秒毎に、味方の耐久が上限を占める割合が一番低いキャラの耐久が上限の1％分回復する",
		arg = {}
	}
	pg.base.strategy_data_template[300007] = {
		buff_id = 0,
		name = "コアレベルLv3",
		iconSize = "",
		type = 10,
		id = 300007,
		icon = "buff_yumia_3",
		buff_type = 0,
		desc = "戦闘開始後15秒及びその後20秒毎に支援弾幕を展開し、この弾幕が命中した敵は8秒間受けるダメージが3％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[300008] = {
		buff_id = 0,
		name = "コアレベルLv4",
		iconSize = "",
		type = 10,
		id = 300008,
		icon = "buff_yumia_4",
		buff_type = 0,
		desc = "戦闘で味方キャラが入手する好感度がアップする",
		arg = {}
	}
	pg.base.strategy_data_template[300009] = {
		buff_id = 0,
		name = "コアレベルLv1",
		iconSize = "",
		type = 10,
		id = 300009,
		icon = "buff_yumia_1",
		buff_type = 0,
		desc = "戦闘中、味方の火力・航空・雷装が3%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[300010] = {
		buff_id = 0,
		name = "コアレベルLv2",
		iconSize = "",
		type = 10,
		id = 300010,
		icon = "buff_yumia_2",
		buff_type = 0,
		desc = "戦闘中15秒毎に、味方の耐久が上限を占める割合が一番低いキャラの耐久が上限の1％分回復する",
		arg = {}
	}
	pg.base.strategy_data_template[300011] = {
		buff_id = 0,
		name = "コアレベルLv3",
		iconSize = "",
		type = 10,
		id = 300011,
		icon = "buff_yumia_3",
		buff_type = 0,
		desc = "戦闘開始後15秒及びその後20秒毎に支援弾幕を展開し、この弾幕が命中した敵は8秒間受けるダメージが3％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[300012] = {
		buff_id = 0,
		name = "コアレベルLv4",
		iconSize = "",
		type = 10,
		id = 300012,
		icon = "buff_yumia_4",
		buff_type = 0,
		desc = "戦闘で味方キャラが入手する好感度がアップする",
		arg = {}
	}
end)()
;(function()
	pg.base.strategy_data_template[300013] = {
		buff_id = 0,
		name = "コアレベルLv1",
		iconSize = "",
		type = 10,
		id = 300013,
		icon = "buff_yumia_1",
		buff_type = 0,
		desc = "戦闘中、味方の火力・航空・雷装が3%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[300014] = {
		buff_id = 0,
		name = "コアレベルLv2",
		iconSize = "",
		type = 10,
		id = 300014,
		icon = "buff_yumia_2",
		buff_type = 0,
		desc = "戦闘中15秒毎に、味方の耐久が上限を占める割合が一番低いキャラの耐久が上限の1％分回復する",
		arg = {}
	}
	pg.base.strategy_data_template[300015] = {
		buff_id = 0,
		name = "コアレベルLv3",
		iconSize = "",
		type = 10,
		id = 300015,
		icon = "buff_yumia_3",
		buff_type = 0,
		desc = "戦闘開始後15秒及びその後20秒毎に支援弾幕を展開し、この弾幕が命中した敵は8秒間受けるダメージが3％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[300016] = {
		buff_id = 0,
		name = "コアレベルLv4",
		iconSize = "",
		type = 10,
		id = 300016,
		icon = "buff_yumia_4",
		buff_type = 0,
		desc = "戦闘で味方キャラが入手する好感度がアップする",
		arg = {}
	}
	pg.base.strategy_data_template[300017] = {
		buff_id = 0,
		name = "コアレベルLv1",
		iconSize = "",
		type = 10,
		id = 300017,
		icon = "buff_yumia_1",
		buff_type = 0,
		desc = "戦闘中、味方の火力・航空・雷装が3%アップ",
		arg = {}
	}
	pg.base.strategy_data_template[300018] = {
		buff_id = 0,
		name = "コアレベルLv2",
		iconSize = "",
		type = 10,
		id = 300018,
		icon = "buff_yumia_2",
		buff_type = 0,
		desc = "戦闘中15秒毎に、味方の耐久が上限を占める割合が一番低いキャラの耐久が上限の1％分回復する",
		arg = {}
	}
	pg.base.strategy_data_template[300019] = {
		buff_id = 0,
		name = "コアレベルLv3",
		iconSize = "",
		type = 10,
		id = 300019,
		icon = "buff_yumia_3",
		buff_type = 0,
		desc = "戦闘開始後15秒及びその後20秒毎に支援弾幕を展開し、この弾幕が命中した敵は8秒間受けるダメージが3％アップする",
		arg = {}
	}
	pg.base.strategy_data_template[300020] = {
		buff_id = 0,
		name = "コアレベルLv4",
		iconSize = "",
		type = 10,
		id = 300020,
		icon = "buff_yumia_4",
		buff_type = 0,
		desc = "戦闘で味方キャラが入手する好感度がアップする",
		arg = {}
	}
end)()
