return {
	id = "YOUYINGMICHENG3",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "Just a few minutes ago, this street was the liveliest place in all of the East Side's commercial district.",
			bgm = "battle-visioncity-1",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashin = {
				delay = 0,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "But by the time we arrive, everything has changed. An unseen force has plucked this bright, bustling street out of reality.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "Though it's still midday, a leaden gloom hangs over the city, turning the light cold.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist Operative",
			dir = 1,
			NextIcon = 1,
			actor = 900557,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "It's nothing like the commercial district outside. Rules, distance, direction... Everything is distorted.",
			actorPosition = {
				x = -200,
				y = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			actor = 0,
			NextIcon = 1,
			nameColor = "#A9F548FF",
			say = "Our navigation systems are malfunctioning, too.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Newbie Operative",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "What are we going to do?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist Operative",
			dir = 1,
			actor = 900557,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Isn't it obvious? We're gonna find that poltergeist and capture it.",
			actorPosition = {
				x = -200,
				y = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			actor = 0,
			NextIcon = 1,
			nameColor = "#A9F548FF",
			say = "Tiger, can you sense anything up ahead?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_495",
			NextIcon = 1,
			dir = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Hmm... There are many discordant sounds coming from deep inside the anomaly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_495",
			NextIcon = 1,
			dir = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Grating arguments, intense clashes...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist",
			dir = 1,
			NextIcon = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "It's such an unpleasantly chaotic performance. One can hardly tell at this point whether they're human or poltergeist.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist",
			dir = 1,
			NextIcon = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "...Wait.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "Tiger raises her baton slightly, pointing it toward a side street engulfed by shadow up ahead.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist",
			dir = 1,
			NextIcon = 1,
			actor = 202380,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "One restless melody... draws ever nearer.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Innocent Victim",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "*pant*... *pant*... D-did I manage to shake them?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Innocent Victim",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "It still feels like... something is behind me, staring...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "Unable to bear the fear, Z14 turns around. A black shadow is looming there, silently.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#FF9B93",
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist",
			dir = 1,
			NextIcon = 1,
			actor = 900560,
			actorName = "???",
			hidePaintObj = true,
			say = "...#&...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Innocent Victim",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Huh... Aah?! Oww... I tripped...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "The shadow begins its soundless approach.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Innocent Victim",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "D-don't come any closer!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Newbie Operative",
			dir = 1,
			NextIcon = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "That's as far as you go!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "Dazzling light cuts between them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "The shadow backs off just slightly. After hesitating there for a few seconds, it instantly vanishes from sight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Newbie Operative",
			dir = 1,
			NextIcon = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "I'm on my first official mission, and I... Hey, it ran away! Commander, should we give chase?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist Operative",
			dir = 1,
			NextIcon = 1,
			actor = 900557,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Hold it. It could be a trap.",
			actorPosition = {
				x = -200,
				y = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Innocent Victim",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Wow...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Newbie Operative",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Commander, we've found a survivor!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Newbie Operative",
			dir = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "You're safe now! Ah... You scraped your knee? I'll help you up.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Innocent Victim",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Thanks, umm... Are you that girl who was on the train?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Newbie Operative",
			dir = 1,
			NextIcon = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Huh? Umm...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_170",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "One girl sitting in another seat timidly held up her Urban Survival Guide book.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = true,
					name = "memoryFog"
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_170",
			factiontag = "Passenger",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "A-anyway... I'm more scared of those poltergeists, you know?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Newbie Operative",
			dir = 1,
			NextIcon = 1,
			actor = 231211,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Oh, it's you! Hehe, I'm surprised you remembered me... Especially because I'm in uniform.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = false,
					name = "memoryFog"
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Innocent Victim",
			dir = 1,
			NextIcon = 1,
			actor = 401140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Yeah! You seem so much more reliable now...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Observe Z14.)",
					flag = 1
				}
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "Photos had been attached to the joint investigation request. Z14 was one of the kidnapping victims.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			actor = 0,
			NextIcon = 1,
			nameColor = "#A9F548FF",
			say = "(This anomaly must be connected to that case.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			shakeTime = 3,
			say = "The anomalous space suddenly lurches. Harsh electrical interference engulfs us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "Instantly after, streetside shop windows, glass storefronts, and even reflections in puddles on the ground display the same exact image.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			NextIcon = 1,
			bgName = "star_level_bg_495",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "It's the pale, blurred face of a woman, backed by a pitch-black background.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_495",
			factiontag = "Poltergeist",
			actorName = "???",
			NextIcon = 1,
			actor = 403143,
			nameColor = "#FF9B93",
			hidePaintObj = true,
			say = "You mustn't go any further...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
